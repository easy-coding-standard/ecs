...
class EditorConfigFactory
{
    ...
    public function parse(string $editorConfigFileContents): \Symplify\EasyCodingStandard\Configuration\EditorConfig\EditorConfig
    {
        ...
        $config = array_merge(is_array($fullConfig['*'] ?? []) ? $fullConfig['*'] ?? [] : iterator_to_array(is_array($fullConfig['*'] ?? []) ? new \ArrayIterator($fullConfig['*'] ?? []) : $fullConfig['*'] ?? []), is_array($fullConfig['*.php'] ?? []) ? $fullConfig['*.php'] ?? [] : iterator_to_array(is_array($fullConfig['*.php'] ?? []) ? new \ArrayIterator($fullConfig['*.php'] ?? []) : $fullConfig['*.php'] ?? []));
