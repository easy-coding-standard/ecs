...
final class File extends BaseFile
{
    ...
    public function process() : void
    {
        ...
        foreach ($this->tokens as $stackPtr => $token) {
            ...
            foreach ($this->tokenListeners[$token['code']] as $sniff) {
                ...
                $sniffIsDisabled = $this->isSniffStillDisabled(\get_class($sniff), $stackPtr);
                if ($shouldSkipSniff || $sniffIsDisabled) {
                    continue;
                }
                $this->reportActiveSniffClass($sniff);
                $this->disableSnifferUntil(\get_class($sniff), $sniff->process($this, $stackPtr));
            }
        }
     ...
     */
    protected function addMessage(bool $isError, string $message, int $line, int $column, string $sniffClassOrCode, array $data, int $severity, bool $isFixable = \false) : bool
    {
        // skip warnings
        if (!$isError) {
            return \false;
