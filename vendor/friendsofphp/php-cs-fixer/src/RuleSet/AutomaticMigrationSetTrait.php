...
use ECSPrefix202511\Composer\Semver\Semver;
...
trait AutomaticMigrationSetTrait
{
    ...
    private static function getMigrationSets() : array
    {
        static $sets = null;
        if (null === $sets) {
            $sets = \array_values(\array_filter(\PhpCsFixer\RuleSet\RuleSets::getSetDefinitions(), static function (\PhpCsFixer\RuleSet\RuleSetDefinitionInterface $set) : bool {
                return !$set instanceof \PhpCsFixer\RuleSet\DeprecatedRuleSetDefinitionInterface && \is_subclass_of($set, \PhpCsFixer\RuleSet\AbstractMigrationSetDefinition::class);
            }));
        }
        return $sets;
    }
