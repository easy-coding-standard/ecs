...
final class PERCSRiskySet extends AbstractRuleSetDefinition
{
    ...
    {
        return [$this->getHighestPerCsSet()->getName() => \true];
    }
    ...
    private function getHighestPerCsSet(): RuleSetDefinitionInterface
    {
        ...
        if (null === $set) {
            $currentSet = $this;
            $sets = array_filter(RuleSets::getSetDefinitions(), static function (RuleSetDefinitionInterface $set) use ($currentSet): bool {
                return !$set instanceof DeprecatedRuleSetDefinitionInterface && $set->isRisky() === $currentSet->isRisky() && $set->getName() !== $currentSet->getName() && strncmp($set->getName(), str_replace(':risky', '', $currentSet->getName()), strlen(str_replace(':risky', '', $currentSet->getName()))) === 0;
            });
