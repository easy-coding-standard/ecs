    ...
    private function getHighestPerCsSet(): RuleSetDefinitionInterface
    {
        ...
        if (null === $set) {
            ...
            $sets = array_filter(RuleSets::getSetDefinitions(), static function (RuleSetDefinitionInterface $set) use ($currentSet): bool {
                return !$set instanceof DeprecatedRuleSetDefinitionInterface && $set->isRisky() === $currentSet->isRisky() && $set->getName() !== $currentSet->getName() && strncmp($set->getName(), $currentSet->getName(), strlen($currentSet->getName())) === 0;
            });
