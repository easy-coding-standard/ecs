 ...
 */
abstract class AbstractMigrationSetDefinition extends \PhpCsFixer\RuleSet\AbstractRuleSetDefinition
{
    /**
     * @var string
     */
    ...
    {
        $improvement = ['PHPUnit' => 'tests code'][$this->getEntity()] ?? 'code';
    ...
    private function parseRuleSetName(): void
    {
        ...
        if (Preg::match('#^@PHPUnit(\d+)x?(\d)Migration.*$#', $name, $matches)) {
            $this->entity = 'PHPUnit';
            $this->version = ['major' => (int) $matches[1], 'minor' => (int) $matches[2]];
            return;
        }
        // @TODO v4 - `x?` -> `x` @MARKER_deprecated_migration_name_pattern
