...
use ECSPrefix202605\React\ChildProcess\Process as ReactProcess;
use ECSPrefix202605\React\EventLoop\LoopInterface;
use ECSPrefix202605\React\EventLoop\TimerInterface;
...
final class Process
{
    ...
    public function start(callable $onData, callable $onError, callable $onExit) : void
    {
        ...
        if (!\is_writable($sysTempDir)) {
            throw new \PhpCsFixer\Runner\Parallel\ParallelisationException(\sprintf('Failed creating temp file as sys_get_temp_dir="%s" is not writable.', $sysTempDir));
        }
        $stdOut = \tmpfile();
        if (\false === $stdOut) {
