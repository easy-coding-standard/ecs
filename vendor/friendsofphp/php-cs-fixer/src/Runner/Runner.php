...
use ECSPrefix202605\Symfony\Component\EventDispatcher\EventDispatcherInterface;
use ECSPrefix202605\Symfony\Component\Filesystem\Exception\IOException;
use ECSPrefix202605\Symfony\Contracts\EventDispatcher\Event;
/**
...
final class Runner
{
     ...
     */
    private const PARALLEL_BUFFER_SIZE = 16 * (1024 * 1024);
    /**
     ...
     */
    private $workersMemoryUsageByProcess = [];
    /**
    ...
    private $configFile;
    /**
     * @var \PhpCsFixer\Config\RuleCustomisationPolicyInterface
     */
    ...
    )
    {
        ...
        $this->fixers = $fixers;
        $this->fixersByName = \array_reduce($fixers, static function (array $carry, FixerInterface $fixer) : array {
    ...
    public function fix() : array
    {
        ...
        $this->validateRulesNamesForExceptions(\array_keys($ruleCustomisers), <<<'EOT'
...
EOT
        ...
        if (1 === $this->parallelConfig->getMaxProcesses() || $this->fileCount <= $this->parallelConfig->getFilesPerProcess()) {
            return $this->fixSequential();
        }
    ...
    {
        if (\true === \filter_var(\getenv('PHP_CS_FIXER_IGNORE_MISMATCHED_RULES_EXCEPTIONS'), \FILTER_VALIDATE_BOOLEAN)) {
            return;
        }
        if ([] === $ruleExceptions) {
            return;
        ...
        $missingRulesDesc = \implode("\n", \array_map(static function (string $name) use($fixersByName) : string {
            $extra = '';
            if ('' === $name) {
        ...
        throw new \RuntimeException(\sprintf($errorTemplate, $missingRulesDesc));
    }
    /**
    ...
    private function fixParallel() : array
    {
        ...
        if (!\is_numeric($serverPort)) {
            throw new ParallelisationException(\sprintf('Unable to parse server port from "%s"', $server->getAddress() ?? ''));
        }
        ...
        $server->on('connection', static function (ConnectionInterface $connection) use($processPool, $getFileChunk) : void {
            $decoder = new Decoder($connection, \true, 512, \JSON_INVALID_UTF8_IGNORE, self::PARALLEL_BUFFER_SIZE);
        ...
        $processesToSpawn = \min($this->parallelConfig->getMaxProcesses(), \max(1, (int) \ceil($this->fileCount / $this->parallelConfig->getFilesPerProcess())));
        $processFactory = new ProcessFactory();
        for ($i = 0; $i < $processesToSpawn; ++$i) {
            $identifier = ProcessIdentifier::create();
            $process = $processFactory->create($streamSelectLoop, $this->input, new \PhpCsFixer\Runner\RunnerConfig($this->isDryRun, $this->stopOnViolation, $this->parallelConfig, $this->configFile), $identifier, $serverPort);
            ...
            $process->start(
                ...
                function (array $workerResponse) use($processPool, $process, $identifier, $getFileChunk, &$changed) : void {
                    ...
                    if (ParallelAction::WORKER_RESULT === $workerResponse['action']) {
                        \assert(isset($workerResponse['errors'], $workerResponse['file'], $workerResponse['memoryUsage'], $workerResponse['status']));
                        ...
                        foreach ($workerResponse['errors'] as $error) {
                            $this->errorsManager->report(new Error($error['type'], $error['filePath'], null !== $error['source'] ? SourceExceptionFactory::fromArray($error['source']) : null, $error['appliedFixers'], $error['diff']));
                        }
                    ...
                    if (ParallelAction::WORKER_ERROR_REPORT === $workerResponse['action']) {
                        \assert(isset($workerResponse['class'], $workerResponse['message'], $workerResponse['file'], $workerResponse['line'], $workerResponse['code'], $workerResponse['trace']));
                ...
                static function ($exitCode, string $output) use($processPool, $identifier) : void {
                    ...
                    $errorsReported = Preg::matchAll(\sprintf('/^(?:%s)([^\\n]+)+/m', WorkerCommand::ERROR_PREFIX), $output, $matches);
                    if ($errorsReported > 0) {
                        throw WorkerException::fromRaw(\json_decode($matches[1][0], \true, 512, 0));
                    }
                }
            );
        }
    ...
    private function fixFile(\SplFileInfo $file, LintingResultInterface $lintingResult) : ?array
    {
        ...
        try {
            ...
            $this->dispatchEvent(FileProcessed::NAME, new FileProcessed(FileProcessed::STATUS_INVALID));
        ...
        if (Future::isFutureModeEnabled() && !\filter_var(\getenv('PHP_CS_FIXER_NON_MONOLITHIC'), \FILTER_VALIDATE_BOOL) && !$tokens->isMonolithicPhp()) {
            $this->dispatchEvent(FileProcessed::NAME, new FileProcessed(FileProcessed::STATUS_NON_MONOLITHIC));
            return null;
        }
        ...
        try {
            ...
            throw new \RuntimeException(\sprintf('Error while analysing file "%s": %s', $filePathname, $e->getMessage()), $e->getCode(), $e);
        }
        $this->validateRulesNamesForExceptions($rulesIgnoredByAnnotations, <<<EOT
...
EOT
        ...
        try {
            foreach ($this->fixers as $fixer) {
                if (\in_array($fixer->getName(), $rulesIgnoredByAnnotations, \true)) {
                    continue;
                }
                ...
                if (null !== $customiser) {
                    ...
                    if (\true !== $actualFixer) {
                        if (\get_class($fixer) !== \get_class($actualFixer)) {
                            throw new \RuntimeException(\sprintf('The fixer returned by the Rule Customisation Policy must be of the same class as the original fixer (expected `%s`, got `%s`).', \get_class($fixer), \get_class($actualFixer)));
                        }
                        $fixer = $actualFixer;
                    }
                }
        ...
        if ($oldHash !== $newHash) {
            ...
            if (!$this->isDryRun) {
                ...
                if (\is_dir($fileRealPath)) {
                    throw new IOException(\sprintf('Cannot write file "%s" as the location exists as directory.', $fileRealPath), 0, null, $fileRealPath);
                }
                if (!\is_writable($fileRealPath)) {
                    throw new IOException(\sprintf('Cannot write to file "%s" as it is not writable.', $fileRealPath), 0, null, $fileRealPath);
                }
                if (\false === @\file_put_contents($fileRealPath, $new)) {
                    $error = \error_get_last();
                    throw new IOException(\sprintf('Failed to write file "%s", "%s".', $fileRealPath, null !== $error ? $error['message'] : 'no reason available'), 0, null, $fileRealPath);
                }
            }
        }
        $this->cacheManager->setFileHash($filePathname, $newHash);
        $this->dispatchEvent(FileProcessed::NAME, new FileProcessed(null !== $fixInfo ? FileProcessed::STATUS_FIXED : FileProcessed::STATUS_NO_CHANGES, $newHash));
        return $fixInfo;
    }
    /**
    ...
    private function getLintingFileIterator() : \PhpCsFixer\Runner\LintingResultAwareFileIteratorInterface
    {
        $fileFilterIterator = $this->getFilteringFileIterator();
        return $this->linter->isAsync() ? new \PhpCsFixer\Runner\FileCachingLintingFileIterator($fileFilterIterator, $this->linter) : new \PhpCsFixer\Runner\LintingFileIterator($fileFilterIterator, $this->linter);
    }
    private function getFilteringFileIterator() : \PhpCsFixer\Runner\FileFilterIterator
    {
        ...
        return new \PhpCsFixer\Runner\FileFilterIterator($this->fileIterator instanceof \IteratorAggregate ? $this->fileIterator->getIterator() : $this->fileIterator, $this->eventDispatcher, $this->cacheManager);
    }
}
