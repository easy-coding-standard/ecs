...
' . self::REGEX_TYPE . '
    ...
    private const ALIASES = ['boolean' => 'bool', 'callback' => 'callable', 'double' => 'float', 'false' => 'bool', 'integer' => 'int', 'list' => 'array', 'real' => 'float', 'true' => 'bool'];
    /**
     * @var string
     */
    private $value;
    ...
    public function getTypes(): array
    {
        if ($this->isCompositeType) {
            return array_map(static function (array $type) {
    ...
    public function mapTypes(\Closure $callback): self
    {
        ...
        foreach ($this->innerTypeExpressions as ['start_index' => $startIndexOrig, 'expression' => $inner]) {
            ...
            if ($inner->value !== $innerValueOrig) {
                $value = substr_replace($value, $inner->value, $startIndexOrig + $startIndexOffset, \strlen($innerValueOrig));
                $startIndexOffset += \strlen($inner->value) - \strlen($innerValueOrig);
            }
        }
        $type = $value === $this->value ? $this : $this->inner($value);
    ...
    public function sortTypes(\Closure $compareCallback): self
    {
        return $this->mapTypes(function (self $type) use ($compareCallback): self {
            ...
            $innerTypeExpressions = Utils::stableSort($type->innerTypeExpressions, static function (array $v): self {
                return $v['expression'];
            }, $compareCallback);
            if ($innerTypeExpressions !== $type->innerTypeExpressions) {
                $value = implode($type->getTypesGlue(), array_map(static function (array $v): string {
                    return $v['expression']->toString();
    ...
    public function removeDuplicateTypes(): self
    {
        return $this->mapTypes(function (self $type): self {
            ...
            foreach ($type->innerTypeExpressions as $innerType) {
                $normalized = $innerType['expression']->sortTypes(static function (self $a, self $b): int {
                    return $a->toString() <=> $b->toString();
                })->toString();
                if (!\in_array($normalized, $seenNormalized, \true)) {
                    $seenNormalized[] = $normalized;
                    $uniqueTypeExpressions[] = $innerType['expression'];
                }
            }
            $value = implode($type->getTypesGlue(), array_map(static function (self $expr): string {
    ...
    {
        foreach ($this->getTypes() as $type) {
            if (\in_array($type, ['null', 'mixed'], \true) || strncmp($type, '?', strlen('?')) === 0) {
                return \true;
            }
        }
        return \false;
    ...
    private function parse(): void
    {
        ...
        $index = 0;
        while (\true) {
            Preg::match('{\G' . self::REGEX_TYPE . '(?<glue_raw>\h*(?<glue>[|&])\h*(?!$)|$)}', $this->value, $matches, \PREG_OFFSET_CAPTURE, $index);
            if ([] === $matches) {
            ...
            if (($matches['glue'][0] ?? '') !== '') {
                ...
                $seenGlues[$matches['glue'][0]] = \true;
            }
            $innerValues[] = ['start_index' => $index, 'value' => $matches['type'][0], 'next_glue' => $matches['glue'][0] ?? null, 'next_glue_raw' => $matches['glue_raw'][0] ?? null];
            ...
            if (\strlen($this->value) <= $index) {
                ...
                \assert([] !== $seenGlues);
                $this->isCompositeType = \true;
                reset($seenGlues);
                $this->typesGlue = key($seenGlues);
                if (1 === \count($seenGlues)) {
                    foreach ($innerValues as $innerValue) {
                        $this->innerTypeExpressions[] = ['start_index' => $innerValue['start_index'], 'expression' => $this->inner($innerValue['value'])];
                    }
                } else {
                    for ($i = 0; $i < \count($innerValues); ++$i) {
                        ...
                        while (\true) {
                            ...
                            ++$i;
                            \assert(isset($innerValues[$i]));
                            // for PHPStan
                        }
                        $this->innerTypeExpressions[] = ['start_index' => $innerStartIndex, 'expression' => $this->inner($innerValue)];
                    }
                }
                return;
            }
        ...
        if ('' !== $matches['nullable'][0]) {
            ...
            if ('' !== ($matches['conditional'][0] ?? '')) {
                ...
                $this->innerTypeExpressions[] = ['start_index' => $index, 'expression' => $this->inner($matches['conditional_cond_right_types'][0])];
                $index += \strlen($matches['conditional_cond_right_types'][0]) + \strlen($matches['conditional_true_start'][0]);
                $this->innerTypeExpressions[] = ['start_index' => $index, 'expression' => $this->inner($matches['conditional_true_types'][0])];
    ...
    private function normalize(string $type): string
    {
        ...
        if (\in_array($type, ['array', 'bool', 'callable', 'false', 'float', 'int', 'iterable', 'mixed', 'never', 'null', 'object', 'resource', 'string', 'true', 'void'], \true)) {
            return $type;
        }
        if (Preg::match('/\[\]$/', $type)) {
            return 'array';
