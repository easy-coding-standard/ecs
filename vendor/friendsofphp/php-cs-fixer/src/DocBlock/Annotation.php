...
final class Annotation
{
     ...
     */
    public const TAGS_WITH_TYPES = ['extends', 'implements', 'method', 'param', 'param-out', 'phpstan-import-type', 'phpstan-type', 'phpstan-var', 'property', 'property-read', 'property-write', 'psalm-import-type', 'psalm-type', 'psalm-var', 'return', 'throws', 'type', 'var'];
    /**
    ...
    /**
     * The associated tag.
     * @var \PhpCsFixer\DocBlock\Tag|null
     */
    private $tag;
    /**
    ...
    public function getTypes() : array
    {
        if (null === $this->types) {
            ...
            $this->types = null === $typeExpression ? [] : $typeExpression->getTypes();
        }
        return $this->types;
    }
    /**
    ...
    public function getNormalizedTypes() : array
    {
        ...
        $normalizedTypeExpression = $typeExpression->mapTypes(static function (\PhpCsFixer\DocBlock\TypeExpression $v) {
            return new \PhpCsFixer\DocBlock\TypeExpression(\strtolower($v->toString()), null, []);
        })->sortTypes(static function (\PhpCsFixer\DocBlock\TypeExpression $a, \PhpCsFixer\DocBlock\TypeExpression $b) {
    ...
    {
        return \in_array($this->getTag()->getName(), self::TAGS_WITH_TYPES, \true);
    }
    /**
    ...
    private function getTypesContent() : ?string
    {
        if (null === $this->typesContent) {
            ...
            if (Preg::match('{^(?:\\h*\\*|/\\*\\*)[\\h*]*@' . $name . '\\h+' . \PhpCsFixer\DocBlock\TypeExpression::REGEX_TYPES . '(?:(?:[*\\h\\v]|\\&?[\\.\\$\\s]).*)?\\r?$}is', $this->getContent(), $matches)) {
