...
use ECSPrefix202605\Ergebnis\AgentDetector;
...
use ECSPrefix202605\Symfony\Component\Finder\Finder as SymfonyFinder;
/**
...
final class ConfigurationResolver
{
    ...
    public const PATH_MODE_VALUES = [self::PATH_MODE_OVERRIDE, self::PATH_MODE_INTERSECTION];
    public const BOOL_YES = 'yes';
    public const BOOL_NO = 'no';
    public const BOOL_VALUES = [self::BOOL_YES, self::BOOL_NO];
    /**
     * @TODO v4: this is no longer needed due to `MARKER-multi-paths-vs-only-cwd-config`
     * @var string|null
     */
    private $deprecatedNestedConfigDir;
    /**
     * @var bool|null
     */
    private $allowRisky;
    /**
     * @var \PhpCsFixer\ConfigInterface|null
     */
    private $config;
    /**
     * @var string|null
    ...
    /**
     * @var \PhpCsFixer\Console\Report\FixReport\ReporterInterface|null
     */
    private $reporter;
    /**
     * @var bool|null
     ...
     * @var bool|null
     */
    private $configRulesAreOverridden;
    /**
     ...
     * @var _Options
     */
    private $options = ['allow-risky' => null, 'cache-file' => null, 'config' => null, 'diff' => null, 'dry-run' => null, 'format' => null, 'path' => [], 'path-mode' => self::PATH_MODE_OVERRIDE, 'rules' => null, 'sequential' => null, 'show-progress' => null, 'stop-on-violation' => null, 'using-cache' => null, 'allow-unsupported-php-version' => null, 'verbosity' => null];
    /**
     * @var string|null
     */
    private $cacheFile;
    /**
     * @var \PhpCsFixer\Cache\CacheManagerInterface|null
     */
    private $cacheManager;
    /**
     * @var \PhpCsFixer\Differ\DifferInterface|null
     */
    private $differ;
    /**
     * @var \PhpCsFixer\Cache\Directory|null
     */
    private $directory;
    /**
    ...
    private $format;
    /**
     * @var \PhpCsFixer\Linter\Linter|null
     */
    private $linter;
    /**
    ...
    private $progress;
    /**
     * @var \PhpCsFixer\RuleSet\RuleSet|null
     */
    private $ruleSet;
    /**
     * @var bool|null
     ...
     * @var bool|null
     */
    private $isUnsupportedPhpVersionAllowed;
    /**
     * @var \PhpCsFixer\Config\RuleCustomisationPolicyInterface|null
     */
    private $ruleCustomisationPolicy;
    /**
     * @var \PhpCsFixer\FixerFactory|null
     */
    ...
    public function getCacheManager(): CacheManagerInterface
    {
        if (null === $this->cacheManager) {
            ...
            if (null === $cacheFile) {
                ...
                $this->cacheManager = new FileCacheManager(new FileHandler($cacheFile), new Signature(\PHP_VERSION, $this->toolInfo->getVersion(), $this->getConfig()->getIndent(), $this->getConfig()->getLineEnding(), $this->getRules(), $this->getRuleCustomisationPolicy()->getPolicyVersionForCache()), $this->isDryRun(), $this->getDirectory());
            }
        }
    ...
    public function getConfig(): ConfigInterface
    {
        if (null === $this->config) {
            foreach ($this->computeConfigFiles() as $configFile) {
                ...
                if (null !== $this->deprecatedNestedConfigDir && strncmp($configFile, $this->deprecatedNestedConfigDir, strlen($this->deprecatedNestedConfigDir)) === 0) {
                    // @TODO v4: when removing, remove also TODO with `MARKER-multi-paths-vs-only-cwd-config`
                    Future::triggerDeprecation(new InvalidConfigurationException("Configuration file `{$configFile}` is picked as file inside passed `path` CLI argument. This will be ignored in the future and only config file in `cwd` will be picked. Please use `config` CLI option instead if you want to keep current behaviour."));
                }
    ...
    public function getParallelConfig(): ParallelConfig
    {
        ...
        return \true !== $this->options['sequential'] && $config instanceof ParallelAwareConfigInterface ? $config->getParallelConfig() : ParallelConfigFactory::sequential();
    }
    ...
    {
        if (null === $this->differ) {
            $this->differ = \true === $this->options['diff'] ? new UnifiedDiffer() : new NullDiffer();
        }
        return $this->differ;
    }
        ...
        if (null === $this->progress) {
            if ('txt' === $this->resolveFormat()) {
                ...
                if (null === $progressType) {
                    $progressType = $this->getConfig()->getHideProgress() ? ProgressOutputType::NONE : ProgressOutputType::BAR;
                } elseif (!\in_array($progressType, ProgressOutputType::all(), \true)) {
                    throw new InvalidConfigurationException(\sprintf('The progress type "%s" is not defined, supported are %s.', $progressType, Utils::naturalLanguageJoin(ProgressOutputType::all())));
                }
    ...
    public function getUnsupportedPhpVersionAllowed(): bool
    {
        if (null === $this->isUnsupportedPhpVersionAllowed) {
            if (null === $this->options['allow-unsupported-php-version']) {
                $config = $this->getConfig();
                $this->isUnsupportedPhpVersionAllowed = $config instanceof UnsupportedPhpVersionAllowedConfigInterface ? $config->getUnsupportedPhpVersionAllowed() : \false;
            } else {
    ...
    public function getRuleCustomisationPolicy(): RuleCustomisationPolicyInterface
    {
        if (null === $this->ruleCustomisationPolicy) {
            ...
            $this->ruleCustomisationPolicy = $this->ruleCustomisationPolicy ?? new NullRuleCustomisationPolicy();
        }
    ...
    private function parseRules(): array
    {
        ...
        if (strncmp($rules, '{', strlen('{')) === 0) {
            try {
                return json_decode($rules, \true, 512, 0);
        ...
        $this->configRulesAreOverridden = \true;
        return $rules;
    }
    /**
    ...
    private function resolveFinder(): iterable
    {
        ...
        if (!\in_array($this->options['path-mode'], self::PATH_MODE_VALUES, \true)) {
        ...
        try {
            $nestedFinder = $currentFinder instanceof \IteratorAggregate ? $currentFinder->getIterator() : ($currentFinder instanceof \Traversable ? $currentFinder : new \ArrayIterator($currentFinder));
        } catch (\Exception $e) {
        }
        if ($isIntersectionPathMode) {
    ...
    {
        return $this->toolInfo->isInstalledAsPhar() || $this->toolInfo->isInstalledByComposer() || $this->toolInfo->isRunInsideDocker() || filter_var(getenv('PHP_CS_FIXER_ENFORCE_CACHE'), \FILTER_VALIDATE_BOOL);
    }
}
