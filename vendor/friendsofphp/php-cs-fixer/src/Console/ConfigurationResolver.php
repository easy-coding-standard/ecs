...
use ECSPrefix202511\Symfony\Component\Filesystem\Filesystem;
use ECSPrefix202511\Symfony\Component\Finder\Finder as SymfonyFinder;
/**
        ...
        if (\count($unknownFixers) > 0) {
            ...
            foreach ($unknownFixers as $unknownFixer) {
                if (isset($renamedRules[$unknownFixer])) {
                    ...
                    $hasOldRule = \true;
                    $message .= \sprintf('"%s" is renamed (did you mean "%s"?%s), ', $unknownFixer, $renamedRules[$unknownFixer]['new_name'], isset($renamedRules[$unknownFixer]['config']) ? ' (note: use configuration "' . Utils::toString($renamedRules[$unknownFixer]['config']) . '")' : '');
                } else {
