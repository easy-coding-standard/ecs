    ...
    public function detectOldVendor(): void
    {
        if ($this->toolInfo->isInstalledByComposer()) {
            ...
            if (ToolInfo::COMPOSER_LEGACY_PACKAGE_NAME === $details['name']) {
                $this->warnings[] = \sprintf('You are running PHP CS Fixer installed with old vendor `%s`. Please update to `%s`.', ToolInfo::COMPOSER_LEGACY_PACKAGE_NAME, ToolInfo::COMPOSER_PACKAGE_NAME);
            }
        }
    }
    ...
    public function detectHigherPhpVersion(): void
    {
        try {
            ...
            if (version_compare($currentPhpMajorMinor, $minPhpVersion, '>')) {
                $this->warnings[] = \sprintf('You are running PHP CS Fixer on PHP %1$s, but the minimum PHP version supported by your project in composer.json is PHP %2$s. Executing PHP CS Fixer on newer PHP versions may introduce syntax or features not yet available in PHP %2$s, which could cause issues under that version. It is recommended to run PHP CS Fixer on PHP %2$s, to fit your project specifics.', $currentPhpVersion, $minPhpVersion);
            }
        } catch (\Throwable $e) {
            $this->warnings[] = \sprintf('Unable to determine minimum PHP version supported by your project from composer.json: %s', $e->getMessage());
        }
    }
    /**
     * @return list<string>
