...
final class InitCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $io->note(['While we start, we must tell you that we put our diligence to NOT change the meaning of your codebase.', 'Yet, some of the rules are explicitly _risky_ to apply. A rule is _risky_ if it could change code behaviour, e.g. transforming `==` into `===` or removal of trailing whitespaces within multiline strings.', 'Such rules are improving your codebase even further, yet you shall always review changes proposed by _risky_ rules carefully.']);
        ...
        $generateSetsBehindAutoSet = static function () use($setAuto, $setAutoRisky, $isRiskyAllowed) : array {
            $sets = \array_merge(\array_keys($setAuto->getRulesCandidates()), $isRiskyAllowed ? \array_keys($setAutoRisky->getRulesCandidates()) : []);
            \natcasesort($sets);
            return $sets;
        };
        $setsBehindAutoSet = $generateSetsBehindAutoSet();
        $io->listing(\array_map(static function (RuleSetDefinitionInterface $item) : string {
            return \sprintf('<fg=blue>`%s`</> - %s', $item->getName(), $item->getDescription());
        }, \array_map(
            static function (string $name) use($setsByName) : RuleSetDefinitionInterface {
                return $setsByName[$name];
            },
            // @phpstan-ignore-line offsetAccess.notFound
            $setsBehindAutoSet
        )));
        $rules = [];
        $useAutoSet = 'yes' === $io->choice("Do you want to use <fg=blue>{$setAutoWithOptionalRiskySetNamesTextual}</> ruleset?", ['yes', 'no'], 'yes');
        if ($useAutoSet) {
        ...
        $generateExtraSets = static function () use($isRiskyAllowed) : array {
            ...
            $extraSets = [$setSymfony->getName(), $setPhpCsFixer->getName()];
            if ($isRiskyAllowed) {
        ...
        $extraSets = \array_merge(\false === $useAutoSet ? $setsBehindAutoSet : [], $generateExtraSets());
        \natcasesort($extraSets);
        $sets = $io->choice('Do you want to use any of other recommended ruleset? (multi-choice)', \array_combine($extraSets, \array_map(
            static function (string $item) use($setsByName) : string {
                return $setsByName[$item]->getDescription();
            },
            // @phpstan-ignore-line offsetAccess.notFound
            $extraSets
        )) + ['none' => 'none'], 'none', \true);
        ...
        $rules = \array_merge($rules, \array_unique(\array_filter($sets, static function ($item) {
            return 'none' !== $item;
        })));
        ...
        $content = \str_replace(['/*{{ IS_RISKY_ALLOWED }}*/', '/*{{ RULES }}*/'], [$isRiskyAllowed ? 'true' : 'false', "[\n" . \implode(",\n", \array_map(static function ($item) {
            return "        '{$item}' => true";
        }, $rules)) . "\n    ]"], $readResult);
