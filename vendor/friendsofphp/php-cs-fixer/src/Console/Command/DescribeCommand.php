...
use ECSPrefix202605\Symfony\Component\Console\Exception\RuntimeException;
use ECSPrefix202605\Symfony\Component\Console\Formatter\OutputFormatter;
use ECSPrefix202605\Symfony\Component\Console\Helper\TreeHelper;
use ECSPrefix202605\Symfony\Component\Console\Helper\TreeNode;
use ECSPrefix202605\Symfony\Component\Console\Input\ArrayInput;
...
use ECSPrefix202605\Symfony\Component\Console\Output\ConsoleOutputInterface;
...
final class DescribeCommand extends Command
{
    ...
    {
        $this->setDefinition([new InputArgument('name', InputArgument::OPTIONAL, 'Name of rule / set.', null, function () {
            return \array_merge($this->getSetNames(), \array_keys($this->getFixers()));
        }), new InputOption('config', '', InputOption::VALUE_REQUIRED, 'The path to a .php-cs-fixer.php file.'), new InputOption('expand', '', InputOption::VALUE_NONE, 'Shall nested sets be expanded into nested rules.'), new InputOption('format', '', InputOption::VALUE_REQUIRED, 'To output results in other formats (txt, tree).', 'txt', ['txt', 'tree'])]);
    }
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        if (null === $name) {
            ...
            $shallDescribeConfigInUse = 'yes' === $io->choice('Do you want to describe used configuration? (alias:`@`', ['yes', 'no'], 'yes');
            if ($shallDescribeConfigInUse) {
                $name = self::SET_ALIAS_TO_DESCRIBE_CONFIG;
            } else {
                $name = $io->choice('Please select rule / set to describe', \array_merge($this->getSetNames(), \array_keys($this->getFixers())));
            }
        }
        if ('tree' === $format) {
            if (\strncmp($name, '@', \strlen('@')) !== 0) {
                throw new \InvalidArgumentException('The "--format=tree" option is available only when describing a set (name starting with "@").');
            }
        ...
        if (\strncmp($name, '@', \strlen('@')) !== 0) {
            if (\true === $expand) {
                throw new \InvalidArgumentException('The "--expand" option is available only when describing a set (name starting with "@").');
            }
        }
        try {
            if (\strncmp($name, '@', \strlen('@')) === 0) {
    ...
    private function describeRule(OutputInterface $output, string $name) : void
    {
        ...
        foreach ($tags as $tag) {
            if (DocumentationTagType::DEPRECATED === $tag->type) {
                Future::triggerDeprecation(new \RuntimeException(\str_replace('`', '"', \sprintf('%s%s', \str_replace('This rule', \sprintf('Rule "%s"', $name), $tag->title), null !== $tag->description ? '. ' . $tag->description : ''))));
            ...
            foreach ($options as $option) {
                ...
                if (null === $allowed) {
                    ...
                    $allowed = \array_map(static function ($value) : string {
                        return $value instanceof AllowedValueSubset ? 'a subset of <comment>' . Utils::toString($value->getAllowedValues()) . '</comment>' : '<comment>' . Utils::toString($value) . '</comment>';
                    }, $allowed);
                }
                if (null !== $allowed) {
                ...
                if ($option->hasDefault()) {
                    $line .= \sprintf('defaults to <comment>%s</comment>', Utils::toString($option->getDefault()));
                } else {
        ...
        if (0 === \count($definition->getCodeSamples())) {
            $output->writeln(['Fixing examples are not available for this rule.', '']);
        } elseif (0 === \count($codeSamples)) {
            $output->writeln(['Fixing examples <error>cannot be</error> demonstrated on the current PHP version.', '']);
        } else {
    ...
    {
        if (!\in_array($name, [self::SET_ALIAS_TO_DESCRIBE_CONFIG, self::SET_ALIAS_TO_DESCRIBE_RULES_WITHOUT_SET], \true) && !\in_array($name, $this->getSetNames(), \true)) {
            throw new \PhpCsFixer\Console\Command\DescribeNameNotFoundException($name, 'set');
        }
        if (self::SET_ALIAS_TO_DESCRIBE_CONFIG === $name) {
            $aliasedRuleSetDefinition = $this->createRuleSetDefinition(null, [], ['getDescription' => null === $resolver->getConfigFile() ? 'Default rules, no config file.' : 'Rules defined in used config.', 'getName' => \sprintf('@ - %s', $resolver->getConfig()->getName()), 'getRules' => $resolver->getConfig()->getRules(), 'isRisky' => $resolver->getRiskyAllowed()]);
        } elseif (self::SET_ALIAS_TO_DESCRIBE_RULES_WITHOUT_SET === $name) {
            $rulesWithoutSet = \array_filter($this->getFixers(), static function (string $name) : bool {
                return [] === FixerDocumentGenerator::getSetsOfRule($name);
            }, \ARRAY_FILTER_USE_KEY);
            $aliasedRuleSetDefinition = $this->createRuleSetDefinition(null, [], ['getDescription' => 'Rules that are not part of any set.', 'getName' => '@- - rules without set', 'getRules' => \array_combine(\array_map(static function (FixerInterface $fixer) : string {
                return $fixer->getName();
            }, $rulesWithoutSet), \array_fill(0, \count($rulesWithoutSet), \true)), 'isRisky' => array_any($rulesWithoutSet, static function (FixerInterface $fixer) : bool {
        ...
        foreach ($tags as $tag) {
            if (DocumentationTagType::DEPRECATED === $tag->type) {
                Future::triggerDeprecation(new \RuntimeException(\str_replace('`', '"', \sprintf('%s%s', \str_replace('This rule set', \sprintf('Rule set "%s"', $name), $tag->title), null !== $tag->description ? '. ' . $tag->description : ''))));
            }
            $output->writeln(\sprintf('<error>%s</error>', $tag->title));
            $tagDescription = $tag->description;
            if (null !== $tagDescription) {
    ...
    {
        $tags = DocumentationTagGenerator::analyseRuleSet($ruleSetDefinition);
        $extra = [] !== $tags ? ' ' . \implode(' ', \array_map(static function (DocumentationTag $tag) : string {
    ...
    private function describeSetContentAsTxt(OutputInterface $output, RuleSetDefinitionInterface $ruleSetDefinition, array $ruleSetDefinitions, array $fixers) : void
    {
        ...
        foreach ($ruleSetDefinition->getRules() as $rule => $config) {
            if (\strncmp($rule, '@', \strlen('@')) === 0) {
                ...
                $help .= \sprintf(" * <info>%s</info>%s%s\n   | %s\n\n", $rule, [] !== $tags ? ' ' : '', \implode(' ', \array_map(static function (DocumentationTag $tag) : string {
                    return "<error>{$tag->type}</error>";
                }, $tags)), $this->replaceRstLinks($set->getDescription()));
                continue;
            }
            ...
            $help .= \sprintf(" * <info>%s</info>%s%s\n   | %s\n%s\n", $rule, [] !== $tags ? ' ' : '', \implode(' ', \array_map(static function (DocumentationTag $tag) : string {
                return "<error>{$tag->type}</error>";
            }, $tags)), $definition->getSummary(), \true !== $config ? \sprintf("   <comment>| Configuration: %s</comment>\n", Utils::toString($config)) : '');
        }
        $output->write($help);
    }
    /**
    ...
    private function replaceRstLinks(string $content) : string
    {
        return Preg::replaceCallback('/(`[^<]+<[^>]+>`_)/', static function (array $matches) {
            return Preg::replaceCallback('/`(.*)<(.*)>`_/', static function (array $matches) : string {
    ...
    private function createRuleSetDefinition(?RuleSetDefinitionInterface $ruleSetDefinition, array $adjustments, array $overrides) : RuleSetDefinitionInterface
    {
        ...
        {
            /**
             * @var \PhpCsFixer\RuleSet\RuleSetDefinitionInterface|null
             */
            private $original;
            /** @var list<'expand'> */
            private $adjustments;
             ...
             * @param array{getDescription?: string, getName?: string, getRules?: array<string, array<string, mixed>|bool>, isRisky?: bool} $overrides
             */
            public function __construct(?RuleSetDefinitionInterface $original, array $adjustments, array $overrides)
            {
            ...
            {
                return $this->overrides[__FUNCTION__] ?? (null !== $this->original ? $this->original->{__FUNCTION__}() : 'unknown description');
                // @phpstan-ignore method.dynamicName
            }
            public function getName() : string
            {
                $value = $this->overrides[__FUNCTION__] ?? (null !== $this->original ? $this->original->{__FUNCTION__}() : 'unknown name');
                // @phpstan-ignore method.dynamicName
                if (\in_array('expand', $this->adjustments, \true)) {
                    $value .= ' (expanded)';
                }
                return $value;
            }
            ...
            {
                $value = $this->overrides[__FUNCTION__] ?? (null !== $this->original ? $this->original->{__FUNCTION__}() : null);
                // @phpstan-ignore method.dynamicName
                if (null === $value) {
