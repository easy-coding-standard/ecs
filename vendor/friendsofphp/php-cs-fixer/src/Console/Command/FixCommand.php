...
use ECSPrefix202605\Symfony\Component\EventDispatcher\EventDispatcher;
use ECSPrefix202605\Symfony\Component\EventDispatcher\EventDispatcherInterface;
use ECSPrefix202605\Symfony\Component\Stopwatch\Stopwatch;
/**
...
class FixCommand extends Command
{
    ...
    private $toolInfo;
    /**
     * @var \PhpCsFixer\Console\Output\Progress\ProgressOutputFactory
     */
    private $progressOutputFactory;
...
EOF;
    ...
    protected function configure() : void
    {
        ...
        $this->setDefinition([new InputArgument('path', InputArgument::IS_ARRAY, 'The path(s) that rules will be run against (each path can be a file or directory).'), new InputOption('path-mode', '', InputOption::VALUE_REQUIRED, \PhpCsFixer\Console\Command\HelpCommand::getDescriptionWithAllowedValues('Specify path mode (%s).', ConfigurationResolver::PATH_MODE_VALUES), ConfigurationResolver::PATH_MODE_OVERRIDE, ConfigurationResolver::PATH_MODE_VALUES), new InputOption('allow-risky', '', InputOption::VALUE_REQUIRED, \PhpCsFixer\Console\Command\HelpCommand::getDescriptionWithAllowedValues('Are risky fixers allowed (%s).', ConfigurationResolver::BOOL_VALUES), null, ConfigurationResolver::BOOL_VALUES), new InputOption('config', '', InputOption::VALUE_REQUIRED, 'The path to a config file.'), new InputOption('dry-run', '', InputOption::VALUE_NONE, 'Only shows which files would have been modified.'), new InputOption('rules', '', InputOption::VALUE_REQUIRED, 'List of rules that should be run against configured paths.', null, static function () {
            $fixerFactory = new FixerFactory();
            $fixerFactory->registerBuiltInFixers();
            $fixers = \array_map(static function (FixerInterface $fixer) {
                return $fixer->getName();
            }, $fixerFactory->getFixers());
            return \array_merge(RuleSets::getSetDefinitionNames(), $fixers);
        }), new InputOption('using-cache', '', InputOption::VALUE_REQUIRED, \PhpCsFixer\Console\Command\HelpCommand::getDescriptionWithAllowedValues('Should cache be used (%s).', ConfigurationResolver::BOOL_VALUES), null, ConfigurationResolver::BOOL_VALUES), new InputOption('allow-unsupported-php-version', '', InputOption::VALUE_REQUIRED, \PhpCsFixer\Console\Command\HelpCommand::getDescriptionWithAllowedValues('Should the command refuse to run on unsupported PHP version (%s).', ConfigurationResolver::BOOL_VALUES), null, ConfigurationResolver::BOOL_VALUES), new InputOption('cache-file', '', InputOption::VALUE_REQUIRED, 'The path to the cache file.'), new InputOption('diff', '', InputOption::VALUE_NONE, 'Prints diff for each file.'), new InputOption('format', '', InputOption::VALUE_REQUIRED, \PhpCsFixer\Console\Command\HelpCommand::getDescriptionWithAllowedValues('To output results in other formats (%s).', $formats), null, $formats), new InputOption('stop-on-violation', '', InputOption::VALUE_NONE, 'Stop execution on first violation.'), new InputOption('show-progress', '', InputOption::VALUE_REQUIRED, \PhpCsFixer\Console\Command\HelpCommand::getDescriptionWithAllowedValues('Type of progress indicator (%s).', $progressOutputTypes), null, $progressOutputTypes), new InputOption('sequential', '', InputOption::VALUE_NONE, 'Enforce sequential analysis.')]);
    }
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        if (null !== $stdErr) {
            ...
            if (\version_compare(\PHP_VERSION, ConfigInterface::PHP_VERSION_SYNTAX_SUPPORTED . '.99', '>')) {
                $message = \sprintf('PHP CS Fixer currently supports PHP syntax only up to PHP %s, current PHP version: %s.', ConfigInterface::PHP_VERSION_SYNTAX_SUPPORTED, \PHP_VERSION);
                if (!$resolver->getUnsupportedPhpVersionAllowed()) {
                    ...
                    $stdErr->writeln(\sprintf($stdErr->isDecorated() ? '<bg=red;fg=white;>%s</>' : '%s', $message));
                    return 1;
                }
                ...
                $stdErr->writeln(\sprintf($stdErr->isDecorated() ? '<bg=yellow;fg=black;>%s</>' : '%s', $message));
            }
            ...
            if (null === $configFile && ConfigurationResolver::IGNORE_CONFIG_FILE !== $passedConfig && null === $passedRules) {
                if (\false === $input->isInteractive()) {
                    $stdErr->writeln(\sprintf($stdErr->isDecorated() ? '<bg=yellow;fg=black;>%s</>' : '%s', 'No config file found. Please create one using `php-cs-fixer init`.'));
                } else {
                    $io = new SymfonyStyle($input, $stdErr);
                    $shallCreateConfigFile = 'yes' === $io->choice('Do you want to create the config file?', ['yes', 'no'], 'yes');
                    if ($shallCreateConfigFile) {
                        $returnCode = $this->getApplication()->doRun(new ArrayInput(['command' => 'init']), $output);
        ...
        $finder = new \ArrayIterator(\array_filter(\iterator_to_array(\is_array($resolver->getFinder()) ? new \ArrayIterator($resolver->getFinder()) : $resolver->getFinder()), static function (\SplFileInfo $fileInfo) {
            return \false !== $fileInfo->getRealPath();
        }));
        if (null !== $stdErr) {
            if ($resolver->configFinderIsOverridden()) {
                $stdErr->writeln(\sprintf($stdErr->isDecorated() ? '<bg=yellow;fg=black;>%s</>' : '%s', 'Paths from configuration have been overridden by paths provided as command arguments.'));
            }
            if ($resolver->configRulesAreOverridden()) {
                $stdErr->writeln(\sprintf($stdErr->isDecorated() ? '<bg=yellow;fg=black;>%s</>' : '%s', 'Rules from configuration have been overridden by rules provided as command argument.'));
            }
        }
        ...
        $reportSummary = new ReportSummary(
            ...
            \memory_get_peak_usage(\true) + $runner->getWorkersMemoryUsage(),
    ...
    protected function isDryRun(InputInterface $input) : bool
    {
        ...
        // @phpstan-ignore symfonyConsole.optionNotFound (Because PHPStan doesn't recognise the method is overridden in the child class and this parameter is _not_ used in the child class.)
    }
}
