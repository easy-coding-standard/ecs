...
use ECSPrefix202511\React\Socket\TcpConnector;
...
final class WorkerCommand extends Command
{
    ...
    private $toolInfo;
    /**
     * @var \PhpCsFixer\Console\ConfigurationResolver
     */
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $tcpConnector->connect(\sprintf('127.0.0.1:%d', $port))->then(
            /** @codeCoverageIgnore */
            function (ConnectionInterface $connection) use($loop, $runner, $identifier) : void {
                ...
                $in = new Decoder($connection, \true, 512, \JSON_INVALID_UTF8_IGNORE);
                ...
                $handleError = static function (\Throwable $error) use($out) : void {
                    $out->write(['action' => ParallelAction::WORKER_ERROR_REPORT, 'class' => \get_class($error), 'message' => $error->getMessage(), 'file' => $error->getFile(), 'line' => $error->getLine(), 'code' => $error->getCode(), 'trace' => $error->getTraceAsString()]);
                };
                ...
                $in->on('data', function (array $json) use($loop, $runner, $out) : void {
                    ...
                    foreach ($files as $path) {
                        ...
                        $out->write(['action' => ParallelAction::WORKER_RESULT, 'file' => $path, 'fileHash' => $this->events[0]->getFileHash(), 'status' => $this->events[0]->getStatus(), 'fixInfo' => \array_pop($analysisResult), 'errors' => $this->errorsManager->forPath($path)]);
                    }
    ...
    private function createRunner(InputInterface $input) : Runner
    {
        ...
        $this->configurationResolver = new ConfigurationResolver(
            ...
            [
                ...
                // IMPORTANT! WorkerCommand is called with file that already passed filtering, so here we can rely on PATH_MODE_OVERRIDE.
        ...
        return new Runner(
            null,
            // Paths are known when parallelisation server requests new chunk, not now
            ...
            // IMPORTANT! We pass null cache, as cache is read&write in main process and we do not need to do it again.
            $this->configurationResolver->getDirectory(),
            $this->configurationResolver->shouldStopOnViolation(),
            ParallelConfigFactory::sequential(),
            // IMPORTANT! Worker must run in sequential mode.
            null,
