...
use ECSPrefix202605\React\Socket\TcpConnector;
            ...
            function (ConnectionInterface $connection) use($loop, $runner, $identifier) : void {
                ...
                $in->on('data', function (array $json) use($loop, $runner, $out) : void {
                    ...
                    foreach ($files as $path) {
                        ...
                        $out->write(['action' => ParallelAction::WORKER_RESULT, 'errors' => $this->errorsManager->forPath($path), 'file' => $path, 'fileHash' => $this->events[0]->getFileHash(), 'fixInfo' => \array_pop($analysisResult), 'memoryUsage' => \memory_get_peak_usage(\true), 'status' => $this->events[0]->getStatus()]);
                    }
