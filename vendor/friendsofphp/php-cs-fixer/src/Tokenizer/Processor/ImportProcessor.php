...
final class ImportProcessor
{
    ...
    public function insertImports(Tokens $tokens, array $imports, int $atIndex): void
    {
        ...
        if (!$tokens[$atIndex]->isWhitespace() || strpos($tokens[$atIndex]->getContent(), "\n") === \false) {
        ...
        foreach ($imports as $type => $typeImports) {
            ...
            foreach ($typeImports as $name) {
                $items = array_merge($items, [new Token([\T_WHITESPACE, $lineEnding]), new Token([\T_USE, 'use']), new Token([\T_WHITESPACE, ' '])]);
                if ('const' === $type) {
