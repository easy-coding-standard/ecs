...
final class TokensAnalyzer
{
    ...
    /**
     * @readonly
     * @var \PhpCsFixer\Tokenizer\Analyzer\GotoLabelAnalyzer
     */
    ...
    public function getClassyModifiers(int $index): array
    {
        ...
        while (\true) {
            ...
            } elseif ($this->tokens[$index]->isGivenKind(\PhpCsFixer\Tokenizer\FCT::T_READONLY)) {
    ...
    public function isAnonymousClass(int $index): bool
    {
        ...
        $index = $this->tokens->getPrevMeaningfulToken($index);
        if ($this->tokens[$index]->isGivenKind(\PhpCsFixer\Tokenizer\FCT::T_READONLY)) {
            $index = $this->tokens->getPrevMeaningfulToken($index);
        }
    ...
    public function isConstantInvocation(int $index): bool
    {
        ...
        if ($this->tokens[$nextIndex]->equalsAny(['(', '{']) || $this->tokens[$nextIndex]->isGivenKind([\T_DOUBLE_COLON, \T_ELLIPSIS, \T_NS_SEPARATOR, \PhpCsFixer\Tokenizer\CT::T_RETURN_REF, \PhpCsFixer\Tokenizer\CT::T_TYPE_ALTERNATION, \PhpCsFixer\Tokenizer\CT::T_TYPE_INTERSECTION, \T_VARIABLE])) {
            return \false;
        }
        ...
        $prevIndex = $this->tokens->getPrevMeaningfulToken($index);
        if ($this->tokens[$prevIndex]->isGivenKind(\PhpCsFixer\Tokenizer\Token::getClassyTokenKinds())) {
            return \false;
        }
        if ($this->tokens[$prevIndex]->isGivenKind([\T_AS, \T_CONST, \T_DOUBLE_COLON, \T_FUNCTION, \T_GOTO, \PhpCsFixer\Tokenizer\CT::T_GROUP_IMPORT_BRACE_OPEN, \PhpCsFixer\Tokenizer\CT::T_TYPE_COLON, \PhpCsFixer\Tokenizer\CT::T_TYPE_ALTERNATION, \PhpCsFixer\Tokenizer\CT::T_TYPE_INTERSECTION]) || $this->tokens[$prevIndex]->isObjectOperator()) {
            return \false;
        }
        if ($this->tokens[$prevIndex]->isGivenKind(\T_CASE) && $this->tokens->isAllTokenKindsFound([\PhpCsFixer\Tokenizer\FCT::T_ENUM])) {
    ...
    public function isUnaryPredecessorOperator(int $index): bool
    {
        ...
        if (!$token->equalsAny(['+', '-', '&', [\PhpCsFixer\Tokenizer\CT::T_RETURN_REF]])) {
            return \false;
        }
        $prevToken = $tokens[$tokens->getPrevMeaningfulToken($index)];
        ...
        $prevToken = $tokens[$tokens->getPrevTokenOfKind($index, [';', '{', '}', [\T_DOUBLE_ARROW], [\T_FN], [\T_FUNCTION], [\T_OPEN_TAG], [\T_OPEN_TAG_WITH_ECHO]])];
    ...
    public function isSuperGlobal(int $index): bool
    {
        ...
        return \in_array(strtoupper($token->getContent()), ['$_COOKIE', '$_ENV', '$_FILES', '$_GET', '$_POST', '$_REQUEST', '$_SERVER', '$_SESSION', '$GLOBALS'], \true);
    }
    /**
    ...
    private function findClassyElements(int $classIndex, int $index): array
    {
        ...
        for ($count = \count($this->tokens); $index < $count; ++$index) {
            ...
            if ($token->isGivenKind(\PhpCsFixer\Tokenizer\CT::T_PROPERTY_HOOK_BRACE_OPEN)) {
                $index = $this->tokens->getNextTokenOfKind($index, [[\PhpCsFixer\Tokenizer\CT::T_PROPERTY_HOOK_BRACE_CLOSE]]);
                continue;
            }
            if ($token->isGivenKind(\PhpCsFixer\Tokenizer\FCT::T_ATTRIBUTE)) {
                $index = $this->tokens->findBlockEnd(\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_ATTRIBUTE, $index);
                continue;
            }
                ...
                if ('__construct' === $this->tokens[$functionNameIndex]->getContent()) {
                    ...
                    foreach ($this->tokens->findGivenKind([\PhpCsFixer\Tokenizer\CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PUBLIC, \PhpCsFixer\Tokenizer\CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PROTECTED, \PhpCsFixer\Tokenizer\CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PRIVATE, \PhpCsFixer\Tokenizer\FCT::T_READONLY, \T_FINAL], $openParenthesis, $closeParenthesis) as $kindElements) {
                        foreach (array_keys($kindElements) as $promotedPropertyModifierIndex) {
                            ...
                            $elements[$promotedPropertyVariableIndex] = ['classIndex' => $classIndex, 'token' => $this->tokens[$promotedPropertyVariableIndex], 'type' => 'promoted_property'];
                        }
                    }
                }
