...
class Tokens extends \SplFixedArray
{
    ...
    public static function detectBlockType(\PhpCsFixer\Tokenizer\Token $token): ?array
    {
        ...
        if (null === $blockEdgeKinds) {
            ...
            foreach (self::getBlockEdgeDefinitions() as $type => $definition) {
                $blockEdgeKinds[\is_string($definition['start']) ? $definition['start'] : $definition['start'][0]] = ['type' => $type, 'isStart' => \true];
    ...
    public static function fromArray($array, $saveIndices = null): self
    {
        ...
        if (\false !== $saveIndices && !$arrayIsListFunction($array)) {
            Future::triggerDeprecation(new \InvalidArgumentException(\sprintf('Parameter "array" should be a list. This will be enforced in version %d.0.', Application::getMajorVersion() + 1)));
        ...
        $tokens->clearChanged();
        $tokens->generateCode();
        // ensure code hash is calculated, so it's registered in cache
        return $tokens;
    }
    /**
    ...
    public function offsetUnset($index): void
    {
        if (\count($this) - 1 !== $index) {
            Future::triggerDeprecation(new \InvalidArgumentException(\sprintf('Tokens should be a list - only the last index can be unset. This will be enforced in version %d.0.', Application::getMajorVersion() + 1)));
        }
        if (isset($this[$index])) {
    ...
    public function offsetSet($index, $newval): void
    {
        if (0 > $index || \count($this) <= $index) {
            Future::triggerDeprecation(new \InvalidArgumentException(\sprintf('Tokens should be a list - index must be within the existing range. This will be enforced in version %d.0.', Application::getMajorVersion() + 1)));
        }
        if (!$newval instanceof \PhpCsFixer\Tokenizer\Token) {
            Future::triggerDeprecation(new \InvalidArgumentException(\sprintf('Tokens should be a list of Token instances - newval must be a Token. This will be enforced in version %d.0.', Application::getMajorVersion() + 1)));
        }
        if (isset($this[$index])) {
            if (isset($this->blockStartCache[$index])) {
                unset($this->blockEndCache[$this->blockStartCache[$index]], $this->blockStartCache[$index]);
    ...
    public function generateCode(): string
    {
        ...
        if (null === $this->codeHash) {
            $this->changeCodeHash(self::calculateHash($code));
            // ensure code hash is calculated, so it's registered in cache
        }
        return $code;
    }
    ...
    public function getCollectionHash(): string
    {
        if (null === $this->collectionHash) {
            $this->collectionHash = self::calculateHash($this->getCodeHash() . '#' . \count($this) . '#' . implode('', array_map(static function (?\PhpCsFixer\Tokenizer\Token $token): ?int {
                return null !== $token ? $token->getId() : null;
            }, $this->toArray())));
        }
    ...
    {
        $tokens = array_values(array_filter($tokens, function ($token): bool {
    ...
    public function setCode(string $code): void
    {
        ...
        $this->updateSizeByIncreasingToNewSize(\count($tokens));
        // pre-allocate collection size
    ...
    public function isMonolithicPhp(): bool
    {
        ...
        return 0 === $this->countTokenKind(\T_INLINE_HTML) || 1 === $this->countTokenKind(\T_INLINE_HTML) && Preg::match('/^#!.+$/', $this[0]->getContent());
    }
    /**
     * @param int $start start index
     * @param int $end   end index
     ...
     */
    private function registerFoundToken(\PhpCsFixer\Tokenizer\Token $token): void
    {
        // inlined extractTokenKind() call on the hot path
        $tokenKind = $token->isArray() ? $token->getId() : $token->getContent();
        $this->foundTokenKinds[$tokenKind] = $this->foundTokenKinds[$tokenKind] ?? 0;
        ++$this->foundTokenKinds[$tokenKind];
    }
    /**
     * Unregister token as not found.
     */
    private function unregisterFoundToken(\PhpCsFixer\Tokenizer\Token $token): void
    {
        // inlined extractTokenKind() call on the hot path
        $tokenKind = $token->isArray() ? $token->getId() : $token->getContent();
