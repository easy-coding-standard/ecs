...
final class ArgumentsAnalyzer
{
    ...
    public function getArgumentInfo(Tokens $tokens, int $argumentStart, int $argumentEnd): ArgumentAnalysis
    {
        ...
        for ($index = $argumentStart; $index <= $argumentEnd; ++$index) {
            ...
            if ($token->isComment() || $token->isWhitespace() || $token->isGivenKind(self::ARGUMENT_INFO_SKIP_TYPES) || $token->equals('&')) {
                continue;
            }
        ...
        return new ArgumentAnalysis($info['name'], $info['name_index'], $info['default'], null !== $info['type'] ? new TypeAnalysis($info['type'], $info['type_index_start'], $info['type_index_end']) : null);
    }
}
