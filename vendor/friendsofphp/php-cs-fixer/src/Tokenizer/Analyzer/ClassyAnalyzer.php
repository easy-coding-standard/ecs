...
final class ClassyAnalyzer
{
    public function isClassyInvocation(Tokens $tokens, int $index): bool
    {
        ...
        if ((new \PhpCsFixer\Tokenizer\Analyzer\Analysis\TypeAnalysis($token->getContent()))->isReservedType()) {
            return \false;
        }
        ...
        if (\PHP_VERSION_ID >= 80000 && $nextToken->equals(')') && $prevToken->equals('(') && $tokens[$tokens->getPrevMeaningfulToken($prev)]->isGivenKind(\T_CATCH)) {
            return \true;
        }
