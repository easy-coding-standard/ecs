...
final class DataProviderAnalyzer
{
    ...
    {
        $fullyQualifiedNameAnalyzer = new \PhpCsFixer\Tokenizer\Analyzer\FullyQualifiedNameAnalyzer($tokens);
        ...
        foreach ($methods as $methodIndex) {
            ...
            if (null !== $attributeIndex) {
                foreach (\PhpCsFixer\Tokenizer\Analyzer\AttributeAnalyzer::collect($tokens, $attributeIndex) as $attributeAnalysis) {
            ...
            if (null !== $docCommentIndex) {
                Preg::matchAll('/@dataProvider\h+((' . self::REGEX_CLASS . '::)?' . TypeExpression::REGEX_IDENTIFIER . ')/', $tokens[$docCommentIndex]->getContent(), $matches, \PREG_OFFSET_CAPTURE);
        ...
        foreach ($dataProviders as $dataProviderName => $dataProviderUsages) {
            ...
            $dataProviderAnalyses[$methods[$lowercaseDataProviderName]] = new DataProviderAnalysis($tokens[$methods[$lowercaseDataProviderName]]->getContent(), $methods[$lowercaseDataProviderName], $dataProviderUsages);
        }
