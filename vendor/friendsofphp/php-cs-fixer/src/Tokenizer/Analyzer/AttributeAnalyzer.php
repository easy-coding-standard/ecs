...
final class AttributeAnalyzer
{
    ...
    {
        if (!$tokens[$index]->isGivenKind(\T_STRING) || !$tokens->isAnyTokenKindsFound([FCT::T_ATTRIBUTE])) {
            return \false;
        }
    ...
    public static function collectOne(Tokens $tokens, int $index): AttributeAnalysis
    {
        ...
        return new AttributeAnalysis($startIndex, $endIndex - 1, $index, $closingIndex, self::collectAttributes($tokens, $index, $closingIndex));
    }
    public static function determineAttributeFullyQualifiedName(Tokens $tokens, string $name, int $index): string
    {
        ...
        [$namespaceAnalysis, $namespaceUseAnalyses] = \PhpCsFixer\Tokenizer\Analyzer\NamespacesAnalyzer::collectNamespaceAnalysis($tokens, $index);
