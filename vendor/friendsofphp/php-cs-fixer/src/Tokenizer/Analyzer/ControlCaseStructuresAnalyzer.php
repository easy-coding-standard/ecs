...
{
    private const SUPPORTED_TYPES_WITH_CASE_OR_DEFAULT = [\T_SWITCH, FCT::T_MATCH, FCT::T_ENUM];
    /**
    ...
    public static function findControlStructures(Tokens $tokens, array $types): iterable
    {
        ...
        foreach ($types as $type) {
            if (!\in_array($type, self::SUPPORTED_TYPES_WITH_CASE_OR_DEFAULT, \true)) {
        ...
        foreach ($tokens as $index => $token) {
            if ($token->isGivenKind(self::SUPPORTED_TYPES_WITH_CASE_OR_DEFAULT)) {
                ++$depth;
                $stack[$depth] = ['kind' => $token->getId(), 'index' => $index, 'brace_count' => 0, 'cases' => [], 'default' => null, 'alternative_syntax' => \false];
    ...
    {
        $default = null === $analysis['default'] ? null : new DefaultAnalysis($analysis['default']['index'], $analysis['default']['open']);
        $cases = [];
        ...
        if (\T_SWITCH === $analysis['kind']) {
            return new SwitchAnalysis($analysis['index'], $analysis['open'], $analysis['end'], $cases, $default);
        }
        if (FCT::T_ENUM === $analysis['kind']) {
            return new EnumAnalysis($analysis['index'], $analysis['open'], $analysis['end'], $cases);
        }
        if (FCT::T_MATCH === $analysis['kind']) {
            return new MatchAnalysis($analysis['index'], $analysis['open'], $analysis['end'], $default);
        }
