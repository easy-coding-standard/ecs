...
final class FixerAnnotationAnalyzer
{
    ...
    public function find(Tokens $tokens): array
    {
        ...
        for (0; $index < $count; ++$index) {
            $token = $tokens[$index];
            if ($token->isGivenKind([\T_OPEN_TAG, \T_OPEN_TAG_WITH_ECHO, \T_WHITESPACE]) || $token->equals(';')) {
                continue;
            }
        ...
        for ($indexBackwards = $count - 1; $indexBackwards > $index; --$indexBackwards) {
            ...
            if ($token->isGivenKind([\T_CLOSE_TAG, \T_WHITESPACE]) || $token->equals(';')) {
                continue;
            }
            if ($token->isComment()) {
                $comments[] = $token->getContent();
        ...
        foreach ($matches[1] as $match) {
            ...
            $annotations[$matchParts[0]] = $annotations[$matchParts[0]] ?? [];
            array_push($annotations[$matchParts[0]], ...explode(',', $matchParts[1]));
        }
        foreach ($annotations as $annotation => $vals) {
            $duplicates = array_keys(array_filter(array_count_values($vals), static function (int $c): bool {
                return $c > 1;
            }));
