...
final class NamespacesAnalyzer
{
    ...
    public function getDeclarations(Tokens $tokens): array
    {
        ...
        if (0 === \count($namespaces) && $tokens->isTokenKindFound(\T_OPEN_TAG)) {
            $namespaces[] = new NamespaceAnalysis('', '', $openTagIndex = $tokens[0]->isGivenKind(\T_INLINE_HTML) ? 1 : 0, $openTagIndex, $openTagIndex, \count($tokens) - 1);
        }
        return $namespaces;
    }
    ...
    public static function collectNamespaceAnalysis(Tokens $tokens, int $startIndex): array
    {
        ...
        $namespaceUseAnalyses = (new \PhpCsFixer\Tokenizer\Analyzer\NamespaceUsesAnalyzer())->getDeclarationsInNamespace($tokens, $namespaceAnalysis);
        $uses = [];
