...
final class Token
{
    ...
    {
        return [\T_ARRAY_CAST, \T_BOOL_CAST, \T_DOUBLE_CAST, \T_INT_CAST, \T_OBJECT_CAST, \T_STRING_CAST, \T_UNSET_CAST, \PhpCsFixer\Tokenizer\FCT::T_VOID_CAST];
    }
    /**
    ...
    {
        return [\T_CLASS, \T_TRAIT, \T_INTERFACE, \PhpCsFixer\Tokenizer\FCT::T_ENUM];
    }
    /**
    ...
    {
        return [\T_OBJECT_OPERATOR, \PhpCsFixer\Tokenizer\FCT::T_NULLSAFE_OBJECT_OPERATOR];
    }
    /**
    ...
    {
        if ('&' === $other) {
            return '&' === $this->content && (null === $this->id || $this->isGivenKind([\PhpCsFixer\Tokenizer\FCT::T_AMPERSAND_FOLLOWED_BY_VAR_OR_VARARG, \PhpCsFixer\Tokenizer\FCT::T_AMPERSAND_NOT_FOLLOWED_BY_VAR_OR_VARARG]));
        }
        if (null === $this->id && '&' === $this->content) {
            return $other instanceof self && '&' === $other->content && (null === $other->id || $other->isGivenKind([\PhpCsFixer\Tokenizer\FCT::T_AMPERSAND_FOLLOWED_BY_VAR_OR_VARARG, \PhpCsFixer\Tokenizer\FCT::T_AMPERSAND_NOT_FOLLOWED_BY_VAR_OR_VARARG]));
        }
    ...
    {
        Future::triggerDeprecation(new \InvalidArgumentException(\sprintf('Method "%s" is deprecated and will be removed in the next major version.', __METHOD__)));
        if (\is_array($caseSensitive)) {
            return $caseSensitive[$key] ?? \true;
        }
    ...
    {
        return $this->isArray && \in_array(strtolower($this->content), ['true', 'false', 'null'], \true);
    }
    /**
    ...
    {
        return ['id' => $this->id, 'name' => $this->getName(), 'content' => $this->content, 'isArray' => $this->isArray, 'changed' => \false];
    }
    /**
    ...
    {
        try {
            return json_encode($this->toArray(), \JSON_PRETTY_PRINT | \JSON_NUMERIC_CHECK);
        } catch (\JsonException $e) {
            return json_encode(['errorDescription' => 'Cannot encode Tokens to JSON.', 'rawErrorMessage' => $e->getMessage()], \JSON_PRETTY_PRINT | \JSON_NUMERIC_CHECK);
        }
    }
    /**
