...
final class BraceTransformer extends AbstractTransformer
{
    ...
    private function transformIntoPropertyHookBraces(Tokens $tokens, int $index): void
    {
        if (\PHP_VERSION_ID < 80400) {
            return;
            // @TODO: drop condition when PHP 8.4+ is required or majority of the users are using 8.4+
        }
        $token = $tokens[$index];
        if (!$token->equals('{')) {
            return;
        ...
        if (!$tokens[$nextIndex]->equalsAny([[\T_STRING, 'get'], [\T_STRING, 'set']], \false)) {
            return;
        }
    ...
    private function transformIntoDynamicClassConstantFetchBraces(Tokens $tokens, int $index): void
    {
        if (\PHP_VERSION_ID < 80300) {
            return;
            // @TODO: drop condition when PHP 8.3+ is required or majority of the users are using 8.3+
        }
        $token = $tokens[$index];
        if (!$token->equals('{')) {
            return;
