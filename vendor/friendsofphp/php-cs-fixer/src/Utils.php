...
final class Utils
{
    ...
    public static function stableSort(array $elements, callable $getComparedValue, callable $compareValues): array
    {
        ...
        // @phpstan-ignore return.type (PHPStan cannot understand that the result will still be L template)
    }
    /**
    ...
    {
        return \is_array($value) ? self::arrayToString($value) : self::scalarToString($value);
    }
    /**
     * @param mixed $value
     */
    ...
    private static function arrayToString(array $value): string
    {
        ...
        $isHash = !$arrayIsListFunction($value);
        $str = '[';
        foreach ($value as $k => $v) {
            if ($isHash) {
                $str .= self::scalarToString($k) . ' => ';
            }
            $str .= \is_array($v) ? self::arrayToString($v) . ', ' : self::scalarToString($v) . ', ';
        }
