...
final class GroupImportFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    {
        return new FixerDefinition('There MUST be group use for the same namespaces.', [new CodeSample("<?php\nuse Foo\\Bar;\nuse Foo\\Baz;\n"), new CodeSample(<<<'PHP'
<?php

namespace ECSPrefix202605;

use ECSPrefix202605\A\Foo;
use function ECSPrefix202605\B\foo;
use ECSPrefix202605\A\Bar;
use function ECSPrefix202605\B\bar;
...
, ['group_types' => [self::GROUP_CLASSY]])]);
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('group_types', 'Defines the order of import types.'))->setAllowedTypes(['string[]'])->setAllowedValues([static function (array $types) use ($allowedTypes): bool {
            foreach ($types as $type) {
                if (!\in_array($type, $allowedTypes, \true)) {
                    throw new InvalidOptionsException(\sprintf('Invalid group type: %s, allowed types: %s.', $type, Utils::naturalLanguageJoin($allowedTypes)));
                }
            }
            return \true;
        }])->setDefault($allowedTypes)->getOption()]);
    }
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        $typeMap = [NamespaceUseAnalysis::TYPE_CLASS => self::GROUP_CLASSY, NamespaceUseAnalysis::TYPE_FUNCTION => self::GROUP_FUNCTIONS, NamespaceUseAnalysis::TYPE_CONSTANT => self::GROUP_CONSTANTS];
        ...
        foreach ($useWithSameNamespaces as $type => $uses) {
            if (!\in_array($typeMap[$type], $this->configuration['group_types'], \true)) {
                continue;
            }
    ...
    private function getSameNamespacesByType(Tokens $tokens): array
    {
        ...
        $allNamespaceAndType = array_map(function (NamespaceUseAnalysis $useDeclaration): string {
    ...
    private function addGroupUseStatements(array $statements, Tokens $tokens): void
    {
        ...
        foreach ($statements as $index => $useDeclaration) {
            if ($this->areDeclarationsDifferent($currentUseDeclaration, $useDeclaration)) {
                ...
                $insertIndex += $this->createNewGroup($tokens, $insertIndex, $useDeclaration, rtrim($this->getNamespaceNameWithSlash($currentUseDeclaration), '\\'));
            } else {
    ...
    private function getNamespaceNameWithSlash(NamespaceUseAnalysis $useDeclaration): string
    {
        ...
        return (string) substr($useDeclaration->getFullName(), 0, $position + 1);
    }
    /**
    ...
    {
        $newTokens = [new Token([\T_STRING, (string) substr($useDeclaration->getFullName(), (int) strripos($useDeclaration->getFullName(), '\\') + 1)]), new Token([\T_WHITESPACE, ' ']), new Token([\T_AS, 'as']), new Token([\T_WHITESPACE, ' '])];
        $tokens->insertAt($insertIndex, $newTokens);
