...
use function ECSPrefix202511\Aaa;
use const AA;

PHP
, ['case_sensitive' => \true]), new CodeSample(<<<'PHP'
<?php
use Acme\Bar;
use Bar1;
use Acme;
...
, ['sort_algorithm' => self::SORT_NONE, 'imports_order' => [self::IMPORT_TYPE_CONST, self::IMPORT_TYPE_CLASS, self::IMPORT_TYPE_FUNCTION]])]);
    ...
    protected function createConfigurationDefinition() : FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('sort_algorithm', 'Whether the statements should be sorted alphabetically or by length (*deprecated*), or not sorted.'))->setAllowedValues(self::SUPPORTED_SORT_ALGORITHMS)->setDefault(self::SORT_ALPHA)->setNormalizer(static function (Options $options, ?string $value) use($fixerName) : ?string {
            if (self::SORT_LENGTH === $value) {
                Future::triggerDeprecation(new InvalidFixerConfigurationException($fixerName, \sprintf('Option "sort_algorithm:%s" is deprecated and will be removed in version %d.0.', self::SORT_LENGTH, Application::getMajorVersion() + 1)));
            }
            return $value;
        })->getOption(), (new FixerOptionBuilder('imports_order', 'Defines the order of import types.'))->setAllowedTypes(['string[]', 'null'])->setAllowedValues([static function (?array $value) use($supportedSortTypes) : bool {
            if (null !== $value) {
    ...
    private function sortAlphabetically(array $first, array $second) : int
    {
        ...
        return \true === $this->configuration['case_sensitive'] ? $firstNamespace <=> $secondNamespace : \strcasecmp($firstNamespace, $secondNamespace);
    }
    /**
    ...
    private function sortByLength(array $first, array $second) : int
    {
        ...
        if ($firstNamespaceLength === $secondNamespaceLength) {
            $sortResult = \true === $this->configuration['case_sensitive'] ? $firstNamespace <=> $secondNamespace : \strcasecmp($firstNamespace, $secondNamespace);
        } else {
                    ...
                    if ($group && self::SORT_NONE !== $this->configuration['sort_algorithm']) {
                        ...
                        for ($k1 = $k + 1; $k1 < $namespaceTokensCount; ++$k1) {
                            ...
                            for ($k2 = $k1;; ++$k2) {
                                ...
                                if ('' === $firstIndent && $namespaceTokens[$k2]->isWhitespace() && \strpos($namespaceTokens[$k2]->getContent(), $lineEnding) !== \false) {
    ...
    private function setNewOrder(Tokens $tokens, array $usesOrder) : void
    {
        ...
        foreach (\array_reverse($usesOrder, \true) as $index => $use) {
            $code = \sprintf('<?php use %s%s;', self::IMPORT_TYPE_CLASS === $use['importType'] ? '' : ' ' . $use['importType'] . ' ', $use['namespace']);
            ...
            if (self::IMPORT_TYPE_CLASS !== $use['importType']) {
                ...
                if ($tokens[$prevIndex]->equals(',')) {
                    ...
                    // clear `<?php use const ` or `<?php use function `
                }
            }
