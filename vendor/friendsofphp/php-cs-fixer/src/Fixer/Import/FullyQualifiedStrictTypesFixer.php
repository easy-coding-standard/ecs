...
use ECSPrefix202511\Foo\Bar;
use ECSPrefix202511\Foo\Bar\Baz;
use ECSPrefix202511\Foo\OtherClass;
use ECSPrefix202511\Foo\SomeContract;
use ECSPrefix202511\Foo\SomeException;
/**
* @see \Foo\Bar\Baz
*/
class SomeClass extends \ECSPrefix202511\Foo\OtherClass implements \ECSPrefix202511\Foo\SomeContract
{
    /**
    * @var \Foo\Bar\Baz
    */
    ...
    */
    ...
    public function doX(\ECSPrefix202511\Foo\Bar $foo, \Exception $e) : \ECSPrefix202511\Foo\Bar\Baz
    {
        try {
        } catch (\ECSPrefix202511\Foo\SomeException $e) {
        }
    }
}
/**
* @see \Foo\Bar\Baz
*/
\class_alias('ECSPrefix202511\\SomeClass', 'SomeClass', \false);
...
class SomeClass
{
    public function doY(Foo\NotImported $u, \ECSPrefix202511\Foo\NotImported $v)
    {
    }
}
\class_alias('ECSPrefix202511\\SomeClass', 'SomeClass', \false);
...
use ECSPrefix202511\Foo\A;
try {
    foo();
} catch (\Exception|\ECSPrefix202511\Foo\A $e) {
}
namespace ECSPrefix202511\Foo\Bar;

class SomeClass implements \ECSPrefix202511\Foo\Bar\Baz
{
}
...
namespace ECSPrefix202511\Foo\Test;

class Foo extends \ECSPrefix202511\Other\BaseClass implements \ECSPrefix202511\Other\Interface1, \ECSPrefix202511\Other\Interface2
{
    /** @var \Other\PropertyPhpDoc */
    private $array;
    public function __construct(\ECSPrefix202511\Other\FunctionArgument $arg)
    {
    }
    public function foo() : \ECSPrefix202511\Other\FunctionReturnType
    {
        try {
            \ECSPrefix202511\Other\StaticFunctionCall::bar();
        } catch (\ECSPrefix202511\Other\CaughtThrowable $e) {
        }
    }
}
...
, ['import_symbols' => \true])]);
    ...
    private function setupUsesFromDiscoveredSymbols(array &$uses, string $namespaceName) : void
    {
        foreach ($this->discoveredSymbols as $kind => $discoveredSymbols) {
            ...
            foreach ($discoveredSymbols as $symbol) {
                while (\true) {
                    $shortEndNameLower = \strtolower(\strpos($symbol, '\\') !== \false ? (string) \substr($symbol, \strrpos($symbol, '\\') + 1) : $symbol);
            ...
            foreach ($discoveredFqcnByShortNameLower[$kind] ?? [] as $fqcn) {
                ...
                if (\strpos($shortenedName, '\\') !== \false) {
                    ...
                    $shortEndName = \strpos($fqcn, '\\') !== \false ? (string) \substr($fqcn, \strrpos($fqcn, '\\') + 1) : $fqcn;
                    \assert('' !== $shortEndName);
