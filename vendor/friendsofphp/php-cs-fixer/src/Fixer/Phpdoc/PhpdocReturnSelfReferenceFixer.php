...
final class PhpdocReturnSelfReferenceFixer extends AbstractFixer implements ConfigurableFixerInterface
{
     ...
     */
    private const TO_TYPES = ['$this', 'static', 'self'];
...
, ['replacements' => ['this' => 'self']])]);
    ...
    protected function createConfigurationDefinition() : FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('replacements', 'Mapping between replaced return types with new ones.'))->setAllowedTypes(['array<string, string>'])->setNormalizer(static function (Options $options, array $value) use($default) : array {
            $normalizedValue = [];
            foreach ($value as $from => $to) {
                ...
                if (!isset($default[$from])) {
                    throw new InvalidOptionsException(\sprintf('Unknown key "%s", expected any of %s.', \gettype($from) . '#' . $from, Utils::naturalLanguageJoin(\array_keys($default))));
                }
                if (!\in_array($to, self::TO_TYPES, \true)) {
                    throw new InvalidOptionsException(\sprintf('Unknown value "%s", expected any of %s.', \is_object($to) ? \get_class($to) : \gettype($to) . (\is_resource($to) ? '' : '#' . $to), Utils::naturalLanguageJoin(self::TO_TYPES)));
                }
