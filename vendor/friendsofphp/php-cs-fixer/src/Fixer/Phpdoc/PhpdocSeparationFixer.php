...
final class PhpdocSeparationFixer extends AbstractFixer implements ConfigurableFixerInterface
{
     ...
     */
    public const OPTION_GROUPS_DEFAULT = [['author', 'copyright', 'license'], ['category', 'package', 'subpackage'], ['property', 'property-read', 'property-write'], ['deprecated', 'link', 'see', 'since']];
    /**
...
EOF;
        return new FixerDefinition('Annotations in PHPDoc should be grouped together so that annotations of the same type immediately follow each other. Annotations of a different type are separated by a single blank line.', [new CodeSample($code), new CodeSample($code, ['groups' => [['deprecated', 'link', 'see', 'since'], ['author', 'copyright', 'license'], ['category', 'package', 'subpackage'], ['property', 'property-read', 'property-write'], ['param', 'return']]]), new CodeSample($code, ['groups' => [['author', 'throws', 'custom'], ['return', 'param']]]), new CodeSample(<<<'EOF'
<?php
...
EOF
, ['groups' => [['ORM\*'], ['Assert\*']]]), new CodeSample($code, ['skip_unlisted_annotations' => \true])]);
    }
    /**
     * {@inheritdoc}
     *
                ...
                foreach ($group as $member) {
                    if (isset($tags[$member])) {
                        if ($groupIndex === $tags[$member]) {
                            throw new InvalidOptionsException('The option "groups" value is invalid. ' . 'The "' . $member . '" tag is specified more than once.');
                        }
                        throw new InvalidOptionsException('The option "groups" value is invalid. ' . 'The "' . $member . '" tag belongs to more than one group.');
                    }
