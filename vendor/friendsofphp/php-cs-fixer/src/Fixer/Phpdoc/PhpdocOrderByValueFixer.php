...
, ['annotations' => ['author']])]);
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        for ($index = $tokens->count() - 1; $index > 0; --$index) {
            foreach ($this->configuration['annotations'] as $type => $typeLowerCase) {
                ...
                if (!$tokens[$index]->isGivenKind(\T_DOC_COMMENT) || !Preg::match($findPattern, $tokens[$index]->getContent())) {
                    continue;
                }
                ...
                foreach ($annotations as $annotation) {
                    ...
                    $comparableContent = Preg::replace($replacePattern, $replacement, strtolower(trim($rawContent)));
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('annotations', 'List of annotations to order, e.g. `["covers"]`.'))->setAllowedTypes(['string[]'])->setAllowedValues([new AllowedValueSubset($allowedValues)])->setNormalizer(static function (Options $options, array $value): array {
            $normalized = [];
