...
final class NoSuperfluousPhpdocTagsFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    /** @var _TypeInfo */
    private const NO_TYPE_INFO = ['types' => [], 'allows_null' => \true];
    private const SYMBOL_KINDS = [\T_CLASS, \T_INTERFACE, FCT::T_ENUM];
    private const MODIFIER_KINDS = [\T_PRIVATE, \T_PROTECTED, \T_PUBLIC, \T_ABSTRACT, \T_FINAL, \T_STATIC, FCT::T_READONLY, FCT::T_PRIVATE_SET, FCT::T_PROTECTED_SET, FCT::T_PUBLIC_SET];
...
, ['allow_unused_params' => \true])]);
    ...
    {
        $typeKinds = [CT::T_NULLABLE_TYPE, CT::T_ARRAY_TYPEHINT, CT::T_TYPE_ALTERNATION, CT::T_TYPE_INTERSECTION, \T_STRING, \T_NS_SEPARATOR];
    ...
    private function fixPropertyDocComment(string $content, Tokens $tokens, array $element, ?string $namespace, ?string $currentSymbol, array $shortNames) : string
    {
        if (\count($element['types']) > 0) {
            \reset($element['types']);
            $propertyTypeInfo = $this->parseTypeHint($tokens, \key($element['types']));
        } else {
    ...
    private function getArgumentsInfo(Tokens $tokens, int $start, int $end) : array
    {
        ...
        for ($index = $start; $index <= $end; ++$index) {
            ...
            if (!$info['allows_null']) {
                ...
                if ($tokens[$nextIndex]->equals('=') && $tokens[$tokens->getNextMeaningfulToken($nextIndex)]->equals([\T_STRING, 'null'], \false)) {
    ...
    private function getReturnTypeInfo(Tokens $tokens, int $closingParenthesisIndex) : array
    {
        ...
        return $tokens[$colonIndex]->isGivenKind(CT::T_TYPE_COLON) ? $this->parseTypeHint($tokens, $tokens->getNextMeaningfulToken($colonIndex)) : self::NO_TYPE_INFO;
    }
    /**
    ...
    private function parseTypeHint(Tokens $tokens, int $index) : array
    {
        ...
        return ['types' => $types, 'allows_null' => $allowsNull];
    }
    /**
     ...
     */
    private function annotationIsSuperfluous(Annotation $annotation, array $info, ?string $namespace, ?string $currentSymbol, array $symbolShortNames) : bool
    {
