...
PHP
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        foreach ($tokens as $index => $token) {
            ...
            if (null !== $end) {
                ...
                if (!$this->isCorrectlyFormatted($content) && (1 === $end || $doc->isMultiLine() && ':' !== substr(rtrim($doc->getLine(1)->getContent()), -1))) {
    ...
    {
        if (strpos(strtolower($content), strtolower('{@inheritdoc}')) !== \false) {
            return \true;
        }
