...
EOF;
        return new FixerDefinition('Annotations in PHPDoc should be ordered in defined sequence.', [new CodeSample($code), new CodeSample($code, ['order' => ['param', 'return', 'throws']]), new CodeSample($code, ['order' => ['param', 'throws', 'return']]), new CodeSample($code, ['order' => ['param', 'custom', 'throws', 'return']])]);
    }
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('order', 'Sequence in which annotations in PHPDoc should be ordered.'))->setAllowedTypes(['string[]'])->setAllowedValues([static function (array $order) : bool {
            if (\count($order) < 2) {
            ...
            if (\count($order) !== \count($unique)) {
                $duplicates = \array_keys(\array_filter(\array_count_values($order), static function (int $count) : bool {
                    return $count > 1;
                }));
                throw new InvalidOptionsException(\sprintf('The option "order" value is invalid. Tag%s %s %s duplicated.', \count($duplicates) > 1 ? 's' : '', Utils::naturalLanguageJoin($duplicates), \count($duplicates) > 1 ? 'are' : 'is'));
            }
            return \true;
        }])->setDefault(Future::getV4OrV3(['param', 'return', 'throws'], ['param', 'throws', 'return']))->getOption()]);
    }
    protected function configurePostNormalisation() : void
    {
        ...
        foreach ($this->configuration['order'] as $type) {
            ...
            if (!\in_array('phpstan-' . $type, $this->configuration['order'], \true)) {
                $this->configurationOrder[] = 'phpstan-' . $type;
            }
            if (!\in_array('psalm-' . $type, $this->configuration['order'], \true)) {
