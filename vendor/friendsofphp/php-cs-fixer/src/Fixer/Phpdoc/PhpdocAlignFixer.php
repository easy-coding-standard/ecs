...
EOF;
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        ...
        $tags = new FixerOptionBuilder('tags', 'The tags that should be aligned. Allowed values are tags with name (`\'' . implode('\', \'', self::TAGS_WITH_NAME) . '\'`), tags with method signature (`\'' . implode('\', \'', self::TAGS_WITH_METHOD_SIGNATURE) . '\'`) and any custom tag with description (e.g. `@tag <desc>`).');
        $tags->setAllowedTypes(['string[]'])->setDefault(self::DEFAULT_TAGS);
        ...
        $spacing->setAllowedTypes(['int', 'array<string, int>'])->setAllowedValues([$allowPositiveIntegers])->setDefault(self::DEFAULT_SPACING);
        ...
        for ($i = 0, $l = \count($docBlock->getLines()); $i < $l; ++$i) {
            ...
            foreach ($items as $j => $item) {
                if (null === $item['tag']) {
                    ...
                    if (\in_array($itemOpeningLine['tag'], self::TAGS_WITH_NAME, \true) || \in_array($itemOpeningLine['tag'], self::TAGS_WITH_METHOD_SIGNATURE, \true)) {
                    ...
                    $line = $item['indent'] . ' * ' . ('' !== $itemOpeningLine['hint'] ? ' ' : '') . $this->getIndent($tagMax + $hintMax + $extraIndent, $this->getLeftAlignedDescriptionIndent($items, $j)) . $item['desc'];
                    $docBlock->getLine($current + $j)->setContent($line . $lineEnding);
                    continue;
                }
                $currTag = $item['tag'];
                ...
                if ($hasStatic) {
                    $line .= $this->getIndent($tagMax - \strlen($item['tag']) + $spacingForTag, '' !== $item['static'] ? $spacingForTag : 0) . ('' !== $item['static'] ? $item['static'] : $this->getIndent(6, 0));
                    $hintVerticalAlignIndent = $spacingForTag;
                } else {
                    $hintVerticalAlignIndent = $tagMax - \strlen($item['tag']) + $spacingForTag;
                }
                $line .= $this->getIndent($hintVerticalAlignIndent, '' !== $item['hint'] ? $spacingForTag : 0) . $item['hint'];
                if ('' !== $item['var']) {
                    $line .= $this->getIndent((0 !== $hintMax ? $hintMax : -1) - mb_strlen($item['hint']) + $spacingForTag, $spacingForTag) . $item['var'] . ('' !== $item['desc'] ? $this->getIndent($varMax - \strlen($item['var']) + $spacingForTag, $spacingForTag) . $item['desc'] : '');
                } elseif ('' !== $item['desc']) {
    ...
    {
        return \is_int($this->spacing) ? $this->spacing : $this->spacing[$tag ?? ''] ?? $this->spacing[self::DEFAULT_SPACING_KEY] ?? self::DEFAULT_SPACING;
    }
    /**
    ...
    private function getLeftAlignedDescriptionIndent(array $items, int $index): int
    {
        ...
        return $this->getSentenceIndent($item['static'], $spacingForTag) + $this->getSentenceIndent($item['tag'], $spacingForTag) + $this->getSentenceIndent($item['hint'], $spacingForTag) + $this->getSentenceIndent($item['var'], $spacingForTag);
    }
    /**
