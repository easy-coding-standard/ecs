...
final class NoEmptyPhpdocFixer extends AbstractFixer
{
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        foreach ($tokens as $index => $token) {
            ...
            if ($tokens[$index - 1]->isGivenKind([\T_OPEN_TAG, \T_WHITESPACE]) && substr_count($tokens[$index - 1]->getContent(), "\n") > 0 && $tokens[$index + 1]->isGivenKind(\T_WHITESPACE) && Preg::match('/^\R/', $tokens[$index + 1]->getContent())) {
                $tokens[$index - 1] = new Token([$tokens[$index - 1]->getId(), Preg::replace('/\h*$/', '', $tokens[$index - 1]->getContent())]);
