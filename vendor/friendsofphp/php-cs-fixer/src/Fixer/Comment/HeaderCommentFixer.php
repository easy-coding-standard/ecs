...
final class HeaderCommentFixer extends AbstractFixer implements ConfigurableFixerInterface, WhitespacesAwareFixerInterface
{
    ...
    {
        return new FixerDefinition('Add, replace or remove header comment.', [new CodeSample(<<<'PHP'
<?php

declare (strict_types=1);
namespace ECSPrefix202511\A\B;

echo 1;

PHP
, ['header' => 'Made with love.']), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['header' => 'Made with love.', 'comment_type' => self::HEADER_PHPDOC, 'location' => 'after_open', 'separate' => 'bottom']), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['header' => 'Made with love.', 'comment_type' => self::HEADER_COMMENT, 'location' => 'after_declare_strict']), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['header' => 'Made with love.', 'validator' => '/Made with love(?P<EXTRA>.*)??/s', 'comment_type' => self::HEADER_COMMENT, 'location' => 'after_declare_strict']), new CodeSample(<<<'PHP'
<?php
...
, ['header' => ''])]);
    ...
    protected function createConfigurationDefinition() : FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('header', 'Proper header content.'))->setAllowedTypes(['string'])->setNormalizer(static function (Options $options, string $value) use($fixerName) : string {
            if ('' === \trim($value)) {
                return '';
            }
            ...
            return $value;
        })->getOption(), (new FixerOptionBuilder('validator', 'RegEx validator for header content.'))->setAllowedTypes(['string', 'null'])->setNormalizer(static function (Options $options, ?string $value) use($fixerName) : ?string {
            if (null !== $value) {
                try {
                    Preg::match($value, '');
    ...
    private function doesTokenFulfillValidator(Token $token) : bool
    {
        ...
        $lines = \implode("\n", \array_map(static function (string $line) : string {
            return ' *' === $line ? '' : (\strncmp($line, ' * ', \strlen(' * ')) === 0 ? (string) \substr($line, 3) : $line);
    ...
    private function findHeaderCommentCurrentIndex(Tokens $tokens, string $headerAsComment, int $headerNewIndex) : ?int
    {
        ...
        if ($headerAsComment === $tokens[$index]->getContent() || null !== $this->configuration['validator'] && $this->doesTokenFulfillValidator($tokens[$index])) {
            return $index;
        }
        return null;
    }
