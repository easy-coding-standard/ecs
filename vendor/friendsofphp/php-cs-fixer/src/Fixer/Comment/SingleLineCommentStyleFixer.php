...
, ['comment_types' => ['asterisk']]), new CodeSample("<?php # comment\n", ['comment_types' => ['hash']])]);
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        foreach ($tokens as $index => $token) {
            ...
            if (!$this->asteriskEnabled || \strpos($commentContent, '?>') !== \false || \strncmp($content, '/*', \strlen('/*')) !== 0 || Preg::match('/[^\\s\\*].*\\R.*[^\\s\\*]/s', $commentContent)) {
                continue;
            }
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('comment_types', 'List of comment types to fix.'))->setAllowedTypes(['string[]'])->setAllowedValues([new AllowedValueSubset(['asterisk', 'hash'])])->setDefault(['asterisk', 'hash'])->getOption()]);
    }
}
