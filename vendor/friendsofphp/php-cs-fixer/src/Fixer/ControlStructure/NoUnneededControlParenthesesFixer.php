...
final class NoUnneededControlParenthesesFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    private const CONFIG_OPTIONS = ['break', 'clone', 'continue', 'echo_print', 'negative_instanceof', 'others', 'return', 'switch_case', 'yield', 'yield_from'];
    private const TOKEN_TYPE_CONFIG_MAP = [\T_BREAK => 'break', \T_CASE => 'switch_case', \T_CONTINUE => 'continue', \T_ECHO => 'echo_print', \T_PRINT => 'echo_print', \T_RETURN => 'return', \T_YIELD => 'yield', \T_YIELD_FROM => 'yield_from'];
    // handled by the `include` rule
    private const TOKEN_TYPE_NO_CONFIG = [\T_REQUIRE, \T_REQUIRE_ONCE, \T_INCLUDE, \T_INCLUDE_ONCE];
...
, ['statements' => ['break', 'continue']])]);
        ...
        foreach ($tokens as $openIndex => $token) {
            ...
            if (null !== $instanceOfIndex) {
                if ($this->isWrappedInstanceOf($tokens, $instanceOfIndex, $beforeOpenIndex, $openIndex, $closeIndex, $afterCloseIndex)) {
                    $this->removeUselessParenthesisPair($tokens, $beforeOpenIndex, $afterCloseIndex, $openIndex, $closeIndex, $tokens[$beforeOpenIndex]->equals('!') ? 'negative_instanceof' : 'others');
                }
                continue;
            }
    ...
    {
        $defaults = array_filter(self::CONFIG_OPTIONS, static function (string $option): bool {
            return 'negative_instanceof' !== $option && 'others' !== $option && 'yield_from' !== $option;
        });
        return new FixerConfigurationResolver([(new FixerOptionBuilder('statements', 'List of control statements to fix.'))->setAllowedTypes(['string[]'])->setAllowedValues([new AllowedValueSubset(self::CONFIG_OPTIONS)])->setDefault(array_values($defaults))->getOption()]);
    }
    private function isUselessWrapped(Tokens $tokens, int $beforeOpenIndex, int $afterCloseIndex): bool
    {
        return $this->isSingleStatement($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedFnBody($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedForElement($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedLanguageConstructArgument($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedSequenceElement($tokens, $beforeOpenIndex, $afterCloseIndex);
    }
    private function isWrappedCloneArgument(Tokens $tokens, int $beforeOpenIndex, int $openIndex, int $closeIndex, int $afterCloseIndex): bool
    {
        ...
        if (!($tokens[$beforeOpenIndex]->equals('?') || $this->isSimpleAssignment($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isSingleStatement($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedFnBody($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedForElement($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedSequenceElement($tokens, $beforeOpenIndex, $afterCloseIndex))) {
            return \false;
        }
        ...
        if ($tokens[$newCandidateIndex]->isGivenKind(\T_NEW)) {
            ...
            // `clone (new X)`, `clone (new X())`, clone (new X(Y))`
        }
    ...
    private function getIndexOfInstanceOfStatement(Tokens $tokens, int $openIndex, int $closeIndex): ?int
    {
        ...
        reset($instanceOfIndex);
        return 1 === \count($instanceOfIndex) ? key($instanceOfIndex) : null;
    }
    ...
    {
        if ($this->containsOperation($tokens, $openIndex, $instanceOfIndex) || $this->containsOperation($tokens, $instanceOfIndex, $closeIndex)) {
            return \false;
        }
        if ($tokens[$beforeOpenIndex]->equals('!')) {
            $beforeOpenIndex = $tokens->getPrevMeaningfulToken($beforeOpenIndex);
        }
        return $this->isSimpleAssignment($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isSingleStatement($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedFnBody($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedForElement($tokens, $beforeOpenIndex, $afterCloseIndex) || $this->isWrappedSequenceElement($tokens, $beforeOpenIndex, $afterCloseIndex);
    }
    private function isWrappedPartOfOperation(Tokens $tokens, int $beforeOpenIndex, int $openIndex, int $closeIndex, int $afterCloseIndex): bool
    {
        ...
        if ($this->isAccess($tokens, $afterCloseIndex)) {
            ...
            if ($this->tokensAnalyzer->isUnarySuccessorOperator($afterCloseIndex)) {
                // post unary operation are only valid here
        ...
        $beforeIsBlockOpenOrComma = $beforeToken->equals(',') || null !== $this->getBlock($tokens, $beforeOpenIndex, \true);
        ...
        return $beforeIsStatementOpen && $afterIsBinaryOperation || $beforeIsBinaryOperation && $afterIsStatementEnd;
        // `+ (X);`
    }
    ...
    private function isWrappedSequenceElement(Tokens $tokens, int $startIndex, int $endIndex): bool
    {
        ...
        $blockTypeStart = $this->getBlock($tokens, $startIndex, \true);
        $blockTypeEnd = $this->getBlock($tokens, $endIndex, \false);
        return $startIsComma && null !== $blockTypeEnd || $endIsComma && null !== $blockTypeStart || null !== $blockTypeEnd && null !== $blockTypeStart;
        // any type of `{(X)}`, `[(X)]` and `((X))`
    }
    ...
    {
        while (\true) {
            $block = $this->getBlock($tokens, $index, \true);
            if (null !== $block) {
                $index = $tokens->findBlockEnd($block['type'], $index);
                $index = $tokens->getNextMeaningfulToken($index);
                continue;
            }
            if ($tokens[$index]->isObjectOperator() || $tokens[$index]->equalsAny(['$', [\T_PAAMAYIM_NEKUDOTAYIM], [\T_STRING], [\T_VARIABLE]])) {
                $index = $tokens->getNextMeaningfulToken($index);
                continue;
            }
            break;
    ...
    private function getBlock(Tokens $tokens, int $index, bool $isStart): ?array
    {
        ...
        return null !== $block && $isStart === $block['isStart'] && \in_array($block['type'], self::BLOCK_TYPES, \true) ? $block : null;
    }
    ...
    private function removeUselessParenthesisPair(Tokens $tokens, int $beforeOpenIndex, int $afterCloseIndex, int $openIndex, int $closeIndex, ?string $configType): void
    {
        $statements = $this->configuration['statements'];
        if (null === $configType || !\in_array($configType, $statements, \true)) {
            return;
        }
        $needsSpaceAfter = !$this->isAccess($tokens, $afterCloseIndex) && !$tokens[$afterCloseIndex]->equalsAny([';', ',', [\T_CLOSE_TAG]]) && null === $this->getBlock($tokens, $afterCloseIndex, \false) && !($tokens[$afterCloseIndex]->equalsAny([':', ';']) && $tokens[$beforeOpenIndex]->isGivenKind(\T_CASE));
        $needsSpaceBefore = !$this->isPreUnaryOperation($tokens, $beforeOpenIndex) && !$tokens[$beforeOpenIndex]->equalsAny(['}', [\T_EXIT], [\T_OPEN_TAG]]) && null === $this->getBlock($tokens, $beforeOpenIndex, \true);
