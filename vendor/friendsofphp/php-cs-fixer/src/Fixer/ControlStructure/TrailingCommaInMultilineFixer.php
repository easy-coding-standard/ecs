...
, ['after_heredoc' => \true]), new CodeSample("<?php\nfoo(\n    1,\n    2\n);\n", ['elements' => [self::ELEMENTS_ARGUMENTS]]), new VersionSpecificCodeSample("<?php\nfunction foo(\n    \$x,\n    \$y\n)\n{\n}\n", new VersionSpecification(80000), ['elements' => [self::ELEMENTS_PARAMETERS]])]);
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        $fixArrays = \in_array(self::ELEMENTS_ARRAYS, $configuredElements, \true);
        $fixArguments = \in_array(self::ELEMENTS_ARGUMENTS, $configuredElements, \true);
        $fixParameters = \PHP_VERSION_ID >= 80000 && \in_array(self::ELEMENTS_PARAMETERS, $configuredElements, \true);
        // @TODO: drop condition when PHP 8.0+ is required
        $fixMatch = \PHP_VERSION_ID >= 80000 && \in_array(self::MATCH_EXPRESSIONS, $configuredElements, \true);
        // @TODO: drop condition when PHP 8.0+ is required
        ...
        for ($index = $tokens->count() - 1; $index >= 0; --$index) {
            ...
            if ($fixArguments && $tokens[$prevIndex]->equalsAny([']', [\T_CLASS], [\T_STRING], [\T_VARIABLE], [\T_STATIC], [\T_ISSET], [\T_UNSET], [\T_LIST]]) && !$tokens[$prevPrevIndex]->isGivenKind(\T_FUNCTION)) {
                $this->fixBlock($tokens, $index);
                continue;
            }
