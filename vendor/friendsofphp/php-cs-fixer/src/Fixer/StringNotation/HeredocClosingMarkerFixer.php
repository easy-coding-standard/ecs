...
final class HeredocClosingMarkerFixer extends AbstractFixer implements ConfigurableFixerInterface
{
     ...
     */
    public const RESERVED_CLOSING_MARKERS = ['CSS', 'DIFF', 'HTML', 'JS', 'JSON', 'MD', 'PHP', 'PYTHON', 'RST', 'TS', 'SQL', 'XML', 'YAML'];
...
, ['explicit_heredoc_style' => \true])]);
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        foreach ($tokens as $index => $token) {
            ...
            if (null !== $startIndex && $token->isGivenKind(\T_END_HEREDOC)) {
                $existingClosingMarker = \trim($token->getContent());
    ...
    {
        $isNowdoc = \strpos($startToken->getContent(), '\'') !== \false;
        $markerQuote = $isNowdoc ? '\'' : (\true === $this->configuration['explicit_heredoc_style'] ? '"' : '');
        return [new Token([$startToken->getId(), Preg::replace('/<<<\\h*\\K["\']?[^\\s"\']+["\']?/', $markerQuote . $newClosingMarker . $markerQuote, $startToken->getContent())]), new Token([$endToken->getId(), Preg::replace('/\\S+/', $newClosingMarker, $endToken->getContent())])];
    }
}
