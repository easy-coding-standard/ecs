...
final class FunctionToConstantFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('functions', 'List of function names to fix.'))->setAllowedTypes(['string[]'])->setAllowedValues([new AllowedValueSubset($functionNames)])->setDefault(['get_called_class', 'get_class', 'get_class_this', 'php_sapi_name', 'phpversion', 'pi'])->getOption()]);
    }
    /**
    ...
    private function fixFunctionCallToConstant(Tokens $tokens, int $index, int $braceOpenIndex, int $braceCloseIndex, array $replacements): void
    {
        ...
        if ($replacements[0]->isGivenKind([\T_CLASS_C, \T_STATIC]) || $replacements[0]->isGivenKind(\T_STRING) && 'self' === $replacements[0]->getContent()) {
    ...
    {
        $clones = array_map(static function (Token $token): Token {
            return clone $token;
