...
PHP
, ['constructs_followed_by_a_single_space' => ['yield_from']])]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        $tokenKinds = array_merge(array_values($this->fixTokenMapContainASingleSpace), array_values($this->fixTokenMapPrecededByASingleSpace), array_values($this->fixTokenMapFollowedByASingleSpace));
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        for ($index = $tokens->count() - 2; $index >= 0; --$index) {
            ...
            if ($token->isGivenKind(\T_OPEN_TAG)) {
                if ($tokens[$whitespaceTokenIndex]->isGivenKind(\T_WHITESPACE) && strpos($tokens[$whitespaceTokenIndex]->getContent(), "\n") === \false && strpos($token->getContent(), "\n") === \false) {
            ...
            if ($tokens[$whitespaceTokenIndex]->isWhitespace() && strpos($tokens[$whitespaceTokenIndex]->getContent(), "\n") !== \false) {
                $nextNextToken = $tokens[$whitespaceTokenIndex + 1];
                if ($nextNextToken->isGivenKind(FCT::T_ATTRIBUTE) || $nextNextToken->isComment() && strncmp($nextNextToken->getContent(), '#[', strlen('#[')) === 0) {
                    continue;
                }
    ...
    private function isMultilineExtendsOrImplementsWithMoreThanOneAncestor(Tokens $tokens, int $index): bool
    {
        ...
        return LogicException('Not reachable code was reached.');
    ...
    private function isMultilineCommaSeparatedConstant(Tokens $tokens, int $constantIndex): bool
    {
        ...
        while (!$tokens[$index]->equalsAny([';', [\T_CLOSE_TAG]])) {
            ++$index;
            $isMultilineConstant = $isMultilineConstant || strpos($tokens[$index]->getContent(), "\n") !== \false;
            if ($tokens[$index]->equals(',')) {
                $hasMoreThanOneConstant = \true;
            }
            $blockType = Tokens::detectBlockType($tokens[$index]);
            if (null !== $blockType && \true === $blockType['isStart']) {
    ...
    private function isFullLineCommentBefore(Tokens $tokens, int $index): bool
    {
        ...
        return strncmp($content, '#', strlen('#')) === 0 || strncmp($content, '//', strlen('//')) === 0;
    }
}
