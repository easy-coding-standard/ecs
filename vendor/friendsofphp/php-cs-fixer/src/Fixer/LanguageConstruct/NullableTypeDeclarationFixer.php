...
\class_alias('ECSPrefix202605\\ValueObject', 'ValueObject', \false);
...
, new VersionSpecification(80000), ['syntax' => self::OPTION_SYNTAX_QUESTION_MARK])]);
    ...
    {
        return \PHP_VERSION_ID >= 80000 && $tokens->isTokenKindFound($this->candidateTokenKind);
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        $this->candidateTokenKind = self::OPTION_SYNTAX_QUESTION_MARK === $this->configuration['syntax'] ? CT::T_TYPE_ALTERNATION : CT::T_NULLABLE_TYPE;
        // `?` -> `|`
    }
    protected function createConfigurationDefinition() : FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('syntax', 'Whether to use question mark (`?`) or explicit `null` union for nullable type.'))->setAllowedValues([self::OPTION_SYNTAX_UNION, self::OPTION_SYNTAX_QUESTION_MARK])->setDefault(self::OPTION_SYNTAX_QUESTION_MARK)->getOption()]);
    }
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        $functionsAnalyzer = new FunctionsAnalyzer();
        foreach (\array_reverse($this->getElements($tokens), \true) as $index => $type) {
            if ('property' === $type) {
    ...
    private function getElements(Tokens $tokens) : array
    {
        ...
        $elements = \array_map(static function (array $element) : string {
            return 'method' === $element['type'] ? 'function' : $element['type'];
        }, \array_filter($tokensAnalyzer->getClassyElements(), static function (array $element) : bool {
            return \in_array($element['type'], ['method', 'property'], \true);
        }));
        foreach ($tokens as $index => $token) {
            if ($token->isGivenKind(\T_FN) || $token->isGivenKind(\T_FUNCTION) && !isset($elements[$index])) {
    ...
    {
        foreach (\array_reverse($functionsAnalyzer->getFunctionArguments($tokens, $index), \true) as $argumentInfo) {
    ...
    private function normalizeNullableType(Tokens $tokens, TypeAnalysis $typeAnalysis) : void
    {
        ...
        $tokens->overrideRange($typeAnalysis->getStartIndex(), $typeAnalysis->getEndIndex(), $this->createTypeDeclarationTokens($normalizedType, $isQuestionMarkSyntax));
    ...
    private function convertToNullableType(string $type) : array
    {
        if (\strncmp($type, '?', \strlen('?')) === 0) {
            return [$type];
            // no need to convert; already fixed
        }
    ...
    private function convertToExplicitUnionType(string $type) : array
    {
        ...
        return ['null', (string) \substr($type, 1)];
    }
    /**
    ...
    private function createTypeDeclarationTokens(array $types, bool $isQuestionMarkSyntax) : array
    {
        ...
        foreach ($types as $index => $type) {
            $specialType = ['?' => CT::T_NULLABLE_TYPE, 'array' => CT::T_ARRAY_TYPEHINT, 'callable' => \T_CALLABLE, 'static' => \T_STATIC][\strtolower($type)] ?? null;
            if (null !== $specialType) {
