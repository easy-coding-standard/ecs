...
final class NullableTypeDeclarationForDefaultNullValueFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    private const CONSTRUCTOR_PROPERTY_MODIFIERS = [CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PUBLIC, CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PROTECTED, CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PRIVATE, FCT::T_PUBLIC_SET, FCT::T_PROTECTED_SET, FCT::T_PRIVATE_SET, FCT::T_READONLY];
    public function getDefinition(): FixerDefinitionInterface
    {
        return new FixerDefinition('Adds or removes `?` before single type declarations or `|null` at the end of union types when parameters have a default `null` value.', [new CodeSample("<?php\nfunction sample(string \$str = null)\n{}\n"), new CodeSample("<?php\nfunction sample(?string \$str = null)\n{}\n", ['use_nullable_type_declaration' => \false]), new VersionSpecificCodeSample("<?php\nfunction sample(string|int \$str = null)\n{}\n", new VersionSpecification(80000)), new VersionSpecificCodeSample("<?php\nfunction sample(string|int|null \$str = null)\n{}\n", new VersionSpecification(80000), ['use_nullable_type_declaration' => \false]), new VersionSpecificCodeSample("<?php\nfunction sample(\\Foo&\\Bar \$str = null)\n{}\n", new VersionSpecification(80200)), new VersionSpecificCodeSample("<?php\nfunction sample((\\Foo&\\Bar)|null \$str = null)\n{}\n", new VersionSpecification(80200), ['use_nullable_type_declaration' => \false])]);
    }
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('use_nullable_type_declaration', 'Whether to add or remove `?` or `|null` to parameters with a default `null` value.'))->setAllowedTypes(['bool'])->setDefault(\true)->setDeprecationMessage('Behaviour will follow default one.')->getOption()]);
    }
    ...
    private function fixFunctionParameters(Tokens $tokens, array $arguments): void
    {
        foreach (array_reverse($arguments) as $argumentInfo) {
            if (!$argumentInfo->hasTypeAnalysis() || \in_array(strtolower($argumentInfo->getTypeAnalysis()->getName()), ['mixed', 'null'], \true) || !$argumentInfo->hasDefault() || 'null' !== strtolower($argumentInfo->getDefault())) {
                continue;
            }
            ...
            if (strpos($typeAnalysisName, '|') !== \false || strpos($typeAnalysisName, '&') !== \false) {
    ...
    private function fixUnionTypeParameter(Tokens $tokens, TypeAnalysis $argumentTypeInfo): void
    {
        if (\true === $this->configuration['use_nullable_type_declaration']) {
            ...
            $tokens->insertAt($endIndex + 1, [new Token([CT::T_TYPE_ALTERNATION, '|']), new Token([\T_STRING, 'null'])]);
        } elseif ($argumentTypeInfo->isNullable()) {
            ...
            $typeAnalysisName = $argumentTypeInfo->getName();
            if (strpos($typeAnalysisName, '&') !== \false && 1 === substr_count($typeAnalysisName, '|')) {
