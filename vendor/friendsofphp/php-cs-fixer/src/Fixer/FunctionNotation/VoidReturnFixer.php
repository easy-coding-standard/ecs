 ...
 */
final class VoidReturnFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    /** @use ConfigurableFixerTrait<_AutogeneratedInputConfiguration, _AutogeneratedComputedConfiguration> */
    use ConfigurableFixerTrait;
    private const PREVIOUS_TOKENS = [\T_ABSTRACT, \T_FINAL, \T_PRIVATE, \T_PROTECTED, \T_PUBLIC, \T_STATIC, FCT::T_ATTRIBUTE];
    /**
...
PHP;
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('fix_lambda', 'Whether to fix lambda functions as well.'))->setAllowedTypes(['bool'])->setDefault(\true)->getOption()]);
    }
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        for ($index = $tokens->count() - 1; 0 <= $index; --$index) {
            ...
            if (\false === $this->configuration['fix_lambda'] && $this->tokensAnalyzer->isLambda($index)) {
                continue;
            }
            $functionName = $tokens->getNextMeaningfulToken($index);
            // These cause syntax errors.
            if ($tokens[$functionName]->equalsAny([[\T_STRING, '__clone'], [\T_STRING, '__construct'], [\T_STRING, '__debugInfo'], [\T_STRING, '__destruct'], [\T_STRING, '__isset'], [\T_STRING, '__serialize'], [\T_STRING, '__set_state'], [\T_STRING, '__sleep'], [\T_STRING, '__toString']], \false)) {
                continue;
            }
