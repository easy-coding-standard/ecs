...
final class NoUselessPrintfFixer extends AbstractFixer
{
    ...
    {
        return new FixerDefinition('There must be no `printf` calls with only the first argument.', [new CodeSample("<?php\n\nprintf('bar');\n")], null, 'Risky when the `printf` function is overridden.');
    }
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        $tokens->insertSlices(array_combine($printfIndices, array_fill(0, \count($printfIndices), [new Token([\T_PRINT, 'print']), new Token([\T_WHITESPACE, ' '])])));
    }
}
