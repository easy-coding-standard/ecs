...
$b = (function () {
    \assert($this !== null);
    // approach you can use to instruct PHP CS Fixer to not convert this lambda to static, e.g. when you see "Cannot bind an instance to a static closure" error caused by lambda handling outside of your control
})->bindTo(new \stdClass());
    ...
    private function hasPossibleReferenceToThis(Tokens $tokens, int $startIndex, int $endIndex): bool
    {
        for ($i = $startIndex; $i <= $endIndex; ++$i) {
            ...
            if ($tokens[$i]->isGivenKind([
                \T_INCLUDE,
                // loading additional symbols we cannot analyse here
                \T_INCLUDE_ONCE,
                // "
                \T_REQUIRE,
                // "
