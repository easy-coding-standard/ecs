...
final class SingleLineThrowFixer extends AbstractFixer
{
    ...
    {
        return $token->equalsAny(array_merge(self::REMOVE_WHITESPACE_AFTER_TOKENS, self::REMOVE_WHITESPACE_AROUND_TOKENS)) || $token->isObjectOperator();
    }
    private function isNextTokenToClear(Token $token): bool
    {
        return $token->equalsAny(array_merge(self::REMOVE_WHITESPACE_AROUND_TOKENS, self::REMOVE_WHITESPACE_BEFORE_TOKENS)) || $token->isObjectOperator();
    }
}
