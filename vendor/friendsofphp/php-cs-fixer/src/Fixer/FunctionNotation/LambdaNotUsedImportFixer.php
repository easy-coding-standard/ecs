...
{
    /**
     * @var \PhpCsFixer\Tokenizer\Analyzer\ArgumentsAnalyzer
     */
    ...
    private function findNotUsedLambdaImports(Tokens $tokens, array $imports, int $lambdaUseCloseBraceIndex): array
    {
        ...
        for ($index = $lambdaOpenIndex;; ++$index) {
            ...
            if ($token->isGivenKind([CT::T_DYNAMIC_VAR_BRACE_OPEN, \T_EVAL, \T_INCLUDE, \T_INCLUDE_ONCE, \T_REQUIRE, \T_REQUIRE_ONCE])) {
                return [];
            }
            if ($token->equals('$')) {
