...
final class PhpdocToParamTypeFixer extends AbstractPhpdocToTypeDeclarationFixer implements ConfigurableFixerInterface, ExperimentalFixerInterface
{
     ...
     */
    private const SKIPPED_TYPES = ['resource' => \true, 'static' => \true, 'void' => \true];
...
PHP
, ['union_types' => \false])], null, 'The `@param` annotation is mandatory for the fixer to make changes, signatures of methods without it (no docblock, inheritdocs) will not be fixed. Manual actions are required if inherited signatures are not properly documented.');
    }
        ...
        foreach ($tokens as $index => $token) {
            ...
            foreach ($this->getAnnotationsFromDocComment('param', $tokens, $docCommentIndex) as $paramTypeAnnotation) {
                ...
                if (null !== $typeInfo) {
                    ...
                    // null !== $unionTypes, because of previous 2 ifs
                    $paramType = $unionTypes;
                    $isNullable = \false;
                }
                if (\in_array($paramType, $typesToExclude, \true)) {
                    continue;
                }
                ...
                $tokensToInsert[$variableIndex] = \array_merge($this->createTypeDeclarationTokens($paramType, $isNullable), [new Token([\T_WHITESPACE, ' '])]);
            }
        }
