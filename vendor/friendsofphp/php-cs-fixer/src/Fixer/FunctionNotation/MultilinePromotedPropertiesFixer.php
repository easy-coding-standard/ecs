...
, new VersionSpecification(80000), ['minimum_number_of_parameters' => 3])]);
    ...
    private function shouldBeFixed(Tokens $tokens, int $openParenthesisIndex, int $closeParenthesisIndex) : bool
    {
        ...
        for ($index = $openParenthesisIndex + 1; $index < $closeParenthesisIndex; ++$index) {
            ...
            if ($tokens[$index]->isGivenKind([CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PRIVATE, CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PROTECTED, CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PUBLIC])) {
                $promotedParameterFound = \true;
            }
        }
    ...
    {
        if ($this->configuration['keep_blank_lines'] && $tokens[$index]->isWhitespace() && \strpos($tokens[$index]->getContent(), "\n") !== \false) {
            return;
        }
        $tokens->ensureWhitespaceAtIndex($index, 0, $this->whitespacesConfig->getLineEnding() . $indent . $this->whitespacesConfig->getIndent());
    }
}
