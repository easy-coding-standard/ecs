...
namespace ECSPrefix202605\space1;
...
PHP
, ['include' => [self::SET_ALL]]), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['include' => [self::SET_INTERNAL]]), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['include' => [self::SET_COMPILER_OPTIMIZED]])], null, 'Risky when any of the functions are overridden.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('exclude', 'List of functions to ignore.'))->setAllowedTypes(['string[]'])->setAllowedValues([static function (array $value) : bool {
            foreach ($value as $functionName) {
                if ('' === \trim($functionName) || \trim($functionName) !== $functionName) {
    ...
    private function normalizeFunctionNames(array $functionNames) : array
    {
        ...
        foreach ($functionNames as $functionName) {
            $result[\strtolower($functionName)] = \true;
        }
        return $result;
    }
}
