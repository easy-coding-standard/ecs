...
final class NoSpacesAfterFunctionNameFixer extends AbstractFixer
{
     ...
     */
    private const FUNCTIONY_TOKEN_KINDS = [\T_ARRAY, \T_ECHO, \T_EMPTY, \T_EVAL, \T_EXIT, \T_INCLUDE, \T_INCLUDE_ONCE, \T_ISSET, \T_LIST, \T_PRINT, \T_REQUIRE, \T_REQUIRE_ONCE, \T_UNSET, \T_VARIABLE];
    ...
    {
        return $tokens->isAnyTokenKindsFound(array_merge([\T_STRING], self::FUNCTIONY_TOKEN_KINDS));
    }
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        foreach ($tokens as $index => $token) {
            ...
            if (null !== $nextNonWhiteSpace && !$tokens[$nextNonWhiteSpace]->equals(';') && $tokens[$lastTokenIndex]->isGivenKind([\T_ECHO, \T_PRINT, \T_INCLUDE, \T_INCLUDE_ONCE, \T_REQUIRE, \T_REQUIRE_ONCE])) {
                continue;
            }
            ...
            } elseif ($tokens[$lastTokenIndex]->equalsAny([')', ']', [CT::T_DYNAMIC_VAR_BRACE_CLOSE], [CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE]])) {
