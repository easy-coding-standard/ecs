    ...
    private function fixFunctionDefinition(Tokens $tokens, int $startIndex) : void
    {
        ...
        foreach (\array_reverse((new FunctionsAnalyzer())->getFunctionArguments($tokens, $startIndex)) as $argumentAnalysis) {
            ...
            if (null === $argumentAnalysis->getDefault()) {
                $removeDefaultArgument = \true;
                continue;
            }
            if (!$removeDefaultArgument) {
                continue;
            }
            if ('null' === \strtolower($argumentAnalysis->getDefault()) && $argumentAnalysis->hasTypeAnalysis() && !$argumentAnalysis->getTypeAnalysis()->isNullable()) {
                continue;
            }
            $this->removeDefaultValue($tokens, $argumentAnalysis->getNameIndex(), $this->getDefaultValueEnd($tokens, $argumentAnalysis->getNameIndex()));
        }
    }
