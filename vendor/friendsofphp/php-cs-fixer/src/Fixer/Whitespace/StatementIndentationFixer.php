...
final class StatementIndentationFixer extends AbstractFixer implements ConfigurableFixerInterface, WhitespacesAwareFixerInterface
{
    ...
    private const BLOCK_SIGNATURE_FIRST_TOKENS = [\T_USE, \T_IF, \T_ELSE, \T_ELSEIF, \T_FOR, \T_FOREACH, \T_WHILE, \T_DO, \T_SWITCH, \T_CASE, \T_DEFAULT, \T_TRY, \T_CLASS, \T_INTERFACE, \T_TRAIT, \T_EXTENDS, \T_IMPLEMENTS, \T_CONST, FCT::T_MATCH];
     ...
     */
    private $alternativeSyntaxAnalyzer;
    /**
     * @var bool
     */
...
PHP
, ['stick_comment_to_next_continuous_control_statement' => \true])]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('stick_comment_to_next_continuous_control_statement', 'Last comment of code block counts as comment for next block.'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption()]);
    }
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        $lastIndent = $this->getLineIndentationWithBracesCompatibility($tokens, 0, $this->extractIndent($this->computeNewLineContent($tokens, 0)));
        /**
         * @var list<array{
         ...
         * }> $scopes
         */
        $scopes = [['type' => 'block', 'skip' => \false, 'end_index' => $endIndex, 'end_index_inclusive' => \true, 'initial_indent' => $lastIndent, 'is_indented_block' => \false]];
        ...
        foreach ($tokens as $index => $token) {
            ...
            if (isset($noBracesBlockStarts[$index])) {
                $scopes[] = ['type' => 'block', 'skip' => \false, 'end_index' => $this->findStatementEndIndex($tokens, $index, \count($tokens) - 1) + 1, 'end_index_inclusive' => \true, 'initial_indent' => $this->getLineIndentationWithBracesCompatibility($tokens, $index, $lastIndent), 'is_indented_block' => \true];
                ++$currentScope;
            }
            if ($token->equalsAny(self::BLOCK_FIRST_TOKENS) || $token->equals('(') && !$tokens[$tokens->getPrevMeaningfulToken($index)]->isGivenKind(\T_ARRAY) || isset($alternativeBlockStarts[$index]) || isset($caseBlockStarts[$index])) {
                $endIndexInclusive = \true;
                ...
                $scopes[] = ['type' => 'block', 'skip' => $skip, 'end_index' => $endIndex, 'end_index_inclusive' => $endIndexInclusive, 'initial_indent' => $initialIndent, 'is_indented_block' => \true];
                ++$currentScope;
            ...
            if ($isPropertyStart || $token->isGivenKind(self::BLOCK_SIGNATURE_FIRST_TOKENS)) {
                ...
                for ($endIndex = $index + 1, $max = \count($tokens); $endIndex < $max; ++$endIndex) {
                    ...
                    if (!$endToken->isComment()) {
                        ...
                        $noBracesBlockStarts[$noBraceBlockStartIndex] = \true;
                        $endIndex = $closingParenthesisIndex ?? $index;
                        break;
                    }
                }
                $scopes[] = ['type' => 'block_signature', 'skip' => \false, 'end_index' => $endIndex, 'end_index_inclusive' => \true, 'initial_indent' => $this->getLineIndentationWithBracesCompatibility($tokens, $index, $lastIndent), 'is_indented_block' => $isPropertyStart || $token->isGivenKind([\T_EXTENDS, \T_IMPLEMENTS, \T_CONST])];
                continue;
            }
            if ($token->isGivenKind(\T_FUNCTION)) {
                ...
                $scopes[] = ['type' => 'block_signature', 'skip' => \false, 'end_index' => $endIndex, 'end_index_inclusive' => \true, 'initial_indent' => $this->getLineIndentationWithBracesCompatibility($tokens, $index, $lastIndent), 'is_indented_block' => \false];
                continue;
            }
            if ($token->isWhitespace() || $index > 0 && $tokens[$index - 1]->isGivenKind(\T_OPEN_TAG)) {
                $previousOpenTagContent = $tokens[$index - 1]->isGivenKind(\T_OPEN_TAG) ? Preg::replace('/\\S/', '', $tokens[$index - 1]->getContent()) : '';
                $content = $previousOpenTagContent . ($token->isWhitespace() ? $token->getContent() : '');
                if (!Preg::match('/\\R/', $content)) {
                    continue;
                }
                $nextToken = $tokens[$index + 1] ?? null;
                if ($this->bracesFixerCompatibility && null !== $nextToken && $nextToken->isComment() && !$this->isCommentWithFixableIndentation($tokens, $index + 1)) {
                    continue;
                }
                if ('block' === $scopes[$currentScope]['type'] || 'block_signature' === $scopes[$currentScope]['type']) {
                    ...
                    if ($scopes[$currentScope]['is_indented_block']) {
                        ...
                        if (null !== $firstNonWhitespaceTokenIndex && $firstNonWhitespaceTokenIndex < $endIndex || null !== $nextNewlineIndex && $nextNewlineIndex < $endIndex) {
                            if ($tokens[$firstNonWhitespaceTokenIndex]->isGivenKind(\T_COMMENT) && $tokens[$tokens->getNextMeaningfulToken($firstNonWhitespaceTokenIndex)]->equals('}')) {
                                if ($tokens[$tokens->getPrevMeaningfulToken($firstNonWhitespaceTokenIndex)]->equals('{')) {
                                    $indent = \true;
                                } else {
                                    ...
                                    if (null !== $nextNextIndex && $tokens[$nextNextIndex]->isGivenKind([\T_ELSE, \T_ELSEIF])) {
                                        $indent = \true !== $this->configuration['stick_comment_to_next_continuous_control_statement'];
                                    } else {
                                        $indent = \true;
                                    }
                                }
                    ...
                    $content = Preg::replace('/(\\R+)\\h*$/', '$1' . $whitespaces, $content);
                    $previousLineNewIndent = $this->extractIndent($content);
                } else {
                    $content = Preg::replace('/(\\R)' . $scopes[$currentScope]['initial_indent'] . '(\\h*)$/D', '$1' . $scopes[$currentScope]['new_indent'] . '$2', $content);
                }
                ...
                if (null !== $nextToken && $nextToken->isComment()) {
                    $tokens[$index + 1] = new Token([$nextToken->getId(), Preg::replace('/(\\R)' . \preg_quote($previousLineInitialIndent, '/') . '(\\h*\\S+.*)/', '$1' . $previousLineNewIndent . '$2', $nextToken->getContent())]);
                }
            ...
            if ('statement' !== $scopes[$currentScope]['type'] && 'block_signature' !== $scopes[$currentScope]['type']) {
                ...
                $scopes[] = ['type' => 'statement', 'skip' => \false, 'end_index' => $endIndex, 'end_index_inclusive' => \false, 'initial_indent' => $previousLineInitialIndent, 'new_indent' => $previousLineNewIndent, 'is_indented_block' => \true];
            }
        }
    }
    private function findStatementEndIndex(Tokens $tokens, int $index, int $parentScopeEndIndex) : int
    {
        ...
        for ($searchEndIndex = $index; $searchEndIndex <= $parentScopeEndIndex; ++$searchEndIndex) {
            ...
            if ($searchEndToken->isGivenKind(\T_IF) && !$tokens[$tokens->getPrevMeaningfulToken($searchEndIndex)]->isGivenKind(\T_ELSE)) {
                ++$ifLevel;
                continue;
            }
            ...
            if ($ifLevel > 0 && null !== $controlStructureContinuationIndex && $tokens[$controlStructureContinuationIndex]->isGivenKind([\T_ELSE, \T_ELSEIF])) {
                if ($tokens[$controlStructureContinuationIndex]->isGivenKind(\T_ELSE) && !$tokens[$tokens->getNextMeaningfulToken($controlStructureContinuationIndex)]->isGivenKind(\T_IF)) {
                    --$ifLevel;
                }
                $searchEndIndex = $controlStructureContinuationIndex;
                continue;
            }
            if ($doWhileLevel > 0 && null !== $controlStructureContinuationIndex && $tokens[$controlStructureContinuationIndex]->isGivenKind(\T_WHILE)) {
                --$doWhileLevel;
                $searchEndIndex = $controlStructureContinuationIndex;
                continue;
            }
    ...
    private function isPropertyStart(Tokens $tokens, int $index) : bool
    {
        ...
        if (null === $nextIndex || $tokens[$nextIndex]->isGivenKind(self::PROPERTY_KEYWORDS) || $tokens[$nextIndex]->isGivenKind([\T_CONST, \T_FUNCTION])) {
            return \false;
        }
    ...
    private function isCommentWithFixableIndentation(Tokens $tokens, int $index) : bool
    {
        ...
        while (\true) {
            $lastCommentCandidateIndex = $this->getSiblingContinuousSingleLineComment($tokens, $lastCommentIndex, \true);
