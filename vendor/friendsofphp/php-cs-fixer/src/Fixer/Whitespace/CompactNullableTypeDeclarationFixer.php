...
final class CompactNullableTypeDeclarationFixer extends AbstractFixer
{
    ...
    {
        return new FixerDefinition('Remove extra spaces in a nullable type declaration.', [new CodeSample("<?php\nfunction sample(? string \$str): ? string\n{}\n")]);
    }
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        for ($index = $tokens->count() - 1; $index >= 0; --$index) {
            ...
            if ($tokens[$index + 1]->isWhitespace() && $tokens[$index + 2]->isGivenKind([CT::T_ARRAY_TYPEHINT, \T_CALLABLE, \T_NS_SEPARATOR, \T_STATIC, \T_STRING])) {
