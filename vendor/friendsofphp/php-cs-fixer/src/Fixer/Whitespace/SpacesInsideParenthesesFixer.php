...
PHP
, ['space' => 'single'])], 'By default there are not any additional spaces inside parentheses, however with `space=single` configuration option whitespace inside parentheses will be unified to single space.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        if ('single' === $this->configuration['space']) {
            foreach ($tokens as $index => $token) {
                ...
                if (1 === \count($blockContent) && \in_array(' ', $blockContent, \true)) {
                    $this->removeSpaceAroundToken($tokens, $index + 1);
                    continue;
                }
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('space', 'Whether to have `single` or `none` space inside parentheses.'))->setAllowedValues(['none', 'single'])->setDefault('none')->getOption()]);
    }
    /**
    ...
    private function fixParenthesisInnerEdge(Tokens $tokens, int $start, int $end): void
    {
        ...
        if ($tokens[$end - 1]->isWhitespace()) {
            ...
            if (' ' !== $content && strpos($content, "\n") === \false && !$tokens[$tokens->getPrevNonWhitespace($end - 1)]->isComment()) {
        ...
        if ($tokens[$start + 1]->isWhitespace()) {
            ...
            if (' ' !== $content && strpos($content, "\n") === \false && !$tokens[$tokens->getNextNonWhitespace($start + 1)]->isComment()) {
