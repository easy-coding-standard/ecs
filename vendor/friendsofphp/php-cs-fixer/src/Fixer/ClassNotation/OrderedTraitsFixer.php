...
final class OrderedTraitsFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('case_sensitive', 'Whether the sorting should be case sensitive.'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption()]);
    }
    ...
    private function sort(Tokens $tokens, array $elements): void
    {
        ...
        uasort($sortedElements, function (Tokens $useA, Tokens $useB) use ($toTraitName): int {
            return \true === $this->configuration['case_sensitive'] ? $toTraitName($useA) <=> $toTraitName($useB) : strcasecmp($toTraitName($useA), $toTraitName($useB));
        });
