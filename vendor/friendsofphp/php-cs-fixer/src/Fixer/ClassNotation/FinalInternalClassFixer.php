...
use ECSPrefix202605\Symfony\Component\OptionsResolver\Options;
/**
...
final class FinalInternalClassFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    private const DEFAULTS = ['include' => ['internal'], 'exclude' => ['final', 'Entity', 'ECSPrefix202605\ORM\Entity', 'ECSPrefix202605\ORM\Mapping\Entity', 'ECSPrefix202605\Mapping\Entity', 'Document', 'ECSPrefix202605\ODM\Document']];
    ...
    {
        return new FixerDefinition('Internal classes should be `final`.', [new CodeSample("<?php\n/**\n * @internal\n */\nclass Sample\n{\n}\n"), new CodeSample("<?php\n/**\n * @CUSTOM\n */\nclass A{}\n\n/**\n * @CUSTOM\n * @not-fix\n */\nclass B{}\n", ['include' => ['@Custom'], 'exclude' => ['@not-fix']])], null, 'Changing classes to `final` might cause code execution to break.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        for ($index = $tokens->count() - 1; 0 <= $index; --$index) {
            ...
            // make class 'final'
            $tokens->insertSlices([$index => [new Token([\T_FINAL, 'final']), new Token([\T_WHITESPACE, ' '])]]);
        }
    }
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('annotation_include', 'Class level attribute or annotation tags that must be set in order to fix the class (case insensitive).'))->setAllowedTypes(['string[]'])->setAllowedValues($annotationsAsserts)->setDefault(array_map(static function (string $string) {
            return '@' . $string;
        }, self::DEFAULTS['include']))->setNormalizer($annotationsNormalizer)->setDeprecationMessage('Use `include` to configure PHPDoc annotations tags and attributes.')->getOption(), (new FixerOptionBuilder('annotation_exclude', 'Class level attribute or annotation tags that must be omitted to fix the class, even if all of the white list ones are used as well (case insensitive).'))->setAllowedTypes(['string[]'])->setAllowedValues($annotationsAsserts)->setDefault(array_map(static function (string $string) {
            return '@' . $string;
        }, self::DEFAULTS['exclude']))->setNormalizer($annotationsNormalizer)->setDeprecationMessage('Use `exclude` to configure PHPDoc annotations tags and attributes.')->getOption(), (new FixerOptionBuilder('include', 'Class level attribute or annotation tags that must be set in order to fix the class (case insensitive).'))->setAllowedTypes(['string[]'])->setAllowedValues($annotationsAsserts)->setDefault(self::DEFAULTS['include'])->setNormalizer($annotationsNormalizer)->getOption(), (new FixerOptionBuilder('exclude', 'Class level attribute or annotation tags that must be omitted to fix the class, even if all of the white list ones are used as well (case insensitive).'))->setAllowedTypes(['string[]'])->setAllowedValues($annotationsAsserts)->setDefault(self::DEFAULTS['exclude'])->setNormalizer($annotationsNormalizer)->getOption(), (new FixerOptionBuilder('consider_absent_docblock_as_internal_class', 'Whether classes without any DocBlock should be fixed to final.'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption()]);
    }
    /**
    ...
    private function isClassCandidate(TokensAnalyzer $tokensAnalyzer, Tokens $tokens, int $index): bool
    {
        ...
        if (\in_array(\false, $decisions, \true)) {
            return \false;
        }
        return \in_array(\true, $decisions, \true) || [] === $decisions && \true === $this->configuration['consider_absent_docblock_as_internal_class'];
    }
    private function isClassCandidateBasedOnPhpDoc(Tokens $tokens, int $index): ?bool
    {
        ...
        foreach ($doc->getAnnotations() as $annotation) {
            ...
            $tag = strtolower((string) substr(array_shift($matches), 1));
            $tags[$tag] = \true;
        }
    ...
    private function isClassCandidateBasedOnAttribute(Tokens $tokens, int $startIndex, int $endIndex): ?bool
    {
        ...
        while ($currentIndex < $endIndex && null !== $currentIndex = $tokens->getNextMeaningfulToken($currentIndex)) {
            if (!$tokens[$currentIndex]->isGivenKind([\T_STRING, \T_NS_SEPARATOR])) {
                if ('' !== $attributeString) {
                    $attributeCandidates[$attributeString] = \true;
                    $attributeString = '';
                }
                continue;
            }
    ...
    private function assertConfigHasNoConflicts(): void
    {
        foreach (['include' => 'annotation_include', 'exclude' => 'annotation_exclude'] as $newConfigKey => $oldConfigKey) {
            ...
            if ($oldConfigIsSet) {
                ...
                // @phpstan-ignore-line crazy mapping, to be removed while cleaning up deprecated options
