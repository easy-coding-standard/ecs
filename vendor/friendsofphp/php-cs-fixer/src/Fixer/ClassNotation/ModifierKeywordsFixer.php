...
final class ModifierKeywordsFixer extends AbstractFixer implements ConfigurableFixerInterface
{
     ...
     */
    private $elements = ['const', 'method', 'property'];
...
\class_alias('ECSPrefix202605\\ClassName', 'ClassName', \false);
...
, ['elements' => ['const']])]);
    ...
    public function createConfigurationDefinition() : FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('elements', 'The structural elements to fix.'))->setAllowedTypes(['string[]'])->setAllowedValues([new AllowedValueSubset($elements)])->setDefault($elements)->getOption()]);
    }
    protected function configurePostNormalisation() : void
    {
        $this->elements = $this->configuration['elements'];
        if (\in_array('property', $this->elements, \true)) {
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        ...
        foreach (\array_reverse($tokensAnalyzer->getClassyElements(), \true) as $index => $element) {
            if (!\in_array($element['type'], $this->elements, \true)) {
                continue;
            }
            ...
            $expectedKinds = 'property' === $element['type'] || 'promoted_property' === $element['type'] ? self::EXPECTED_KINDS_PROPERTY_KINDS : self::EXPECTED_KINDS_GENERIC;
    ...
    {
        return ' ' === $tokens[$keywordIndex + 1]->getContent() && ($keywordIndex + 2 === $comparedIndex || $keywordIndex + 3 === $comparedIndex && $tokens[$keywordIndex + 2]->equals('&'));
    }
