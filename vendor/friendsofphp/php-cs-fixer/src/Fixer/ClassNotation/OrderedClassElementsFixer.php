...
- `method:*`: specify a single method name (e.g. `method:__invoke`) to set the order of that specific method.');
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens) : void
    {
        for ($i = 1, $count = $tokens->count(); $i < $count; ++$i) {
            ...
            \end($elements);
            $endIndex = $elements[\key($elements)]['end'];
            \reset($elements);
    ...
    protected function createConfigurationDefinition() : FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('order', 'List of strings defining order of elements.'))->setAllowedTypes(['string[]'])->setAllowedValues([static function (array $values) use($builtIns) : bool {
            foreach ($values as $value) {
                if (\in_array($value, $builtIns, \true)) {
                    return \true;
                }
    ...
    private function getElements(Tokens $tokens, int $startIndex) : array
    {
        ...
        while (\true) {
            ...
            for ($i = $startIndex;; ++$i) {
                ...
                } elseif (\in_array($element['type'], ['use_trait', 'case', 'method', 'magic', 'construct', 'destruct'], \true)) {
    ...
    private function sortElements(array $elements) : array
    {
        return Utils::stableSort(
            ...
            function (array $element) : array {
                return ['element' => $element, 'position' => $this->getTypePosition($element)];
            },
            /**
            ...
            function (array $a, array $b) : int {
                return $a['position'] === $b['position'] ? $this->sortGroupElements($a['element'], $b['element']) : $a['position'] <=> $b['position'];
            }
        );
    }
    /**
    ...
    {
        $type = $element['type'];
        if (\in_array($type, ['method', 'magic', 'phpunit'], \true) && isset($this->typePosition["method:{$element['name']}"])) {
        ...
        if (\array_key_exists($type, self::SPECIAL_TYPES)) {
            if (isset($this->typePosition[$type])) {
                ...
                if ('phpunit' === $type) {
                    $position += ['setupbeforeclass' => 1, 'dosetupbeforeclass' => 2, 'teardownafterclass' => 3, 'doteardownafterclass' => 4, 'setup' => 5, 'dosetup' => 6, 'assertpreconditions' => 7, 'assertpostconditions' => 8, 'teardown' => 9, 'doteardown' => 10][$element['name']];
                }
                return $position;
            }
            $type = 'method';
    ...
    private function sortGroupElements(array $a, array $b) : int
    {
        if (self::SORT_ALPHA === $this->configuration['sort_algorithm']) {
            return \true === $this->configuration['case_sensitive'] ? $a['name'] <=> $b['name'] : \strcasecmp($a['name'], $b['name']);
        }
