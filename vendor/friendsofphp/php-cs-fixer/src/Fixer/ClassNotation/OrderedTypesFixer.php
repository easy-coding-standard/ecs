...
interface Foo
{
    public function bar(\ECSPrefix202511\Aaa|\ECSPrefix202511\AA $foo) : string|int;
}
...
, new VersionSpecification(80000), ['sort_algorithm' => 'none', 'null_adjustment' => 'always_last'])]);
    ...
    private function sortTypes(TypeAnalysis $typeAnalysis, Tokens $tokens) : void
    {
        ...
        $tokens->overrideRange($typeAnalysis->getStartIndex(), $typeAnalysis->getEndIndex(), $this->createTypeDeclarationTokens($sortedTypes, $glue));
    }
    private function isTypeSortable(TypeAnalysis $type) : bool
    {
        return \strpos($type->getName(), '|') !== \false || \strpos($type->getName(), '&') !== \false;
    }
    /**
    ...
    private function collectDisjunctiveNormalFormTypes(string $type) : array
    {
        $types = \array_map(static function (string $subType) {
            if (\strncmp($subType, '(', \strlen('(')) === 0) {
                return \explode('&', \trim($subType, '()'));
            }
            return $subType;
        }, \explode('|', $type));
        return [$types, '|'];
    ...
    private function runTypesThroughSortingAlgorithm(array $types) : array
    {
        ...
        \usort($types, function ($a, $b) use($normalizeType) : int {
            ...
            if ('alpha' === $this->configuration['sort_algorithm']) {
                return \true === $this->configuration['case_sensitive'] ? $a <=> $b : \strcasecmp($a, $b);
            }
            return 0;
        });
        return $types;
    ...
    {
        $specialTypes = ['array' => [CT::T_ARRAY_TYPEHINT, 'array'], 'callable' => [\T_CALLABLE, 'callable'], 'static' => [\T_STATIC, 'static']];
        $count = \count($types);
        $newTokens = [];
        foreach ($types as $i => $type) {
            if (\is_array($type)) {
                $newTokens = \array_merge($newTokens, $this->createTypeDeclarationTokens($type, '&', \true));
            ...
            if ($i <= $count - 2) {
                $newTokens[] = new Token(['|' => [CT::T_TYPE_ALTERNATION, '|'], '&' => [CT::T_TYPE_INTERSECTION, '&']][$glue]);
            }
        }
        if ($isDisjunctive) {
