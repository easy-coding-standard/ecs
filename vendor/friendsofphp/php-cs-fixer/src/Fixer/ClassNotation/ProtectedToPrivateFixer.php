...
\class_alias('ECSPrefix202605\Sample', 'Sample', \false);
...
PHP
    ...
    {
        return $tokens->isAnyTokenKindsFound([\T_PROTECTED, CT::T_CONSTRUCTOR_PROPERTY_PROMOTION_PROTECTED, FCT::T_PROTECTED_SET]) && ($tokens->isAllTokenKindsFound([\T_CLASS, \T_FINAL]) || $tokens->isTokenKindFound(FCT::T_ENUM));
    }
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        $this->tokensAnalyzer = new TokensAnalyzer($tokens);
        $classesCandidate = [];
        $classElementTypes = ['method' => \true, 'property' => \true, 'promoted_property' => \true, 'const' => \true];
        foreach ($this->tokensAnalyzer->getClassyElements() as $index => $element) {
            $classIndex = $element['classIndex'];
            $classesCandidate[$classIndex] = $classesCandidate[$classIndex] ?? $this->isClassCandidate($tokens, $classIndex);
