...
final class PhpdocReadonlyClassCommentToKeywordFixer extends AbstractFixer
{
    ...
    {
        return \PHP_VERSION_ID >= 80200 && $tokens->isTokenKindFound(\T_DOC_COMMENT);
    }
    ...
    {
        return new FixerDefinition('Converts readonly comment on classes to the readonly keyword.', [new VersionSpecificCodeSample(<<<EOT
<?php
/** @readonly */
class C {
}
...
EOT
, new VersionSpecification(80200))], null, 'If classes marked with `@readonly` annotation were extended anyway, applying this fixer may break the inheritance for their child classes.');
    }
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        foreach ($tokens as $index => $token) {
            ...
            $addReadonly = \true;
            while ($tokens[$index]->isGivenKind([\T_ABSTRACT, \T_FINAL, \T_PRIVATE, \T_PUBLIC, \T_PROTECTED, \T_READONLY])) {
