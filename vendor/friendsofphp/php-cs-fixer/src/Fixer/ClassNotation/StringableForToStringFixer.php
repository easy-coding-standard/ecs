...
PHP
    ...
    {
        $uses = array_filter($useDeclarations, static function (NamespaceUseAnalysis $namespaceUseAnalysis) use ($index): bool {
            return $namespaceUseAnalysis->getStartIndex() === $index;
        });
        if (0 === \count($uses)) {
            return null;
        }
     ...
     */
    private static function doesImplementStringable(Tokens $tokens, int $classKeywordIndex, int $classOpenBraceIndex, array $stringableInterfaces): bool
    {
        $implementedInterfaces = self::getInterfaces($tokens, $classKeywordIndex, $classOpenBraceIndex);
        if ([] === $implementedInterfaces) {
            return \false;
        }
        if (\in_array('\stringable', $implementedInterfaces, \true)) {
            return \true;
        }
        foreach ($stringableInterfaces as $stringableInterface) {
            if (\in_array($stringableInterface, $implementedInterfaces, \true)) {
                return \true;
            }
        }
        return \false;
    ...
    private static function getInterfaces(Tokens $tokens, int $classKeywordIndex, int $classOpenBraceIndex): array
    {
        ...
        $interfaces = [];
        $interface = '';
        for ($index = $tokens->getNextMeaningfulToken($implementsIndex); $index < $classOpenBraceIndex; $index = $tokens->getNextMeaningfulToken($index)) {
            \assert(\is_int($index));
            if ($tokens[$index]->equals(',')) {
    ...
    private static function addStringableInterface(Tokens $tokens, int $classIndex): void
    {
        ...
        if ($tokens[$implementsIndex]->equals('{')) {
            ...
            $tokens->insertSlices([$prevIndex + 1 => [new Token([\T_WHITESPACE, ' ']), new Token([\T_IMPLEMENTS, 'implements']), new Token([\T_WHITESPACE, ' ']), new Token([\T_NS_SEPARATOR, '\\']), new Token([\T_STRING, \Stringable::class])]]);
            return;
        }
        $afterImplementsIndex = $tokens->getNextMeaningfulToken($implementsIndex);
        \assert(\is_int($afterImplementsIndex));
        $tokens->insertSlices([$afterImplementsIndex => [new Token([\T_NS_SEPARATOR, '\\']), new Token([\T_STRING, \Stringable::class]), new Token(','), new Token([\T_WHITESPACE, ' '])]]);
    }
}
