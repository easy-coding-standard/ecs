...
, ['prefix' => 'provides', 'suffix' => 'Data'])], null, 'Fixer could be risky if one is calling data provider by name as function.');
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('prefix', 'Prefix that replaces "test".'))->setAllowedTypes(['string'])->setDefault('provide')->getOption(), (new FixerOptionBuilder('suffix', 'Suffix to be present at the end.'))->setAllowedTypes(['string'])->setDefault('Cases')->getOption()]);
    }
    protected function applyPhpUnitClassFix(Tokens $tokens, int $startIndex, int $endIndex) : void
    {
        ...
        foreach ($dataProviders as $dataProviderAnalysis) {
            ...
            foreach ($dataProviderAnalysis->getUsageIndices() as [$usageIndex]) {
                ...
                $newContent = $tokens[$usageIndex]->isGivenKind(\T_DOC_COMMENT) ? Preg::replace(\sprintf('/(@dataProvider\\s+)%s/', $dataProviderAnalysis->getName()), \sprintf('$1%s', $dataProviderNewName), $tokens[$usageIndex]->getContent()) : \sprintf('%1$s%2$s%1$s', $tokens[$usageIndex]->getContent()[0], $dataProviderNewName);
