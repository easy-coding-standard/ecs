...
PHP;
    ...
    protected function applyPhpUnitClassFix(Tokens $tokens, int $startIndex, int $endIndex) : void
    {
        ...
        for ($index = $endIndex; $index >= $startIndex; --$index) {
            ...
            foreach (\array_reverse($docBlock->getAnnotations()) as $annotation) {
                ...
                $presentAttributes[$annotationName] = $presentAttributes[$annotationName] ?? self::isAttributeAlreadyPresent($fullyQualifiedNameAnalyzer, $tokens, $index, $tokensToInsert);
                if ([] === $tokensToInsert) {
                    continue;
                }
     ...
     */
    private static function isAttributeAlreadyPresent(FullyQualifiedNameAnalyzer $fullyQualifiedNameAnalyzer, Tokens $tokens, int $index, array $tokensToInsert) : bool
    {
    ...
    private static function fixDataProvider(Tokens $tokens, int $index, Annotation $annotation) : array
    {
        ...
        if (\strpos($matches[1], '::') !== \false) {
            ...
            return self::createAttributeTokens($tokens, $index, 'DataProviderExternal', ...\array_merge(self::toClassConstant($class), [new Token(','), new Token([\T_WHITESPACE, ' ']), self::fixNameAndCreateEscapedStringToken($method)]));
        }
