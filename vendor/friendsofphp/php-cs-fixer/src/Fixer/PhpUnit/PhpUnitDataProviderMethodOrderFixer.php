...
, ['placement' => 'before'])]);
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('placement', 'Where to place the data provider relative to the test where used.'))->setAllowedValues(['after', 'before'])->setDefault('after')->getOption()]);
    }
    ...
    private function getElements(Tokens $tokens, int $startIndex): array
    {
        ...
        return \Closure::bind(static function () use ($methodOrderFixer, $tokens, $startIndex) {
            return $methodOrderFixer->getElements($tokens, $startIndex);
        }, null, OrderedClassElementsFixer::class)();
    }
    /**
    ...
    {
        $methodOrderFixer = new OrderedClassElementsFixer();
        \Closure::bind(static function () use ($methodOrderFixer, $tokens, $startIndex, $endIndex, $elements) {
            return $methodOrderFixer->sortTokens($tokens, $startIndex, $endIndex, $elements);
        }, null, OrderedClassElementsFixer::class)();
    }
    /**
     * @param list<_ClassElement> $elements
    ...
    private function moveMethodElement(array $elements, string $nameKeep, string $nameToMove, bool $after): array
    {
        ...
        $elementToMove = $elements[$iToMove];
        // @phpstan-ignore offsetAccess.notFound
        unset($elements[$iToMove]);
        $c = $iKeep + ($after ? 1 : 0) + ($iToMove < $iKeep ? -1 : 0);
        return array_merge(\array_slice($elements, 0, $c), [$elementToMove], \array_slice($elements, $c));
    }
    /**
     * @return list<array{
     *   array{int, string},
    ...
    private function getDataProvidersWithUsagePairs(Tokens $tokens, int $startIndex, int $endIndex): array
    {
        ...
        foreach ($dataProviderAnalyzer->getDataProviders($tokens, $startIndex, $endIndex) as $dataProviderAnalysis) {
            ...
            foreach ($dataProviderAnalysis->getUsageIndices() as $usageIndex) {
                ...
                end($methodNameTokens);
                $usages[key($methodNameTokens)] = [array_key_last($methodNameTokens), end($methodNameTokens)->getContent(), $usageIndex[1]];
                reset($methodNameTokens);
            }
            \assert([] !== $usages);
            $dataProvidersWithUsagePairs[] = [[$dataProviderAnalysis->getNameIndex(), $dataProviderAnalysis->getName()], $usages];
        }
    ...
    private function getOrigUsageDataProviderOrderPairs(array $dataProvidersWithUsagePairs): array
    {
        ...
        uasort($origUsagesOrderPairs, static function (array $a, array $b): int {
            $cmp = $a[0][0] <=> $b[0][0];
            return 0 !== $cmp ? $cmp : $a[0][2] <=> $b[0][2];
        });
        $origUsageDataProviderOrderPairs = [];
        foreach (array_map(static function (array $v): array {
            return [$v[0][1], $v[1]];
        }, $origUsagesOrderPairs) as [$usageName, $providerName]) {
