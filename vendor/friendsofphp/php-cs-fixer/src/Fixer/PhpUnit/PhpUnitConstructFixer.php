...
final class FooTest extends \ECSPrefix202605\PHPUnit_Framework_TestCase
{
    public function testSomething()
    {
        $this->assertEquals(\false, $b);
        $this->assertSame(\true, $a);
...
, ['assertions' => ['assertSame', 'assertNotSame']])], null, 'Fixer could be risky if one is overriding PHPUnit\'s native methods.');
    ...
    protected function applyPhpUnitClassFix(Tokens $tokens, int $startIndex, int $endIndex): void
    {
        ...
        foreach ($this->configuration['assertions'] as $assertionMethod) {
            for ($index = $startIndex; $index < $endIndex; ++$index) {
                $index = \call_user_func_array(\in_array($assertionMethod, ['assertSame', 'assertEquals'], \true) ? [$this, 'fixAssertPositive'] : [$this, 'fixAssertNegative'], [$tokens, $index, $assertionMethod]);
                if (null === $index) {
                    break;
                }
            }
    ...
    {
        $assertMethods = ['assertEquals', 'assertNotEquals', 'assertNotSame', 'assertSame'];
        return new FixerConfigurationResolver([(new FixerOptionBuilder('assertions', 'List of assertion methods to fix.'))->setAllowedTypes(['string[]'])->setAllowedValues([new AllowedValueSubset($assertMethods)])->setDefault($assertMethods)->getOption()]);
    }
    private function fixAssertNegative(Tokens $tokens, int $index, string $method): ?int
    {
        return $this->fixAssert(['false' => 'assertNotFalse', 'null' => 'assertNotNull', 'true' => 'assertNotTrue'], $tokens, $index, $method);
    }
    private function fixAssertPositive(Tokens $tokens, int $index, string $method): ?int
    {
        return $this->fixAssert(['false' => 'assertFalse', 'null' => 'assertNull', 'true' => 'assertTrue'], $tokens, $index, $method);
    }
    /**
