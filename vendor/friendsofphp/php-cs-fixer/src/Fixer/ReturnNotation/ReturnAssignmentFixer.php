...
, ['skip_named_var_tags' => \true])]);
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('skip_named_var_tags', 'Whether to skip cases where named `@var` tags are used.'))->setDefault(\false)->setAllowedTypes(['bool'])->getOption()]);
    }
    /**
    ...
    private function fixFunction(Tokens $tokens, int $functionIndex, int $functionOpenIndex, int $functionCloseIndex): int
    {
        ...
        for ($i = \count($candidates) - 1; $i >= 0; --$i) {
            ...
            if ($tokens[$assignVarOperatorIndex]->equals('}')) {
                ...
                // test for `anonymous class`, `lambda` and `match`
                if (null === $startIndex) {
                    continue;
                }
            ...
            if ($this->configuration['skip_named_var_tags'] && $this->hasNamedVarTag($tokens, $assignVarIndex, $functionOpenIndex)) {
                continue;
            }
     ...
     * @return int >= 0 number of tokens inserted into the Tokens collection
     */
    private function simplifyReturnStatement(Tokens $tokens, int $assignVarIndex, int $assignVarOperatorIndex, int $returnIndex, int $returnVarEndIndex): int
    {
        $inserted = 0;
