...
final class TernaryToNullCoalescingFixer extends AbstractFixer
{
    ...
    {
        return new FixerDefinition('Use `null` coalescing operator `??` where possible.', [new CodeSample("<?php\n\$sample = isset(\$a) ? \$a : \$b;\n")]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    private function fixIsset(Tokens $tokens, int $index): void
    {
        ...
        if ('$this' === $issetCode) {
            return;
            // null coalescing operator does not with $this
        }
    ...
    {
        return $token->isGivenKind([\T_ARRAY_CAST, \T_BOOLEAN_AND, \T_BOOLEAN_OR, \T_BOOL_CAST, \T_COALESCE, \T_DEC, \T_DOUBLE_CAST, \T_INC, \T_INT_CAST, \T_IS_EQUAL, \T_IS_GREATER_OR_EQUAL, \T_IS_IDENTICAL, \T_IS_NOT_EQUAL, \T_IS_NOT_IDENTICAL, \T_IS_SMALLER_OR_EQUAL, \T_OBJECT_CAST, \T_POW, \T_SL, \T_SPACESHIP, \T_SR, \T_STRING_CAST, \T_UNSET_CAST]) || $token->equalsAny(['!', '%', '&', '*', '+', '-', '/', ':', '^', '|', '~', '.']);
    }
    /**
    ...
    private function hasChangingContent(Tokens $tokens): bool
    {
        foreach ($tokens as $token) {
            if ($token->isGivenKind([\T_DEC, \T_INC, \T_YIELD, \T_YIELD_FROM]) || $token->equals('(')) {
                return \true;
            }
        }
        return \false;
