...
final class LongToShorthandOperatorFixer extends AbstractShortOperatorFixer
{
     ...
     */
    private const OPERATORS = ['+' => [\T_PLUS_EQUAL, '+='], '-' => [\T_MINUS_EQUAL, '-='], '*' => [\T_MUL_EQUAL, '*='], '/' => [\T_DIV_EQUAL, '/='], '&' => [\T_AND_EQUAL, '&='], '.' => [\T_CONCAT_EQUAL, '.='], '%' => [\T_MOD_EQUAL, '%='], '|' => [\T_OR_EQUAL, '|='], '^' => [\T_XOR_EQUAL, '^=']];
    /**
    ...
    {
        return new FixerDefinition('Shorthand notation for operators should be used if possible.', [new CodeSample("<?php\n\$i = \$i + 10;\n")], null, 'Risky when applying for string offsets (e.g. `<?php $text = "foo"; $text[0] = $text[0] & "\x7F";`).');
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        return $tokens->isAnyTokenKindsFound(array_merge(array_keys(self::OPERATORS), [FCT::T_AMPERSAND_FOLLOWED_BY_VAR_OR_VARARG, FCT::T_AMPERSAND_NOT_FOLLOWED_BY_VAR_OR_VARARG]));
    }
    ...
    protected function isOperatorTokenCandidate(Tokens $tokens, int $index): bool
    {
        ...
        return \false;
        // unreachable, but keeps SCA happy
    }
    protected function getReplacementToken(Token $token): Token
    {
        \assert(isset(self::OPERATORS[$token->getContent()]));
        // for PHPStan
