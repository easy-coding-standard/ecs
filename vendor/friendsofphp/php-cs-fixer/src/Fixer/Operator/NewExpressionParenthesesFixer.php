...
final class NewExpressionParenthesesFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    {
        return new FixerDefinition('All `new` expressions with a further call must (not) be wrapped in parentheses.', [new VersionSpecificCodeSample("<?php\n\n(new Foo())->bar();\n", new VersionSpecification(80400)), new VersionSpecificCodeSample("<?php\n\n(new class {})->bar();\n", new VersionSpecification(80400)), new VersionSpecificCodeSample("<?php\n\nnew Foo()->bar();\n", new VersionSpecification(80400), ['use_parentheses' => \true]), new VersionSpecificCodeSample("<?php\n\nnew class {}->bar();\n", new VersionSpecification(80400), ['use_parentheses' => \true])]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        return \PHP_VERSION_ID >= 80400 && $tokens->isTokenKindFound(\T_NEW);
    }
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('use_parentheses', 'Whether `new` expressions with a further call should be wrapped in parentheses or not.'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption()]);
    }
