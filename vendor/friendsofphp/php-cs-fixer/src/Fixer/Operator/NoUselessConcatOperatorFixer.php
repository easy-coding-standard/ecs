...
final class NoUselessConcatOperatorFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    {
        return new FixerDefinition('There should not be useless concat operations.', [new CodeSample("<?php\n\$a = 'a'.'b';\n"), new CodeSample("<?php\n\$a = 'a'.\"b\";\n", ['juggle_simple_strings' => \true])]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('juggle_simple_strings', 'Allow for simple string quote juggling if it results in more concat-operations merges.'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption()]);
    }
    /**
    ...
    private function fixConcatOperation(Tokens $tokens, array $firstOperand, int $concatIndex, array $secondOperand): void
    {
        ...
        if (self::STR_DOUBLE_QUOTE === $firstOperand['type'] && self::STR_DOUBLE_QUOTE === $secondOperand['type'] || self::STR_SINGLE_QUOTE === $firstOperand['type'] && self::STR_SINGLE_QUOTE === $secondOperand['type']) {
        ...
        $operands = [[$firstOperand, $secondOperand], [$secondOperand, $firstOperand]];
    ...
    private function getConcatOperandType(Tokens $tokens, int $index, int $direction): ?array
    {
        if ($tokens[$index]->isGivenKind(\T_CONSTANT_ENCAPSED_STRING)) {
            ...
            if ('b' === $firstChar[0] || 'B' === $firstChar[0]) {
                return null;
                // we don't care about these, priorities are set to do deal with these cases
            }
            return ['start' => $index, 'end' => $index, 'type' => '"' === $firstChar[0] ? self::STR_DOUBLE_QUOTE : self::STR_SINGLE_QUOTE];
        }
        if ($tokens[$index]->equals('"')) {
            $end = $tokens->getTokenOfKindSibling($index, $direction, ['"']);
            return ['start' => 1 === $direction ? $index : $end, 'end' => 1 === $direction ? $end : $index, 'type' => self::STR_DOUBLE_QUOTE_VAR];
        }
        return null;
    }
    /**
     ...
     * @param _ConcatOperandType $secondOperand
     */
    private function mergeConstantEscapedStringOperands(Tokens $tokens, array $firstOperand, int $concatOperatorIndex, array $secondOperand): void
    {
        ...
        $tokens[$firstOperand['start']] = new Token([\T_CONSTANT_ENCAPSED_STRING, $quote . substr($firstOperandTokenContent, 1, -1) . substr($secondOperandTokenContent, 1, -1) . $quote]);
     ...
     * @param _ConcatOperandType $secondOperand
     */
    private function mergeConstantEscapedStringVarOperands(Tokens $tokens, array $firstOperand, int $concatOperatorIndex, array $secondOperand): void
    {
