...
final class BinaryOperatorSpacesFixer extends AbstractFixer implements ConfigurableFixerInterface
{
     ...
     */
    private const ALLOWED_VALUES = [self::ALIGN, self::ALIGN_BY_SCOPE, self::ALIGN_SINGLE_SPACE, self::ALIGN_SINGLE_SPACE_MINIMAL, self::ALIGN_SINGLE_SPACE_BY_SCOPE, self::ALIGN_SINGLE_SPACE_MINIMAL_BY_SCOPE, self::SINGLE_SPACE, self::NO_SPACE, self::AT_LEAST_SINGLE_SPACE, null];
    /**
...
PHP
, ['operators' => ['=' => self::ALIGN, 'xor' => null]]), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['operators' => ['+=' => self::ALIGN_SINGLE_SPACE]]), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['operators' => ['===' => self::ALIGN_SINGLE_SPACE_MINIMAL]]), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['operators' => ['|' => self::NO_SPACE]]), new CodeSample(<<<'PHP'
<?php

namespace ECSPrefix202605;

$array = ["foo" => 1, "baaaaaaaaaaar" => 11];

PHP
, ['operators' => ['=>' => self::SINGLE_SPACE]]), new CodeSample(<<<'PHP'
<?php

namespace ECSPrefix202605;

$array = ["foo" => 12, "baaaaaaaaaaar" => 13, "baz" => 1];

PHP
, ['operators' => ['=>' => self::ALIGN]]), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['operators' => ['=>' => self::ALIGN_BY_SCOPE]]), new CodeSample(<<<'PHP'
<?php
...
PHP
, ['operators' => ['=>' => self::ALIGN_SINGLE_SPACE_BY_SCOPE]]), new CodeSample(<<<'PHP'
<?php
...
, ['operators' => ['=>' => self::ALIGN_SINGLE_SPACE_MINIMAL_BY_SCOPE]])]);
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('default', 'Default fix strategy.'))->setDefault(self::SINGLE_SPACE)->setAllowedValues(self::ALLOWED_VALUES)->getOption(), (new FixerOptionBuilder('operators', 'Dictionary of `binary operator` => `fix strategy` values that differ from the default strategy. Supported are: ' . Utils::naturalLanguageJoinWithBackticks(self::SUPPORTED_OPERATORS) . '.'))->setAllowedTypes(['array<string, ?string>'])->setAllowedValues([static function (array $option): bool {
            foreach ($option as $operator => $value) {
                ...
                if (!\in_array($value, self::ALLOWED_VALUES, \true)) {
                    ...
                    }, self::ALLOWED_VALUES)), \is_object($value) ? \get_class($value) : (null === $value ? 'null' : \gettype($value) . '#' . $value)));
                }
            }
            return \true;
    ...
    private function injectAlignmentPlaceholdersDefault(Tokens $tokens, int $startAt, int $endAt, string $tokenContent): void
    {
        ...
        for ($index = $startAt; $index < $endAt; ++$index) {
            ...
            if (strtolower($content) === $tokenContent && $this->tokensAnalyzer->isBinaryOperator($index) && ('=' !== $content || \false === $this->isEqualPartOfDeclareStatement($tokens, $index)) && $newLineFoundSinceLastPlaceholder) {
    ...
    private function getSubstringWidth(string $haystack, string $needle): int
    {
        ...
        $substring = (string) substr($haystack, 0, $position);
