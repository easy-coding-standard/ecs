...
final class DeclareStrictTypesFixer extends AbstractFixer implements ConfigurableFixerInterface, WhitespacesAwareFixerInterface
{
    ...
    {
        return new FixerDefinition('Force a strict types mode in all files.', [new CodeSample("<?php\n"), new CodeSample("<?php\ndeclare(Strict_Types=0);\n", ['strategy' => 'enforce']), new CodeSample("<?php\ndeclare(Strict_Types=0);\n", ['strategy' => 'add_when_missing']), new CodeSample("<?php\ndeclare(strict_types=1);\n", ['strategy' => 'remove']), new CodeSample("<?php\ndeclare(strict_types=1, ticks=1);\n", ['strategy' => 'remove']), new CodeSample(
            // @TODO v4.0: remove me, "preserve_existing_declaration" option is deprecated
            "<?php\ndeclare(Strict_Types=0);\n",
            ['preserve_existing_declaration' => \false]
        ), new CodeSample(
            // @TODO v4.0: remove me, "preserve_existing_declaration" option is deprecated
            "<?php\ndeclare(Strict_Types=0);\n",
            ['preserve_existing_declaration' => \true]
        )], null, 'Enabling strict types will stop non strict code from working.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function createConfigurationDefinition() : FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('preserve_existing_declaration', 'Whether existing strict_types=? should be preserved and not overridden.'))->setAllowedTypes(['bool'])->setDefault(\false)->setNormalizer(static function (Options $options, bool $value) use($fixerName) : bool {
            if (\true === $value && 'enforce' !== $options['strategy']) {
                throw new InvalidFixerConfigurationException($fixerName, 'Cannot configure "strategy" in parallel to configuring "preserve_existing_declaration".');
            }
            return $value;
        })->setDeprecationMessage('Use `strategy` to configure behaviour.')->getOption(), (new FixerOptionBuilder('strategy', 'Whether existing strict_types=? should be enforced, removed (effectively turning strict mode off and using the engine into default type coercion mode), or added when missing.'))->setAllowedValues(['remove', 'add_when_missing', 'enforce'])->setDefault('enforce')->getOption()]);
    }
    /**
    ...
    protected function configurePostNormalisation() : void
    {
        ...
        if ('enforce' === $this->configuration['strategy'] && \true === $this->configuration['preserve_existing_declaration']) {
    ...
    private function getStrictTypesDeclaration(Tokens $tokens) : ?array
    {
        foreach ($tokens->findGivenKind(\T_DECLARE) as $index => $token) {
            ...
            return ['declare_index' => $index, 'open_parenthesis' => $openParenthesis, 'close_parenthesis' => $closeParenthesis, 'sequence' => $strictTypesSequence];
        }
        return null;
    }
    /**
    ...
    private function removeStrictTypesDeclaration(Tokens $tokens, array $declaration) : void
    {
        ...
        $sequenceIndexMap = \array_fill_keys($sequenceIndices, \true);
    ...
    private function insertSequence(int $openTagIndex, Tokens $tokens) : void
    {
        ...
        if (\strpos($content, ' ') === \false || \strpos($content, "\n") !== \false) {
            $tokens[$openTagIndex] = new Token([$tokens[$openTagIndex]->getId(), \trim($tokens[$openTagIndex]->getContent()) . ' ']);
        }
