...
final class ModernizeStrposFixer extends AbstractFixer implements ConfigurableFixerInterface
{
     ...
     * @var bool
     */
    private $modernizeStripos = \false;
...
PHP
, ['modernize_stripos' => \true])], null, 'Risky if `strpos`, `stripos`, `str_starts_with`, `str_contains` or `strtolower` functions are overridden.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        if (isset($this->configuration['modernize_stripos']) && \true === $this->configuration['modernize_stripos']) {
            $this->modernizeStripos = \true;
        }
    }
    ...
    private function wrapArgumentsWithStrToLower(Tokens $tokens, int $functionIndex) : void
    {
        ...
        \end($arguments);
        $isSecondArgumentTokenWhiteSpace = $tokens[\key($arguments)]->isGivenKind(\T_WHITESPACE);
        \reset($arguments);
        if ($isSecondArgumentTokenWhiteSpace) {
            \end($arguments);
            $secondArgumentIndexStart = $tokens->getNextMeaningfulToken(\key($arguments));
            \reset($arguments);
        } else {
            \end($arguments);
            $secondArgumentIndexStart = \key($arguments);
            \reset($arguments);
        }
        $secondArgumentIndexStart += 3 + ($shouldAddNamespace ? 1 : 0);
        \end($arguments);
        if (!isset($arguments[\key($arguments)])) {
            \reset($arguments);
            return;
        }
        \end($arguments);
        $secondArgumentIndexEnd = $arguments[\key($arguments)] + 6 + ($shouldAddNamespace ? 1 : 0) + ($isSecondArgumentTokenWhiteSpace ? 1 : 0);
        \reset($arguments);
