...
final class NoTrailingCommaInSinglelineFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    {
        return new FixerDefinition('If a list of values separated by a comma is contained on a single line, then the last item MUST NOT have a trailing comma.', [new CodeSample("<?php\nfoo(\$a,);\n\$foo = array(1,);\n[\$foo, \$bar,] = \$array;\nuse a\\{ClassA, ClassB,};\n"), new CodeSample("<?php\nfoo(\$a,);\n[\$foo, \$bar,] = \$array;\n", ['elements' => ['array_destructuring']])]);
    }
    public function isCandidate(Tokens $tokens): bool
    {
        return $tokens->isTokenKindFound(',') && $tokens->isAnyTokenKindsFound([')', CT::T_ARRAY_SQUARE_BRACE_CLOSE, CT::T_DESTRUCTURING_SQUARE_BRACE_CLOSE, CT::T_GROUP_IMPORT_BRACE_CLOSE]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('elements', 'Which elements to fix.'))->setAllowedTypes(['string[]'])->setAllowedValues([new AllowedValueSubset($elements)])->setDefault($elements)->getOption()]);
    }
    ...
    private function shouldBeCleared(Tokens $tokens, int $openIndex): bool
    {
        ...
        if ($tokens[$beforeOpen]->isGivenKind([\T_UNSET, \T_ISSET, \T_VARIABLE, \T_CLASS])) {
            return \in_array('arguments', $elements, \true);
        }
        if ($tokens[$beforeOpen]->isGivenKind(\T_STRING)) {
            return !AttributeAnalyzer::isAttribute($tokens, $beforeOpen) && \in_array('arguments', $elements, \true);
        }
        if ($tokens[$beforeOpen]->equalsAny([')', ']', [CT::T_DYNAMIC_VAR_BRACE_CLOSE], [CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE]])) {
            ...
            return (Tokens::BLOCK_TYPE_ARRAY_INDEX_CURLY_BRACE === $block['type'] || Tokens::BLOCK_TYPE_DYNAMIC_VAR_BRACE === $block['type'] || Tokens::BLOCK_TYPE_INDEX_SQUARE_BRACE === $block['type'] || Tokens::BLOCK_TYPE_PARENTHESIS_BRACE === $block['type']) && \in_array('arguments', $elements, \true);
        }
        return \false;
    }
}
