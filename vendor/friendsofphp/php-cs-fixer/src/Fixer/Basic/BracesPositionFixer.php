...
PHP
, ['allow_single_line_anonymous_functions' => \true])]);
    }
        ...
        foreach ($tokens as $index => $token) {
            ...
            if ($token->isGivenKind($classyTokens)) {
                ...
                if ($tokensAnalyzer->isAnonymousClass($index)) {
                    $allowSingleLineIfEmpty = \true === $this->configuration['allow_single_line_empty_anonymous_classes'];
                ...
                if ($tokensAnalyzer->isLambda($index)) {
                    $allowSingleLine = \true === $this->configuration['allow_single_line_anonymous_functions'];
            ...
            if ($allowSingleLine || $allowSingleLineIfEmpty || $index < $allowSingleLineUntil) {
                ...
                for ($indexInsideBraces = $openBraceIndex + 1; $indexInsideBraces < $closeBraceIndex; ++$indexInsideBraces) {
                    ...
                    if ($allowSingleLineIfEmpty && !$tokenInsideBraces->isWhitespace() && !$tokenInsideBraces->isComment() || $tokenInsideBraces->isWhitespace() && Preg::match('/\\R/', $tokenInsideBraces->getContent())) {
                        $addNewlinesInsideBraces = \true;
                        break;
                    }
                }
            ...
            if ($addNewlinesInsideBraces && !$this->isFollowedByNewLine($tokens, $openBraceIndex) && !$this->hasCommentOnSameLine($tokens, $openBraceIndex) && !$tokens[$tokens->getNextMeaningfulToken($openBraceIndex)]->isGivenKind(\T_CLOSE_TAG)) {
            ...
            if (self::NEXT_LINE_UNLESS_NEWLINE_AT_SIGNATURE_END === $this->configuration[$positionOption]) {
                ...
                if ($tokens[$previousTokenIndex]->equals(')')) {
                    ...
                    if ($tokens[$previousTokenIndex]->isWhitespace() && Preg::match('/\\R/', $tokens[$previousTokenIndex]->getContent())) {
                        $whitespace = ' ';
                    }
                }
            }
            ...
            if (' ' === $whitespace) {
                ...
                for ($indexAfterOpenBrace = $openBraceIndex + 1; $indexAfterOpenBrace < $closeBraceIndex; ++$indexAfterOpenBrace) {
                    ...
                    if ($tokens[$indexAfterOpenBrace]->isComment() && \strncmp($tokens[$indexAfterOpenBrace]->getContent(), '/*', \strlen('/*')) !== 0) {
            ...
            if (null !== $moveBraceToIndex) {
                ...
                if ($tokens[$openBraceIndex]->isWhitespace() && $tokens[$openBraceIndex + 1]->isWhitespace()) {
                    $tokens[$openBraceIndex] = new Token([\T_WHITESPACE, $tokens[$openBraceIndex]->getContent() . $tokens[$openBraceIndex + 1]->getContent()]);
                    $tokens->clearAt($openBraceIndex + 1);
                }
            ...
            if (!$addNewlinesInsideBraces || $tokens[$tokens->getPrevMeaningfulToken($closeBraceIndex)]->isGivenKind(\T_OPEN_TAG)) {
                continue;
            }
