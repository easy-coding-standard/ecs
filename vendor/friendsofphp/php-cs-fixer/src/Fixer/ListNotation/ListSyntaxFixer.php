...
final class ListSyntaxFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    {
        return new FixerDefinition('List (`array` destructuring) assignment should be declared using the configured syntax.', [new CodeSample("<?php\nlist(\$sample) = \$array;\n"), new CodeSample("<?php\n[\$sample] = \$array;\n", ['syntax' => 'long'])]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        return new FixerConfigurationResolver([(new FixerOptionBuilder('syntax', 'Whether to use the `long` or `short` syntax for array destructuring.'))->setAllowedValues(['long', 'short'])->setDefault('short')->getOption()]);
    }
    private function fixToLongSyntax(Tokens $tokens, int $index): void
    {
        $closeIndex = $tokens->getNextTokenOfKind($index, [[CT::T_DESTRUCTURING_SQUARE_BRACE_CLOSE], '[']);
