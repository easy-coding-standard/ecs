...
final class NativeTypeDeclarationCasingFixer extends AbstractFixer
{
    ...
    {
        parent::__construct();
        $this->types = ['array' => \true, 'bool' => \true, 'callable' => \true, 'float' => \true, 'int' => \true, 'iterable' => \true, 'object' => \true, 'parent' => \true, 'self' => \true, 'static' => \true, 'string' => \true, 'void' => \true];
        if (\PHP_VERSION_ID >= 80000) {
            $this->types['false'] = \true;
            $this->types['mixed'] = \true;
            $this->types['null'] = \true;
        }
        if (\PHP_VERSION_ID >= 80100) {
            $this->types['never'] = \true;
        }
    ...
    {
        return new FixerDefinition('Native type declarations should be used in the correct case.', [new CodeSample("<?php\nclass Bar {\n    public function Foo(CALLABLE \$bar): INT\n    {\n        return 1;\n    }\n}\n"), new VersionSpecificCodeSample("<?php\nclass Foo\n{\n    const INT BAR = 1;\n}\n", new VersionSpecification(80300))]);
    }
    public function isCandidate(Tokens $tokens): bool
    {
        $classyFound = $tokens->isAnyTokenKindsFound(Token::getClassyTokenKinds());
        return $tokens->isAnyTokenKindsFound([\T_FUNCTION, \T_FN]) || $classyFound && $tokens->isTokenKindFound(\T_STRING) || \PHP_VERSION_ID >= 80300 && $tokens->isTokenKindFound(\T_CONST) && $classyFound;
    }
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        foreach ($tokens as $index => $token) {
            ...
            if (!$tokens[$prevIndex]->isGivenKind([\T_CONST, CT::T_NULLABLE_TYPE, CT::T_TYPE_ALTERNATION, CT::T_TYPE_COLON]) && !$tokens[$nextIndex]->isGivenKind([\T_VARIABLE, CT::T_TYPE_ALTERNATION])) {
                continue;
            }
