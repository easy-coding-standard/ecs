...
final class OrderedAttributesFixer extends AbstractFixer implements ConfigurableFixerInterface
{
    ...
    private const SUPPORTED_SORT_ALGORITHMS = [self::ORDER_ALPHA, self::ORDER_CUSTOM];
...
\class_alias('ECSPrefix202605\Sample1', 'Sample1', \false);
#[Foo, Bar(3), Qux(new Bar(5)), Corge(a: 'test')]
class Sample2
{
}
\class_alias('ECSPrefix202605\Sample2', 'Sample2', \false);
...
use ECSPrefix202605\A\B\Foo;
use ECSPrefix202605\A\B\Bar as BarAlias;
use ECSPrefix202605\A\B as AB;
#[Foo]
#[BarAlias(3)]
#[AB\Qux(new Bar(5))]
#[\ECSPrefix202605\A\B\Corge(a: 'test')]
class Sample1
{
}
\class_alias('ECSPrefix202605\Sample1', 'Sample1', \false);
...
EOL
, new VersionSpecification(80000), ['sort_algorithm' => self::ORDER_CUSTOM, 'order' => ['ECSPrefix202605\A\B\Qux', 'ECSPrefix202605\A\B\Bar', 'ECSPrefix202605\A\B\Corge']])]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function createConfigurationDefinition(): FixerConfigurationResolverInterface
    {
        ...
        return new FixerConfigurationResolver([(new FixerOptionBuilder('sort_algorithm', 'How the attributes should be sorted.'))->setAllowedValues(self::SUPPORTED_SORT_ALGORITHMS)->setDefault(self::ORDER_ALPHA)->setNormalizer(static function (Options $options, string $value) use ($fixerName): string {
            if (self::ORDER_CUSTOM === $value && [] === $options['order']) {
                throw new InvalidFixerConfigurationException($fixerName, 'The custom order strategy requires providing `order` option with a list of attributes\'s FQNs.');
            }
            return $value;
        })->getOption(), (new FixerOptionBuilder('order', 'A list of FQCNs of attributes defining the desired order used when custom sorting algorithm is configured.'))->setAllowedTypes(['string[]'])->setDefault([])->setNormalizer(static function (Options $options, array $value) use ($fixerName): array {
            ...
            return array_flip(array_values(array_map(static function (string $attribute): string {
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens): void
    {
        ...
        while (null !== $index = $tokens->getNextTokenOfKind($index, [[\T_ATTRIBUTE]])) {
            ...
            $elements = array_map(function (AttributeAnalysis $attributeAnalysis) use ($fullyQualifiedNameAnalyzer, $tokens): array {
                return ['name' => $this->sortAttributes($fullyQualifiedNameAnalyzer, $tokens, $attributeAnalysis->getStartIndex(), $attributeAnalysis->getAttributes()), 'start' => $attributeAnalysis->getStartIndex(), 'end' => $attributeAnalysis->getEndIndex()];
    ...
    private function sortElements(array $elements): array
    {
        usort($elements, function (array $a, array $b): int {
            ...
            return ($this->configuration['order'][$a['name']] ?? \PHP_INT_MAX) <=> ($this->configuration['order'][$b['name']] ?? \PHP_INT_MAX);
        });
        return $elements;
    }
    /**
