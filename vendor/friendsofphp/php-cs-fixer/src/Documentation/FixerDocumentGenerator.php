...
RST;
        ...
        $tags = \PhpCsFixer\Documentation\DocumentationTagGenerator::analyseRule($fixer);
        $warnings = \array_map(static function (\PhpCsFixer\Documentation\DocumentationTag $tag) : string {
            $titleLine = \str_repeat('~', \strlen($tag->title));
            return \sprintf("\n%s\n%s\n\n%s", $tag->title, $titleLine, null === $tag->description ? '' : \PhpCsFixer\Documentation\RstUtils::toRst($tag->description, 0));
        }, $tags);
        if ([] !== $warnings) {
            ...
            foreach ($configurationDefinition->getOptions() as $option) {
                ...
                if (null === $allowed) {
                    ...
                    $allowed = \array_map(static function ($value) : string {
                        return $value instanceof AllowedValueSubset ? 'a subset of ``' . Utils::toString($value->getAllowedValues()) . '``' : '``' . Utils::toString($value) . '``';
                    }, $allowed);
                }
                if (null !== $allowed) {
                ...
                if ($option->hasDefault()) {
                    ...
                    $optionInFutureMode = null;
                    foreach ($fixerInFutureMode->getConfigurationDefinition()->getOptions() as $opt) {
                        if ($option->getName() === $opt->getName()) {
                            $optionInFutureMode = $opt;
                            break;
                        }
                    }
                    \assert(null !== $optionInFutureMode);
                    // if rule exist in v3, shall exist in future mode too, as it does not remove options
            ...
            foreach ($samples as $index => $sample) {
                ...
                if ($fixer instanceof ConfigurableFixerInterface) {
                    if (null === $sample->getConfiguration()) {
                        ...
                        $doc .= \sprintf("\n\nWith configuration: ``%s``.", Utils::toString($sample->getConfiguration()));
                    }
                }
...
RST;
        ...
        $fileName = (string) \substr($fileName, (int) \strrpos($fileName, '/src/Fixer/') + 1);
...
RST;
    ...
    public static function getSetsOfRule(string $ruleName) : array
    {
        ...
        if (null === $ruleSetCache) {
            $definitionNames = \array_keys(\array_filter(RuleSets::getSetDefinitions(), static function (RuleSetDefinitionInterface $definition) : bool {
                return !$definition instanceof AutomaticRuleSetDefinitionInterface;
            }));
            $ruleSetCache = \array_combine($definitionNames, \array_map(static function (string $name) : RuleSet {
                return new RuleSet([$name => \true]);
            }, $definitionNames));
        }
        $ruleSetConfigs = [];
...
RST;
        ...
        foreach ($fixers as $fixer) {
            ...
            $tags = \array_map(static function (\PhpCsFixer\Documentation\DocumentationTag $tag) : string {
                return $tag->type;
            }, \PhpCsFixer\Documentation\DocumentationTagGenerator::analyseRule($fixer));
            $attributes = 0 === \count($tags) ? '' : ' *(' . \implode(', ', $tags) . ')*';
...
RST;
    ...
    {
        $object = Future::runWithEnforcedFutureMode(static function () use($fixer) {
            return (new \ReflectionObject($fixer))->newInstance();
        });
