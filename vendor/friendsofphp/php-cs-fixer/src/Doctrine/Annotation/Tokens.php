...
final class Tokens extends \SplFixedArray
{
    ...
    public static function createFromDocComment(PhpToken $input, array $ignoredTags = []) : self
    {
        ...
        while (\false !== ($nextAtPosition = \strpos($content, '@', $currentPosition))) {
            ...
            while (null !== ($token = $lexer->peek())) {
                if (0 === $index && !$token->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_AT)) {
                    break;
                }
                if (1 === $index) {
                    if (!$token->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_IDENTIFIER) || \in_array($token->getContent(), $ignoredTags, \true)) {
                        break;
                    }
                    $nbScannedTokensToUse = 2;
                }
                if ($index >= 2 && 0 === $nbScopes && !$token->isType([\PhpCsFixer\Doctrine\Annotation\DocLexer::T_NONE, \PhpCsFixer\Doctrine\Annotation\DocLexer::T_OPEN_PARENTHESIS])) {
                    break;
                }
                $scannedTokens[] = $token;
                if ($token->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_OPEN_PARENTHESIS)) {
                    ++$nbScopes;
                } elseif ($token->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_CLOSE_PARENTHESIS)) {
                    if (0 === --$nbScopes) {
    ...
    public function getAnnotationEnd(int $index) : ?int
    {
        ...
        if (isset($this[$index + 2])) {
            if ($this[$index + 2]->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_OPEN_PARENTHESIS)) {
                $currentIndex = $index + 2;
            } elseif (isset($this[$index + 3]) && $this[$index + 2]->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_NONE) && $this[$index + 3]->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_OPEN_PARENTHESIS) && Preg::match('/^(\\R\\s*\\*\\s*)*\\s*$/', $this[$index + 2]->getContent())) {
        ...
        if (null !== $currentIndex) {
            ...
            for ($max = \count($this); $currentIndex < $max; ++$currentIndex) {
                if ($this[$currentIndex]->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_OPEN_PARENTHESIS)) {
                    ++$level;
                } elseif ($this[$currentIndex]->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_CLOSE_PARENTHESIS)) {
                    --$level;
                }
                if (0 === $level) {
                    return $currentIndex;
    ...
    public function offsetSet($index, $token) : void
    {
        if (!$token instanceof \PhpCsFixer\Doctrine\Annotation\Token) {
            throw new \InvalidArgumentException(\sprintf('Token must be an instance of %s, "%s" given.', \PhpCsFixer\Doctrine\Annotation\Token::class, \get_debug_type($token)));
        }
    ...
    private function getMeaningfulTokenSibling(int $index, int $direction) : ?int
    {
        while (\true) {
            ...
            if (!$this[$index]->isType(\PhpCsFixer\Doctrine\Annotation\DocLexer::T_NONE)) {
                return $index;
            }
        }
        return null;
