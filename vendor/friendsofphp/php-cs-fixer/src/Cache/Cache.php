...
final class Cache implements \PhpCsFixer\Cache\CacheInterface
{
    ...
    {
        try {
            return \json_encode(['php' => $this->getSignature()->getPhpVersion(), 'version' => $this->getSignature()->getFixerVersion(), 'indent' => $this->getSignature()->getIndent(), 'lineEnding' => $this->getSignature()->getLineEnding(), 'rules' => $this->getSignature()->getRules(), 'ruleCustomisationPolicyVersion' => $this->getSignature()->getRuleCustomisationPolicyVersion(), 'hashes' => $this->hashes], 0);
        } catch (\JsonException $e) {
            throw new \UnexpectedValueException(\sprintf('Cannot encode cache signature to JSON, error: "%s". If you have non-UTF8 chars in your signature, like in license for `header_comment`, consider enabling `ext-mbstring` or install `symfony/polyfill-mbstring`.', $e->getMessage()));
        }
    }
    /**
    ...
    {
        try {
            $data = \json_decode($json, \true, 512, 0);
        } catch (\JsonException $e) {
            throw new \InvalidArgumentException(\sprintf('Value needs to be a valid JSON string, got "%s", error: "%s".', $json, $e->getMessage()));
        }
        $requiredKeys = [
            'php',
            'version',
        ...
        if (\count($missingKeys) > 0) {
            throw new \InvalidArgumentException(\sprintf('JSON data is missing keys %s', Utils::naturalLanguageJoin(\array_keys($missingKeys))));
        }
        $signature = new \PhpCsFixer\Cache\Signature($data['php'], $data['version'], $data['indent'], $data['lineEnding'], $data['rules'], $data['ruleCustomisationPolicyVersion'] ?? NullRuleCustomisationPolicy::VERSION_FOR_CACHE);
        ...
        $cache->hashes = \array_map(static function ($v) : string {
