...
final class FileHandler implements \PhpCsFixer\Cache\FileHandlerInterface
{
    ...
    public function write(\PhpCsFixer\Cache\CacheInterface $cache) : void
    {
        ...
        $this->fileMTime = \time();
        // we could take the fresh `mtime` of file that we just modified with `$this->getFileCurrentMTime()`, but `time()` should be good enough here and reduce IO operation
    }
    private function getFileCurrentMTime() : int
    {
        \clearstatcache(\true, $this->fileInfo->getPathname());
    ...
    private function readFromHandle(\SplFileObject $fileObject) : ?\PhpCsFixer\Cache\CacheInterface
    {
        try {
            ...
            return \PhpCsFixer\Cache\Cache::fromJson($content);
    ...
    private function ensureFileIsWriteable() : void
    {
        ...
        if ($this->fileInfo->isDir()) {
            throw new IOException(\sprintf('Cannot write cache file "%s" as the location exists as directory.', $this->fileInfo->getRealPath()), 0, null, $this->fileInfo->getPathname());
        }
        if ($this->fileInfo->isFile() && !$this->fileInfo->isWritable()) {
            throw new IOException(\sprintf('Cannot write to file "%s" as it is not writable.', $this->fileInfo->getRealPath()), 0, null, $this->fileInfo->getPathname());
        }
    ...
    private function createFile(string $file) : void
    {
        ...
        if (!@\is_dir($dir)) {
            throw new IOException(\sprintf('Directory of cache file "%s" does not exists and couldn\'t be created.', $file), 0, null, $file);
        }
        @\touch($file);
        @\chmod($file, 0666);
    }
