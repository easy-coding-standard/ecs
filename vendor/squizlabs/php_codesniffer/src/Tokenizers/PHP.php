...
class PHP extends \PHP_CodeSniffer\Tokenizers\Tokenizer
{
     ...
     */
    protected const T_STRING_CONTEXTS = [\T_OBJECT_OPERATOR => \true, \T_NULLSAFE_OBJECT_OPERATOR => \true, \T_FUNCTION => \true, \T_CLASS => \true, \T_INTERFACE => \true, \T_TRAIT => \true, \T_ENUM => \true, \T_ENUM_CASE => \true, \T_EXTENDS => \true, \T_IMPLEMENTS => \true, \T_ATTRIBUTE => \true, \T_NEW => \true, \T_CONST => \true, \T_NS_SEPARATOR => \true, \T_USE => \true, \T_NAMESPACE => \true, \T_PAAMAYIM_NEKUDOTAYIM => \true, \T_GOTO => \true];
    /**
    ...
    protected function tokenize(string $code)
    {
        ...
        for ($stackPtr = 0; $stackPtr < $numTokens; $stackPtr++) {
            ...
            if ($newStackPtr > 0 && isset(Tokens::EMPTY_TOKENS[$finalTokens[$newStackPtr - 1]['code']]) === \false) {
            ...
            */
            if ($tokenIsArray === \true && isset(Tokens::CONTEXT_SENSITIVE_KEYWORDS[$token[0]]) === \true && (isset(static::T_STRING_CONTEXTS[$finalTokens[$lastNotEmptyToken]['code']]) === \true || $finalTokens[$lastNotEmptyToken]['content'] === '&' || $insideConstDeclaration === \true)) {
                if (isset(static::T_STRING_CONTEXTS[$finalTokens[$lastNotEmptyToken]['code']]) === \true) {
                    ...
                    if ($finalTokens[$lastNotEmptyToken]['code'] === \T_NEW && strtolower($token[1]) === 'readonly') {
                        for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                            if (is_array($tokens[$i]) === \false || isset(Tokens::EMPTY_TOKENS[$tokens[$i][0]]) === \false) {
                                break;
                            }
                        }
                    ...
                    if ($token[0] === \T_NAMESPACE) {
                        for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                            ...
                            if (isset(Tokens::EMPTY_TOKENS[$tokens[$i][0]]) === \true) {
                                continue;
                            }
                ...
                if (isset(static::T_STRING_CONTEXTS[$finalTokens[$lastNotEmptyToken]['code']]) === \true && $finalTokens[$lastNotEmptyToken]['code'] === \T_CONST || $insideConstDeclaration === \true) {
                    $preserveKeyword = \true;
                    // Find the next non-empty token.
                    for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                        if (is_array($tokens[$i]) === \true && isset(Tokens::EMPTY_TOKENS[$tokens[$i][0]]) === \true) {
                            continue;
                        }
                        break;
                    }
                ...
                if ($finalTokens[$lastNotEmptyToken]['content'] === '&') {
                    ...
                    for ($i = $lastNotEmptyToken - 1; $i >= 0; $i--) {
                        if (isset(Tokens::EMPTY_TOKENS[$finalTokens[$i]['code']]) === \true) {
                            continue;
                        }
                ...
                if ($preserveKeyword === \false) {
                    ...
                    $finalTokens[$newStackPtr] = ['code' => \T_STRING, 'type' => 'T_STRING', 'content' => $token[1]];
                    $newStackPtr++;
                    continue;
                }
            }
            ...
            */
            if ($tokenIsArray === \true && $token[0] === \T_STATIC && $finalTokens[$lastNotEmptyToken]['code'] !== \T_NEW) {
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    if (is_array($tokens[$i]) === \true && isset(Tokens::EMPTY_TOKENS[$tokens[$i][0]]) === \true) {
                        continue;
                    }
            ...
            */
            if (\PHP_VERSION_ID < 70400 && $tokenIsArray === \true && ($stackPtr === $numTokens - 1 && $token[0] === \T_INLINE_HTML && stripos($token[1], '<?php') === 0 || $stackPtr === $numTokens - 2 && $token[0] === \T_OPEN_TAG && $token[1] === '<?' && is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_STRING && strtolower($tokens[$stackPtr + 1][1]) === 'php')) {
                if ($token[0] === \T_INLINE_HTML) {
                    $finalTokens[$newStackPtr] = ['code' => \T_OPEN_TAG, 'type' => 'T_OPEN_TAG', 'content' => $token[1]];
                } else {
                    $finalTokens[$newStackPtr] = ['code' => \T_OPEN_TAG, 'type' => 'T_OPEN_TAG', 'content' => $token[1] . $tokens[$stackPtr + 1][1]];
                    $stackPtr++;
                }
                $newStackPtr++;
                continue;
            ...
            */
            if ($tokenIsArray === \true && $token[0] === \T_OPEN_TAG && stripos($token[1], '<?php') === 0) {
                $openTagAndWhiteSpace = str_split($token[1], 5);
                $finalTokens[$newStackPtr] = ['code' => \T_OPEN_TAG, 'type' => 'T_OPEN_TAG', 'content' => $openTagAndWhiteSpace[0]];
                $newStackPtr++;
                if (isset($openTagAndWhiteSpace[1]) === \true) {
                    ...
                    if (isset($tokens[$stackPtr + 1]) === \true && $openTagAndWhiteSpace[1] === ' ' && is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_WHITESPACE) {
                        ...
                        $finalTokens[$newStackPtr] = ['code' => \T_WHITESPACE, 'type' => 'T_WHITESPACE', 'content' => $openTagAndWhiteSpace[1]];
                        if (\PHP_CODESNIFFER_VERBOSITY > 1) {
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_DOC_COMMENT || $token[0] === \T_COMMENT && strpos($token[1], '/**') === 0 && $token[1] !== '/**/')) {
            ...
            */
            if (\PHP_VERSION_ID < 80100 && $tokenIsArray === \true && $token[1] === '0' && (isset($tokens[$stackPtr + 1]) === \true && is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_STRING && isset($tokens[$stackPtr + 1][1][0], $tokens[$stackPtr + 1][1][1]) === \true && strtolower($tokens[$stackPtr + 1][1][0]) === 'o' && $tokens[$stackPtr + 1][1][1] !== '_') && preg_match('`^(o[0-7]+(?:_[0-7]+)?)([0-9_]*)$`i', $tokens[$stackPtr + 1][1], $matches) === 1) {
                $finalTokens[$newStackPtr] = ['code' => \T_LNUMBER, 'type' => 'T_LNUMBER', 'content' => $token[1] .= $matches[1]];
                $newStackPtr++;
                if (isset($matches[2]) === \true && $matches[2] !== '') {
                    $type = 'T_LNUMBER';
                    if ($matches[2][0] === '_') {
                        $type = 'T_STRING';
                    }
                    $finalTokens[$newStackPtr] = ['code' => constant($type), 'type' => $type, 'content' => $matches[2]];
                    $newStackPtr++;
                }
                $stackPtr++;
                continue;
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_AMPERSAND_FOLLOWED_BY_VAR_OR_VARARG || $token[0] === \T_AMPERSAND_NOT_FOLLOWED_BY_VAR_OR_VARARG)) {
                $finalTokens[$newStackPtr] = ['code' => \T_BITWISE_AND, 'type' => 'T_BITWISE_AND', 'content' => $token[1]];
                $newStackPtr++;
                continue;
            }
            /*
            ...
            */
            if ($tokenIsArray === \true && $token[0] === \T_STRING && strtolower($token[1]) === 'enum') {
                ...
                if (isset($tokens[$i]) === \true && is_array($tokens[$i]) === \true && $tokens[$i][0] === \T_STRING) {
            ...
            */
            if ($tokenIsArray === \true && $token[0] === \T_CASE && isset(static::T_STRING_CONTEXTS[$finalTokens[$lastNotEmptyToken]['code']]) === \false) {
                $isEnumCase = \false;
                $scope = 1;
            ...
            */
            if ($tokenIsArray === \true && in_array($token[0], [\T_PUBLIC, \T_PROTECTED, \T_PRIVATE], \true) === \true && $stackPtr + 3 < $numTokens && $tokens[$stackPtr + 1] === '(' && is_array($tokens[$stackPtr + 2]) === \true && $tokens[$stackPtr + 2][0] === \T_STRING && strtolower($tokens[$stackPtr + 2][1]) === 'set' && $tokens[$stackPtr + 3] === ')') {
                $newToken = [];
                if ($token[0] === \T_PUBLIC) {
                    $oldCode = 'T_PUBLIC';
             ...
             */
            if (\PHP_VERSION_ID >= 80000 && $tokenIsArray === \true && $token[0] === \T_NAME_FULLY_QUALIFIED) {
            ...
            */
            if (\PHP_VERSION_ID < 80000 && $tokenIsArray === \true && ($token[0] === \T_STRING || $token[0] === \T_NAMESPACE || $token[0] === \T_NS_SEPARATOR && isset($tokens[$stackPtr + 1]) === \true && is_array($tokens[$stackPtr + 1]) === \true && isset(Tokens::EMPTY_TOKENS[$tokens[$stackPtr + 1][0]]) === \false && preg_match(self::PHP_LABEL_REGEX, $tokens[$stackPtr + 1][1]) === 1)) {
                $nameStart = $stackPtr;
                $i = $stackPtr;
                $newToken = [];
                $newToken['content'] = $token[1];
                switch ($token[0]) {
                    ...
                    case \T_NS_SEPARATOR:
                        ...
                        if (is_array($tokens[$i - 1]) === \true && isset(Tokens::EMPTY_TOKENS[$tokens[$i - 1][0]]) === \false && preg_match(self::PHP_LABEL_REGEX, $tokens[$i - 1][1]) === 1) {
                ...
                while (isset($tokens[$i + 1], $tokens[$i + 2]) === \true && is_array($tokens[$i + 1]) === \true && $tokens[$i + 1][0] === \T_NS_SEPARATOR && is_array($tokens[$i + 2]) === \true && isset(Tokens::EMPTY_TOKENS[$tokens[$i + 2][0]]) === \false && preg_match(self::PHP_LABEL_REGEX, $tokens[$i + 2][1]) === 1) {
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_STRING || preg_match(self::PHP_LABEL_REGEX, $token[1]) === 1)) {
                // Get the next non-empty token.
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    if (is_array($tokens[$i]) === \false || isset(Tokens::EMPTY_TOKENS[$tokens[$i][0]]) === \false) {
                        break;
                ...
                if (isset($tokens[$i]) === \true && is_array($tokens[$i]) === \false && $tokens[$i] === ':') {
                    ...
                    for ($j = $stackPtr - 1; $j > 0; $j--) {
                        if (is_array($tokens[$j]) === \false || isset(Tokens::EMPTY_TOKENS[$tokens[$j][0]]) === \false) {
                            break;
                        }
                    }
            ...
            */
            if ($tokenIsArray === \true && strtolower($token[1]) === 'readonly' && (isset(static::T_STRING_CONTEXTS[$finalTokens[$lastNotEmptyToken]['code']]) === \false || $finalTokens[$lastNotEmptyToken]['code'] === \T_NEW)) {
                ...
                if (isset($tokens[$i]) === \false || $tokens[$i] !== '(') {
                    ...
                    for (++$i; $i < $numTokens; $i++) {
                        ...
                        if (isset(Tokens::EMPTY_TOKENS[$tokenType]) === \true) {
                            continue;
                        }
                        if ($tokenType === '|') {
                            ++$foundDNFPipe;
                ...
                if ($isReadonlyKeyword === \true) {
                    $finalTokens[$newStackPtr] = ['code' => \T_READONLY, 'type' => 'T_READONLY', 'content' => $token[1]];
                    $newStackPtr++;
            ...
            */
            if (\PHP_VERSION_ID < 80300 && $tokenIsArray === \true && $token[0] === \T_STRING && strtolower($token[1]) === 'from' && $finalTokens[$lastNotEmptyToken]['code'] === \T_YIELD) {
                /*
                ...
                */
                ...
                $finalTokens[$newStackPtr] = ['code' => \T_YIELD_FROM, 'type' => 'T_YIELD_FROM', 'content' => $token[1]];
                $newStackPtr++;
                if (\PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                continue;
            } elseif ($tokenIsArray === \true && $token[0] === \T_YIELD_FROM && strpos($token[1], $this->eolChar) !== \false && preg_match('`^yield\s+from$`i', $token[1]) === 1) {
                /*
                ...
                */
                ...
                $finalTokens[$newStackPtr] = ['code' => \T_YIELD_FROM, 'type' => 'T_YIELD_FROM', 'content' => substr($token[1], -4)];
                $newStackPtr++;
                if (\PHP_CODESNIFFER_VERBOSITY > 1) {
                    StatusWriter::write("* token {$stackPtr} split into 'yield', one or more whitespace tokens and 'from'", 2);
                }
                continue;
            } elseif (\PHP_VERSION_ID >= 80300 && $tokenIsArray === \true && $token[0] === \T_YIELD_FROM && preg_match('`^yield[ \t]+from$`i', $token[1]) !== 1 && stripos($token[1], 'yield') === 0) {
                /*
                ...
                */
                ...
                $yieldFromSubtokens[] = [0 => \T_YIELD_FROM, 1 => substr($token[1], -4)];
            ...
            */
            if ($tokenIsArray === \true && $token[0] === \T_COALESCE && isset($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === '=') {
                $newToken = [];
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_STRING && strtolower($token[1]) === 'match' || $token[0] === \T_MATCH)) {
                ...
                for ($x = $stackPtr + 1; $x < $numTokens; $x++) {
                    if (isset($tokens[$x][0], Tokens::EMPTY_TOKENS[$tokens[$x][0]]) === \true) {
                        continue;
                    }
                    if ($tokens[$x] !== '(') {
            ...
            /*
                ...
                breaking.
            */
            if ($tokenIsArray === \true && $token[0] === \T_DEFAULT && isset(static::T_STRING_CONTEXTS[$finalTokens[$lastNotEmptyToken]['code']]) === \false) {
                for ($x = $stackPtr + 1; $x < $numTokens; $x++) {
                    if ($tokens[$x] === ',') {
                        // Skip over potential trailing comma (supported in PHP).
                        continue;
                    }
                    if (is_array($tokens[$x]) === \false || isset(Tokens::EMPTY_TOKENS[$tokens[$x][0]]) === \false) {
            ...
            */
            if ($tokenIsArray === \false && $token[0] === '?') {
                ...
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    ...
                    if (isset(Tokens::NAME_TOKENS[$tokenType]) === \true || $tokenType === \T_ARRAY || $tokenType === \T_NAMESPACE || $tokenType === \T_NS_SEPARATOR) {
                    ...
                    if (isset($lastRelevantNonEmpty) === \false && $tokenType !== \T_CALLABLE) {
                        ...
                        $isInlineThen = \true;
                    } elseif (isset($lastRelevantNonEmpty) === \true) {
                        if ($lastRelevantNonEmpty === \T_ARRAY && $tokenType === '(') {
                            // Array declaration in ternary then.
                            $isInlineThen = \true;
                        } elseif (isset(Tokens::NAME_TOKENS[$lastRelevantNonEmpty]) === \true && ($tokenType === \T_DOUBLE_COLON || $tokenType === '(' || $tokenType === ':')) {
                ...
                for ($i = $stackPtr - 1; $i >= 0; $i--) {
                    ...
                    if ($prevNonEmpty === null && isset(Tokens::EMPTY_TOKENS[$tokenType]) === \false) {
                    ...
                    if ($tokenType === \T_FUNCTION || $tokenType === \T_FN || isset(Tokens::METHOD_MODIFIERS[$tokenType]) === \true || isset(Tokens::SCOPE_MODIFIERS[$tokenType]) === \true || $tokenType === \T_VAR || $tokenType === \T_READONLY) {
                        if (\PHP_CODESNIFFER_VERBOSITY > 1) {
                            StatusWriter::write("* token {$stackPtr} changed from ? to T_NULLABLE", 2);
                        }
                        $newToken['code'] = \T_NULLABLE;
                        $newToken['type'] = 'T_NULLABLE';
                        break;
                    } elseif (in_array($tokenType, [\T_DOUBLE_ARROW, \T_OPEN_TAG, \T_OPEN_TAG_WITH_ECHO, '=', '{', ';'], \true) === \true) {
                        if (\PHP_CODESNIFFER_VERBOSITY > 1) {
                            StatusWriter::write("* token {$stackPtr} changed from ? to T_INLINE_THEN", 2);
                        }
                        $newToken['code'] = \T_INLINE_THEN;
            ...
            */
            if ($stackPtr > 1 && (is_array($tokens[$stackPtr - 1]) === \true && $tokens[$stackPtr - 1][0] === \T_PAAMAYIM_NEKUDOTAYIM) && $tokenIsArray === \true && $token[0] !== \T_STRING && $token[0] !== \T_VARIABLE && $token[0] !== \T_DOLLAR && isset(Tokens::EMPTY_TOKENS[$token[0]]) === \false) {
                $newToken = [];
                $newToken['code'] = \T_STRING;
                $newToken['type'] = 'T_STRING';
                $newToken['content'] = $token[1];
            ...
            */
            if ($tokenIsArray === \true && $token[0] === \T_STRING && strtolower($token[1]) === 'fn' && isset(static::T_STRING_CONTEXTS[$finalTokens[$lastNotEmptyToken]['code']]) === \false && $finalTokens[$lastNotEmptyToken]['content'] !== '&') {
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_FUNCTION || $token[0] === \T_FN) && $finalTokens[$lastNotEmptyToken]['code'] !== \T_USE) {
                ...
                if ($parenthesisCloser !== \false) {
                    ...
                    if (isset($tokens[$x]) === \true && is_array($tokens[$x]) === \false && $tokens[$x] === ':') {
                        ...
                        for ($x += 1; $x < $numTokens; $x++) {
                            if (is_array($tokens[$x]) === \true && isset(Tokens::EMPTY_TOKENS[$tokens[$x][0]]) === \true) {
            ...
            /*
                ...
                goto labels.
            */
            if ($tokenIsArray === \true && ($token[0] === \T_STRING || $token[0] === \T_EXIT && (strtolower($token[1]) === 'exit' || strtolower($token[1]) === 'die')) && (is_array($tokens[$stackPtr - 1]) === \false || $tokens[$stackPtr - 1][0] !== \T_PAAMAYIM_NEKUDOTAYIM)) {
                ...
                if (isset($tokens[$i]) === \true && $tokens[$i] === ':') {
                    // Okay, so we have a colon, now we need to make sure that this is not
                    // class constant access, a ternary, enum or switch case.
                    $stopTokens = [\T_DOUBLE_COLON => \true, \T_CASE => \true, \T_SEMICOLON => \true, \T_OPEN_TAG => \true, \T_OPEN_CURLY_BRACKET => \true, \T_INLINE_THEN => \true, \T_ENUM => \true];
                    for ($x = $newStackPtr - 1; $x > 0; $x--) {
                        if (isset($stopTokens[$finalTokens[$x]['code']]) === \true) {
                            break;
                        }
                    }
                    if ($finalTokens[$x]['code'] !== \T_DOUBLE_COLON && $finalTokens[$x]['code'] !== \T_CASE && $finalTokens[$x]['code'] !== \T_INLINE_THEN && $finalTokens[$x]['code'] !== \T_ENUM) {
            ...
            */
            if ($tokenIsArray === \true && strpos($token[1], $this->eolChar) !== \false) {
                ...
                $numLines = count($tokenLines);
                $newToken = ['type' => Tokens::tokenName($token[0]), 'code' => $token[0], 'content' => ''];
                ...
                if ($tokenIsArray === \true && $token[0] === \T_STRING) {
                    ...
                    } elseif (isset(static::T_STRING_CONTEXTS[$finalTokens[$lastNotEmptyToken]['code']]) === \true && $finalTokens[$lastNotEmptyToken]['code'] !== \T_CONST) {
                        $preserveTstring = \true;
        ...
        for ($i = $numTokens - 1; $i >= 0; $i--) {
            ...
            if ($this->tokens[$i]['code'] === \T_FUNCTION) {
                ...
                */
                if (isset($this->tokens[$i]['scope_opener']) === \true) {
                    for ($x = $i + 1; $x < $numTokens; $x++) {
                        if (isset(Tokens::EMPTY_TOKENS[$this->tokens[$x]['code']]) === \false && $this->tokens[$x]['code'] !== \T_BITWISE_AND) {
                            break;
                        }
                    }
                ...
                continue;
            } elseif ($this->tokens[$i]['code'] === \T_CLASS && isset($this->tokens[$i]['scope_opener']) === \true) {
                /*
                ...
                */
                ...
                continue;
            } elseif ($this->tokens[$i]['code'] === \T_FN && isset($this->tokens[$i + 1]) === \true) {
                ...
                if (isset($this->tokens[$x]) === \true && $this->tokens[$x]['code'] === \T_OPEN_PARENTHESIS && isset($this->tokens[$x]['parenthesis_closer']) === \true) {
                    $ignore = Tokens::EMPTY_TOKENS;
                    $ignore += Tokens::NAME_TOKENS;
                    $ignore += [\T_ARRAY => \T_ARRAY, \T_CALLABLE => \T_CALLABLE, \T_COLON => \T_COLON, \T_NULL => \T_NULL, \T_TRUE => \T_TRUE, \T_FALSE => \T_FALSE, \T_NULLABLE => \T_NULLABLE, \T_PARENT => \T_PARENT, \T_SELF => \T_SELF, \T_STATIC => \T_STATIC, \T_TYPE_UNION => \T_TYPE_UNION, \T_TYPE_INTERSECTION => \T_TYPE_INTERSECTION, \T_TYPE_OPEN_PARENTHESIS => \T_TYPE_OPEN_PARENTHESIS, \T_TYPE_CLOSE_PARENTHESIS => \T_TYPE_CLOSE_PARENTHESIS];
                    $closer = $this->tokens[$x]['parenthesis_closer'];
                    ...
                    if ($this->tokens[$arrow]['code'] === \T_DOUBLE_ARROW) {
                        ...
                        for ($scopeCloser = $arrow + 1; $scopeCloser < $numTokens; $scopeCloser++) {
                            ...
                            if (isset($endTokens[$this->tokens[$scopeCloser]['code']]) === \true) {
                                if ($lastEndToken !== null && (isset($this->tokens[$scopeCloser]['parenthesis_opener']) === \true && $this->tokens[$scopeCloser]['parenthesis_opener'] < $arrow || isset($this->tokens[$scopeCloser]['bracket_opener']) === \true && $this->tokens[$scopeCloser]['bracket_opener'] < $arrow)) {
                                    for ($lastNonEmpty = $scopeCloser - 1; $lastNonEmpty > $arrow; $lastNonEmpty--) {
                                        if (isset(Tokens::EMPTY_TOKENS[$this->tokens[$lastNonEmpty]['code']]) === \false) {
                                            $scopeCloser = $lastNonEmpty;
                                            break;
                            ...
                            if ($inTernary === \false && isset($this->tokens[$scopeCloser]['scope_closer'], $this->tokens[$scopeCloser]['scope_condition']) === \true && $scopeCloser === $this->tokens[$scopeCloser]['scope_closer'] && $this->tokens[$this->tokens[$scopeCloser]['scope_condition']]['code'] === \T_FN) {
                ...
                $allowed += [\T_CLOSE_SQUARE_BRACKET => \T_CLOSE_SQUARE_BRACKET, \T_CLOSE_CURLY_BRACKET => \T_CLOSE_CURLY_BRACKET, \T_CLOSE_PARENTHESIS => \T_CLOSE_PARENTHESIS, \T_VARIABLE => \T_VARIABLE, \T_OBJECT_OPERATOR => \T_OBJECT_OPERATOR, \T_NULLSAFE_OBJECT_OPERATOR => \T_NULLSAFE_OBJECT_OPERATOR, \T_CONSTANT_ENCAPSED_STRING => \T_CONSTANT_ENCAPSED_STRING, \T_DOUBLE_QUOTED_STRING => \T_DOUBLE_QUOTED_STRING];
                ...
                for ($x = $i - 1; $x >= 0; $x--) {
                    ...
                    if ($this->tokens[$x]['code'] === \T_CLOSE_CURLY_BRACKET && isset($this->tokens[$x]['scope_condition']) === \true && $this->tokens[$this->tokens[$x]['scope_condition']]['code'] === \T_CLASS) {
                        ...
                        if ($this->tokens[$y]['code'] === \T_OPEN_PARENTHESIS || $this->tokens[$y]['code'] === \T_OPEN_CURLY_BRACKET || $this->tokens[$y]['code'] === \T_EXTENDS || $this->tokens[$y]['code'] === \T_IMPLEMENTS) {
                            break;
                        }
                    }
                    ...
                    if (isset(Tokens::EMPTY_TOKENS[$this->tokens[$x]['code']]) === \false) {
                        ...
                        if ($this->tokens[$x]['code'] === \T_CLOSE_PARENTHESIS && isset($this->tokens[$x]['parenthesis_owner']) === \true && isset(Tokens::SCOPE_OPENERS[$this->tokens[$this->tokens[$x]['parenthesis_owner']]['code']]) === \true || isset($allowed[$this->tokens[$x]['code']]) === \false) {
                            $isShortArray = \true;
                        }
                        break;
                    }
                ...
                continue;
            } elseif ($this->tokens[$i]['code'] === \T_BITWISE_OR || $this->tokens[$i]['code'] === \T_BITWISE_AND || $this->tokens[$i]['code'] === \T_CLOSE_PARENTHESIS) {
                ...
                */
                $allowed = Tokens::NAME_TOKENS;
                $allowed += [\T_CALLABLE => \T_CALLABLE, \T_SELF => \T_SELF, \T_PARENT => \T_PARENT, \T_STATIC => \T_STATIC, \T_FALSE => \T_FALSE, \T_TRUE => \T_TRUE, \T_NULL => \T_NULL];
                $suspectedType = null;
                $typeTokenCountAfter = 0;
                for ($x = $i + 1; $x < $numTokens; $x++) {
                    if (isset(Tokens::EMPTY_TOKENS[$this->tokens[$x]['code']]) === \true) {
                        continue;
                    }
                ...
                if ($typeTokenCountAfter === 0 && ($this->tokens[$i]['code'] !== \T_CLOSE_PARENTHESIS || isset($this->tokens[$i]['parenthesis_owner']) === \true) || isset($suspectedType) === \false) {
                ...
                for ($x = $i - 1; $x >= 0; $x--) {
                    ...
                    if ($suspectedType === 'property or parameter' && $this->tokens[$x]['code'] === \T_OPEN_PARENTHESIS) {
                        ...
                        if (isset($this->tokens[$x]['parenthesis_owner']) === \true && $this->tokens[$this->tokens[$x]['parenthesis_owner']]['code'] === \T_FUNCTION) {
                            $confirmed = \true;
                            break;
                        } else {
                            // This may still be an arrow function which hasn't been handled yet.
                            for ($y = $x - 1; $y > 0; $y--) {
                                if (isset(Tokens::EMPTY_TOKENS[$this->tokens[$y]['code']]) === \false && $this->tokens[$y]['code'] !== \T_BITWISE_AND) {
                                    // Non-whitespace content.
                                    break;
                                }
                            }
                    ...
                    if ($suspectedType === 'return' && $this->tokens[$x]['code'] === \T_COLON) {
                        ...
                        if (isset($this->tokens[$y]['parenthesis_owner']) === \true) {
                            if ($this->tokens[$this->tokens[$y]['parenthesis_owner']]['code'] === \T_FUNCTION || $this->tokens[$this->tokens[$y]['parenthesis_owner']]['code'] === \T_CLOSURE || $this->tokens[$this->tokens[$y]['parenthesis_owner']]['code'] === \T_FN || $this->tokens[$this->tokens[$y]['parenthesis_owner']]['code'] === \T_USE) {
                                $confirmed = \true;
                            }
                            break;
                        }
                        // Arrow functions may not have the parenthesis_owner set correctly yet.
                        if (isset($this->tokens[$y]['parenthesis_opener']) === \true) {
                    ...
                    if ($suspectedType === 'property or parameter' && (isset(Tokens::SCOPE_MODIFIERS[$this->tokens[$x]['code']]) === \true || $this->tokens[$x]['code'] === \T_VAR || $this->tokens[$x]['code'] === \T_STATIC || $this->tokens[$x]['code'] === \T_READONLY || $this->tokens[$x]['code'] === \T_FINAL || $this->tokens[$x]['code'] === \T_ABSTRACT)) {
                ...
                if ($confirmed === \false && $suspectedType === 'property or parameter' && isset($this->tokens[$i]['nested_parenthesis']) === \true) {
                    ...
                    if (isset($this->tokens[$last]['parenthesis_owner']) === \true && $this->tokens[$this->tokens[$last]['parenthesis_owner']]['code'] === \T_FUNCTION) {
                        $confirmed = \true;
                    } else if (isset($this->tokens[$last]['parenthesis_opener']) === \true) {
                        ...
                        if ($this->tokens[$x]['code'] === \T_FN) {
                            for (--$x; $x >= 0; $x--) {
                                if (isset(Tokens::EMPTY_TOKENS[$this->tokens[$x]['code']]) === \true || $this->tokens[$x]['code'] === \T_BITWISE_AND) {
                                    continue;
                                }
                                break;
                            }
                ...
                continue;
            } elseif ($this->tokens[$i]['code'] === \T_TRUE || $this->tokens[$i]['code'] === \T_FALSE || $this->tokens[$i]['code'] === \T_NULL) {
                for ($x = $i + 1; $x < $numTokens; $x++) {
                    if (isset(Tokens::EMPTY_TOKENS[$this->tokens[$x]['code']]) === \false) {
                        // Non-whitespace content.
                        break;
                ...
                if ($x !== $numTokens && isset(static::T_STRING_CONTEXTS[$this->tokens[$x]['code']]) === \true) {
                    if (\PHP_CODESNIFFER_VERBOSITY > 1) {
                        $line = $this->tokens[$i]['line'];
    ...
    public static function standardiseToken($token)
    {
        ...
        if ($token[0] === \T_STRING) {
            ...
            $newToken = ['code' => $token[0], 'type' => Tokens::tokenName($token[0])];
    ...
    private function createAttributesNestingMap()
    {
        ...
        for ($i = 0; $i < $this->numTokens; $i++) {
            ...
            } elseif (isset($this->tokens[$i]['attribute_closer']) === \true && $i === $this->tokens[$i]['attribute_closer']) {
                array_pop($map);
                if (empty($map) === \false) {
                    $this->tokens[$i]['nested_attributes'] = $map;
                }
