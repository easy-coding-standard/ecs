...
class File
{
    ...
    public function process()
    {
        ...
        foreach ($this->tokens as $stackPtr => $token) {
            ...
            if ($checkAnnotations === \true && ($token['code'] === \T_COMMENT || $token['code'] === \T_PHPCS_IGNORE_FILE || $token['code'] === \T_PHPCS_SET || $token['code'] === \T_DOC_COMMENT_STRING || $token['code'] === \T_DOC_COMMENT_TAG || $inTests === \true && $token['code'] === \T_INLINE_HTML)) {
                ...
                if (substr($commentTextLower, 0, 16) === 'phpcs:ignorefile' || substr($commentTextLower, 0, 17) === '@phpcs:ignorefile') {
                    ...
                    $this->fixableWarningCount = 0;
                    return;
                } elseif (substr($commentTextLower, 0, 9) === 'phpcs:set' || substr($commentTextLower, 0, 10) === '@phpcs:set') {
            ...
            foreach ($this->ruleset->tokenListeners[$token['code']] as $listenerData) {
                ...
                if ($this->configCache['trackTime'] === \true) {
        ...
        if (empty($this->firstRunCounts) === \true) {
            $this->firstRunCounts = ['error' => $this->errorCount, 'warning' => $this->warningCount, 'fixableError' => $this->fixableErrorCount, 'fixableWarning' => $this->fixableWarningCount];
        }
    ...
    protected function addMessage(bool $error, string $message, int $line, int $column, string $code, array $data, int $severity, bool $fixable)
    {
        ...
        if (isset($this->tokenizer->ignoredLines[$line]) === \true && $this->tokenizer->ignoredLines[$line]->ignoresEverything() === \true) {
            return \false;
        }
        ...
        if (isset($this->tokenizer->ignoredLines[$line]) === \true && $this->tokenizer->ignoredLines[$line]->isIgnored($sniffCode) === \true) {
            return \false;
        }
        $includeAll = \true;
        ...
        if ($includeAll === \false && ($parts[0] !== 'Internal' && empty($this->configCache['sniffs']) === \false && in_array(strtolower($listenerCode), $this->configCache['sniffs'], \true) === \false || empty($this->configCache['exclude']) === \false && in_array(strtolower($listenerCode), $this->configCache['exclude'], \true) === \true)) {
            return \false;
        }
        ...
        if (trim($this->path, '\'"') === 'STDIN') {
            ...
            foreach ($checkCodes as $checkCode) {
                ...
                if (isset($this->configCache['includePatterns'][$checkCode]) === \true) {
                    ...
                    $excluding = \false;
                } elseif (isset($this->configCache['ignorePatterns'][$checkCode]) === \true) {
    ...
    public function getMethodParameters(int $stackPtr)
    {
        ...
        if ($this->tokens[$stackPtr]['code'] === \T_USE && isset($this->tokens[$stackPtr]['parenthesis_owner']) === \false) {
                ...
                case \T_COMMA:
                    ...
                    if ($visibilityToken !== null || $setVisibilityToken !== null || $readonlyToken !== null) {
                        ...
                        if ($readonlyToken !== null) {
                            $vars[$paramCount]['property_readonly'] = \true;
                    ...
                    break;
                case \T_EQUAL:
                    $defaultStart = $this->findNext(Tokens::EMPTY_TOKENS, $i + 1, null, \true);
                    $equalToken = $i;
                    break;
            }
        }
    ...
    public function getMethodProperties(int $stackPtr)
    {
        ...
        if ($this->tokens[$stackPtr]['code'] === \T_FUNCTION) {
            $valid = [\T_PUBLIC => \T_PUBLIC, \T_PRIVATE => \T_PRIVATE, \T_PROTECTED => \T_PROTECTED, \T_STATIC => \T_STATIC, \T_FINAL => \T_FINAL, \T_ABSTRACT => \T_ABSTRACT, \T_WHITESPACE => \T_WHITESPACE, \T_COMMENT => \T_COMMENT];
        } else {
            $valid = [\T_STATIC => \T_STATIC, \T_WHITESPACE => \T_WHITESPACE, \T_COMMENT => \T_COMMENT];
        }
        $scope = 'public';
        ...
        if (isset($this->tokens[$stackPtr]['parenthesis_closer']) === \true) {
            ...
            $valid += [\T_CALLABLE => \T_CALLABLE, \T_SELF => \T_SELF, \T_PARENT => \T_PARENT, \T_STATIC => \T_STATIC, \T_FALSE => \T_FALSE, \T_TRUE => \T_TRUE, \T_NULL => \T_NULL, \T_TYPE_UNION => \T_TYPE_UNION, \T_TYPE_INTERSECTION => \T_TYPE_INTERSECTION, \T_TYPE_OPEN_PARENTHESIS => \T_TYPE_OPEN_PARENTHESIS, \T_TYPE_CLOSE_PARENTHESIS => \T_TYPE_CLOSE_PARENTHESIS];
    ...
    public function getMemberProperties(int $stackPtr)
    {
        ...
        if (empty($this->tokens[$stackPtr]['conditions']) === \true) {
            throw new RuntimeException('$stackPtr is not a class member var');
        }
        $conditions = $this->tokens[$stackPtr]['conditions'];
        $conditions = array_keys($conditions);
        $ptr = array_pop($conditions);
        if (isset($this->tokens[$ptr]) === \false || isset(Tokens::OO_SCOPE_TOKENS[$this->tokens[$ptr]['code']]) === \false || $this->tokens[$ptr]['code'] === \T_ENUM) {
            throw new RuntimeException('$stackPtr is not a class member var');
        }
        ...
        $valid = [\T_STATIC => \T_STATIC, \T_VAR => \T_VAR, \T_READONLY => \T_READONLY, \T_FINAL => \T_FINAL, \T_ABSTRACT => \T_ABSTRACT];
        ...
        if ($i < $stackPtr) {
            // We've found a type.
            $valid = Tokens::NAME_TOKENS;
            $valid += [\T_CALLABLE => \T_CALLABLE, \T_SELF => \T_SELF, \T_PARENT => \T_PARENT, \T_FALSE => \T_FALSE, \T_TRUE => \T_TRUE, \T_NULL => \T_NULL, \T_TYPE_UNION => \T_TYPE_UNION, \T_TYPE_INTERSECTION => \T_TYPE_INTERSECTION, \T_TYPE_OPEN_PARENTHESIS => \T_TYPE_OPEN_PARENTHESIS, \T_TYPE_CLOSE_PARENTHESIS => \T_TYPE_CLOSE_PARENTHESIS];
            for ($i; $i < $stackPtr; $i++) {
        ...
        return ['scope' => $scope, 'scope_specified' => $scopeSpecified, 'set_scope' => $setScope, 'is_static' => $isStatic, 'is_readonly' => $isReadonly, 'is_final' => $isFinal, 'is_abstract' => $isAbstract, 'type' => $type, 'type_token' => $typeToken, 'type_end_token' => $typeEndToken, 'nullable_type' => $nullableType];
    }
    /**
    ...
    public function getClassProperties(int $stackPtr)
    {
        ...
        $valid = [\T_FINAL => \T_FINAL, \T_ABSTRACT => \T_ABSTRACT, \T_READONLY => \T_READONLY, \T_WHITESPACE => \T_WHITESPACE, \T_COMMENT => \T_COMMENT];
        $isAbstract = \false;
        $isFinal = \false;
        $isReadonly = \false;
        for ($i = $stackPtr - 1; $i > 0; $i--) {
        ...
        return ['is_abstract' => $isAbstract, 'is_final' => $isFinal, 'is_readonly' => $isReadonly];
    }
    /**
    ...
    public function isReference(int $stackPtr)
    {
        ...
        $tokenBefore = $this->findPrevious(Tokens::EMPTY_TOKENS, $stackPtr - 1, null, \true);
        ...
        if (isset(Tokens::ASSIGNMENT_TOKENS[$this->tokens[$tokenBefore]['code']]) === \true) {
            // This is directly after an assignment. It's a reference. Even if
            // it is part of an operation, the other tests will handle it.
            return \true;
        }
        $tokenAfter = $this->findNext(Tokens::EMPTY_TOKENS, $stackPtr + 1, null, \true);
        ...
        if (isset($this->tokens[$stackPtr]['nested_parenthesis']) === \true) {
            ...
            if (isset($this->tokens[$lastBracket]['parenthesis_owner']) === \true) {
                ...
                if ($owner['code'] === \T_FUNCTION || $owner['code'] === \T_CLOSURE || $owner['code'] === \T_FN || $owner['code'] === \T_USE) {
    ...
    public function findPrevious($types, int $start, ?int $end = null, bool $exclude = \false, ?string $value = null, bool $local = \false)
    {
        ...
        for ($i = $start; $i >= $end; $i--) {
            ...
            if ($local === \true) {
                ...
                } elseif (isset($this->tokens[$i]['bracket_opener']) === \true && $i === $this->tokens[$i]['bracket_closer']) {
                    $i = $this->tokens[$i]['bracket_opener'];
                } elseif (isset($this->tokens[$i]['parenthesis_opener']) === \true && $i === $this->tokens[$i]['parenthesis_closer']) {
    ...
    public function findStartOfStatement(int $start, $ignore = null)
    {
            ...
            if ($lastConditionOwner === \T_MATCH && (empty($this->tokens[$matchExpression]['nested_parenthesis']) === \true && empty($this->tokens[$start]['nested_parenthesis']) === \true || empty($this->tokens[$matchExpression]['nested_parenthesis']) === \false && empty($this->tokens[$start]['nested_parenthesis']) === \false && $this->tokens[$matchExpression]['nested_parenthesis'] === $this->tokens[$start]['nested_parenthesis'])) {
                ...
                if ($inNestedExpression === \false) {
                    ...
                    if ($prevMatch <= $this->tokens[$matchExpression]['scope_opener']) {
                        ...
                        $next = $this->findNext(Tokens::EMPTY_TOKENS, $this->tokens[$matchExpression]['scope_opener'] + 1, null, \true);
                        if ($next === \false) {
                    ...
                    $next = $this->findNext(Tokens::EMPTY_TOKENS, $prevMatch + 1, null, \true);
                    if ($next === \false) {
                        // Shouldn't be possible.
                        return $start;
                    }
        ...
        for ($i = $start; $i >= 0; $i--) {
            ...
            if (isset($this->tokens[$i]['scope_opener']) === \true && $i === $this->tokens[$i]['scope_closer'] && $this->tokens[$i]['code'] !== \T_CLOSE_PARENTHESIS && $this->tokens[$i]['code'] !== \T_END_NOWDOC && $this->tokens[$i]['code'] !== \T_END_HEREDOC && $this->tokens[$i]['code'] !== \T_BREAK && $this->tokens[$i]['code'] !== \T_RETURN && $this->tokens[$i]['code'] !== \T_CONTINUE && $this->tokens[$i]['code'] !== \T_THROW && $this->tokens[$i]['code'] !== \T_EXIT && $this->tokens[$i]['code'] !== \T_GOTO) {
    ...
    public function findEndOfStatement(int $start, $ignore = null)
    {
        ...
        for ($i = $start; $i < $this->numTokens; $i++) {
            ...
            if (isset($this->tokens[$i]['scope_closer']) === \true && ($i === $this->tokens[$i]['scope_opener'] || $i === $this->tokens[$i]['scope_condition'])) {
                ...
                if ($i === $start && isset(Tokens::SCOPE_OPENERS[$this->tokens[$i]['code']]) === \true) {
                    return $this->tokens[$i]['scope_closer'];
                }
                $i = $this->tokens[$i]['scope_closer'];
            } elseif (isset($this->tokens[$i]['bracket_closer']) === \true && $i === $this->tokens[$i]['bracket_opener']) {
                $i = $this->tokens[$i]['bracket_closer'];
            } elseif (isset($this->tokens[$i]['parenthesis_closer']) === \true && $i === $this->tokens[$i]['parenthesis_opener']) {
                $i = $this->tokens[$i]['parenthesis_closer'];
     ...
     * @return int|false
     */
    public function getCondition(int $stackPtr, $type, bool $first = \true)
    {
        // Check for the existence of the token.
        if (isset($this->tokens[$stackPtr]) === \false) {
            return \false;
