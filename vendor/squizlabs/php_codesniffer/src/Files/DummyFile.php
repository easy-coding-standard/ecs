...
class DummyFile extends \PHP_CodeSniffer\Files\File
{
    ...
    public function __construct(string $content, Ruleset $ruleset, Config $config)
    {
        ...
        if ($content !== '') {
            if (substr($content, 0, 17) === 'phpcs_input_file:') {
                ...
                $filename = trim((string) substr($content, 17, $eolPos - 17));
     ...
     * @return void
     */
    public function setErrorCounts(int $errorCount, int $warningCount, int $fixableErrorCount, int $fixableWarningCount, int $fixedErrorCount, int $fixedWarningCount)
    {
