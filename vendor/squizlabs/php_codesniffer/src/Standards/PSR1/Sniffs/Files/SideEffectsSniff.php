...
class SideEffectsSniff implements Sniff
{
     ...
     */
    private const SYMBOL_TOKENS = [\T_CLASS => \T_CLASS, \T_INTERFACE => \T_INTERFACE, \T_TRAIT => \T_TRAIT, \T_ENUM => \T_ENUM, \T_FUNCTION => \T_FUNCTION];
    /**
     * Condition tokens within which symbols may be defined.
     *
     * @var array<int|string, int|string>
     */
    private const CONDITION_TOKENS = [\T_IF => \T_IF, \T_ELSE => \T_ELSE, \T_ELSEIF => \T_ELSEIF];
    /**
    ...
    private function searchForConflict(File $phpcsFile, int $start, int $end, array $tokens)
    {
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if (isset(Tokens::BOOLEAN_OPERATORS[$tokens[$i]['code']]) === \true) {
                continue;
            }
            ...
            if ($tokens[$i]['code'] === \T_NAMESPACE || $tokens[$i]['code'] === \T_USE || $tokens[$i]['code'] === \T_DECLARE || $tokens[$i]['code'] === \T_CONST) {
                if (isset($tokens[$i]['scope_opener']) === \true) {
                    ...
                    if ($tokens[$i]['code'] === \T_ENDDECLARE) {
                        $semicolon = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $i + 1, null, \true);
            ...
            if (isset(Tokens::METHOD_MODIFIERS[$tokens[$i]['code']]) === \true || $tokens[$i]['code'] === \T_READONLY) {
                continue;
            }
            // Ignore anon classes.
            ...
            if (isset(self::SYMBOL_TOKENS[$tokens[$i]['code']]) === \true && isset($tokens[$i]['scope_closer']) === \true) {
                if ($firstSymbol === null) {
                    $firstSymbol = $i;
                }
                $i = $tokens[$i]['scope_closer'];
                continue;
            } elseif (($tokens[$i]['code'] === \T_STRING || $tokens[$i]['code'] === \T_NAME_FULLY_QUALIFIED) && strtolower(ltrim($tokens[$i]['content'], '\\')) === 'define') {
                $prev = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $i - 1, null, \true);
            ...
            if (($tokens[$i]['code'] === \T_STRING || $tokens[$i]['code'] === \T_NAME_FULLY_QUALIFIED) && strtolower(ltrim($tokens[$i]['content'], '\\')) === 'defined') {
                $openBracket = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $i + 1, null, \true);
            ...
            // in this case.
            if (isset(self::CONDITION_TOKENS[$tokens[$i]['code']]) === \true) {
        ...
        return ['symbol' => $firstSymbol, 'effect' => $firstEffect];
    }
}
