...
class FileHeaderSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        do {
            ...
            if (empty($headerLines) === \true && $openTag === $stackPtr) {
            ...
            if (isset(Tokens::OO_SCOPE_TOKENS[$tokens[$next]['code']]) === \true) {
    ...
    public function getHeaderLines(File $phpcsFile, int $stackPtr)
    {
        ...
        $headerLines = [];
        $headerLines[] = ['type' => 'tag', 'start' => $stackPtr, 'end' => $stackPtr];
        $foundDocblock = \false;
            ...
            switch ($tokens[$next]['code']) {
                case \T_DOC_COMMENT_OPEN_TAG:
                    ...
                    for ($docToken = $end + 1; $docToken < $phpcsFile->numTokens; $docToken++) {
                        if (isset(Tokens::EMPTY_TOKENS[$tokens[$docToken]['code']]) === \true) {
                            continue;
                        }
                    ...
                    if (isset($commentOpeners[$tokens[$docToken]['code']]) === \false && isset(Tokens::METHOD_MODIFIERS[$tokens[$docToken]['code']]) === \false && $tokens[$docToken]['code'] !== \T_READONLY) {
                        ...
                        if ($annotation === \false) {
                            $foundDocblock = \true;
                            $headerLines[] = ['type' => 'docblock', 'start' => $next, 'end' => $end];
                        }
                    }
                    $next = $end;
                    break;
                case \T_DECLARE:
                case \T_NAMESPACE:
                    if (isset($tokens[$next]['scope_opener']) === \true) {
                ...
                case \T_USE:
                    $type = 'use';
                    $useType = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $next + 1, null, \true);
                    ...
                    $end = $phpcsFile->findEndOfStatement($next);
                    $headerLines[] = ['type' => $type, 'start' => $next, 'end' => $end];
                    $next = $end;
                    break;
                default:
                    // Skip comments as PSR-12 doesn't say if these are allowed or not.
                    if (isset(Tokens::COMMENT_TOKENS[$tokens[$next]['code']]) === \true) {
                        $next = $phpcsFile->findNext(Tokens::COMMENT_TOKENS, $next + 1, null, \true);
                        if ($next === \false) {
    ...
    public function processHeaderLines(File $phpcsFile, array $headerLines)
    {
        ...
        foreach ($headerLines as $i => $line) {
            if (isset($headerLines[$i + 1]) === \false || $headerLines[$i + 1]['type'] !== $line['type']) {
                ...
                // this block.
                $next = $phpcsFile->findNext(\T_WHITESPACE, $line['end'] + 1, null, \true);
                ...
                if (isset($headerLines[$i + 1]) === \true && isset($found[$headerLines[$i + 1]['type']]) === \true) {
                    $error = 'Similar statements must be grouped together inside header blocks; ';
                    $error .= 'the first "%s" statement was found on line %s';
                    $data = [$headerLines[$i + 1]['type'], $tokens[$found[$headerLines[$i + 1]['type']]['start']]['line']];
        ...
        */
        $blockOrder = ['tag' => 'opening PHP tag', 'docblock' => 'file-level docblock', 'declare' => 'declare statements', 'namespace' => 'namespace declaration', 'use' => 'class-based use imports', 'use function' => 'function-based use imports', 'use const' => 'constant-based use imports'];
        foreach (array_keys($found) as $type) {
            ...
            if ($orderedType === \false) {
                ...
                do {
                    $orderedType = next($blockOrder);
                    if (isset($found[key($blockOrder)]) === \true && key($blockOrder) !== $type) {
                        $prevValidType = key($blockOrder);
                    }
                } while ($orderedType !== \false && key($blockOrder) !== $type);
                $error = 'The %s must follow the %s in the file header';
                $data = [$blockOrder[$type], $blockOrder[$prevValidType]];
