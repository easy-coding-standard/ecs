...
class ControlStructureSpacingSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        for ($i = $parenOpener; $i < $parenCloser; $i++) {
            if ($tokens[$i]['column'] !== 1 || $tokens[$i + 1]['line'] > $tokens[$i]['line'] || isset(Tokens::COMMENT_TOKENS[$tokens[$i]['code']]) === \true) {
                continue;
            }
            if ($i + 1 === $parenCloser) {
                break;
            ...
            if (isset(Tokens::TEXT_STRING_TOKENS[$tokens[$i]['code']]) === \true || isset(Tokens::HEREDOC_TOKENS[$tokens[$i]['code']]) === \true) {
                continue;
            }
