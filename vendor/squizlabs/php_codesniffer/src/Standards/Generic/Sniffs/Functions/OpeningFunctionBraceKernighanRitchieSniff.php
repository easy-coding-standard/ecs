...
class OpeningFunctionBraceKernighanRitchieSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        $prev = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $openingBrace - 1, null, \true);
        ...
        if ($length === 1 && isset($tokens[$openingBrace - 1]['orig_content']) === \true && $tokens[$openingBrace - 1]['orig_content'] === "\t") {
            $length = '\t';
        }
        if ($length !== 1) {
