...
class UnusedFunctionParameterSniff implements Sniff
{
     ...
     */
    private const MAGIC_METHODS = ['__destruct' => \true, '__call' => \true, '__callstatic' => \true, '__get' => \true, '__set' => \true, '__isset' => \true, '__unset' => \true, '__sleep' => \true, '__wakeup' => \true, '__serialize' => \true, '__unserialize' => \true, '__tostring' => \true, '__set_state' => \true, '__clone' => \true, '__debuginfo' => \true];
    /**
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if ($token['code'] === \T_FUNCTION) {
            ...
            if ($classPtr !== \false) {
                ...
                if (isset(self::MAGIC_METHODS[$methodNameLc]) === \true) {
                    return;
                }
        ...
        for (; $next <= $end; ++$next) {
            ...
            $code = $token['code'];
            // Ignorable tokens.
            if (isset(Tokens::EMPTY_TOKENS[$code]) === \true) {
                continue;
            }
            if ($foundContent === \false) {
                ...
                if ($code === \T_RETURN) {
                    $firstNonEmptyTokenAfterReturn = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $next + 1, null, \true);
                    if ($tokens[$firstNonEmptyTokenAfterReturn]['code'] === \T_SEMICOLON && $implements !== \false) {
                        return;
                    }
                    $secondNonEmptyTokenAfterReturn = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $firstNonEmptyTokenAfterReturn + 1, null, \true);
            ...
            if ($code === \T_VARIABLE && isset($params[$token['content']]) === \true) {
                ...
                if ($tokens[$nextToken]['code'] === \T_OPEN_CURLY_BRACKET) {
                    ...
                    if ($tokens[$nextToken]['code'] === \T_STRING) {
                        ...
                        if (isset($params[$varContent]) === \true) {
                ...
                for ($i = $next + 1; $i <= $end; $i++) {
                    if (isset($validTokens[$tokens[$i]['code']]) === \true) {
                ...
                foreach ($stringTokens as $stringPtr => $stringToken) {
                    ...
                    if ($varContent !== '' && isset($params[$varContent]) === \true) {
                        unset($params[$varContent]);
                    }
                }
            }
        }
        if ($foundContent === \true && count($params) > 0) {
            ...
            if ($methodParamsCount === 1) {
                foreach ($params as $paramName => $position) {
                    if (in_array($methodParams[0]['type_hint'], $this->ignoreTypeHints, \true) === \true) {
                        continue;
                    }
                    $data = [$paramName];
            ...
            for ($i = $lastIndex; $i >= 0; --$i) {
                if ($foundLastUsed !== \false) {
                    if (isset($params[$methodParams[$i]['name']]) === \true) {
                        $errorInfo[$methodParams[$i]['name']] = ['position' => $params[$methodParams[$i]['name']], 'errorcode' => $errorCode . 'BeforeLastUsed', 'typehint' => $methodParams[$i]['type_hint']];
                    }
                } else if (isset($params[$methodParams[$i]['name']]) === \false) {
                    $foundLastUsed = \true;
                } else {
                    $errorInfo[$methodParams[$i]['name']] = ['position' => $params[$methodParams[$i]['name']], 'errorcode' => $errorCode . 'AfterLastUsed', 'typehint' => $methodParams[$i]['type_hint']];
                }
            }
            if (count($errorInfo) > 0) {
                $errorInfo = array_reverse($errorInfo);
                foreach ($errorInfo as $paramName => $info) {
                    if (in_array($info['typehint'], $this->ignoreTypeHints, \true) === \true) {
                        continue;
                    }
                    $data = [$paramName];
