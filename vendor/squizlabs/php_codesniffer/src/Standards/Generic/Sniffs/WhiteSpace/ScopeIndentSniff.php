...
class ScopeIndentSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if (empty($this->ignoreIndentation) === \true) {
            $this->ignoreIndentation = [\T_INLINE_HTML => \true];
            foreach ($this->ignoreIndentationTokens as $token) {
                ...
                $this->ignoreIndentation[$token] = \true;
            }
        }
        ...
        for ($i = $stackPtr + 1; $i < $phpcsFile->numTokens; $i++) {
            ...
            if ($checkAnnotations === \true && $tokens[$i]['code'] === \T_PHPCS_SET && isset($tokens[$i]['sniffCode']) === \true && $tokens[$i]['sniffCode'] === 'Generic.WhiteSpace.ScopeIndent' && $tokens[$i]['sniffProperty'] === 'exact') {
            ...
            */
            ...
            if ($exact === \true && $i < $disableExactEnd) {
                $exact = \false;
            }
            ...
            if ($tokens[$i]['column'] === 1) {
                ...
                if ($trimmed === '') {
                    if (isset($tokens[$i + 1]) === \true && $tokens[$i]['line'] === $tokens[$i + 1]['line']) {
                        $checkToken = $i + 1;
            ...
            if ($checkToken !== null && $tokens[$checkToken]['code'] === \T_CLOSE_PARENTHESIS && isset($tokens[$checkToken]['parenthesis_opener']) === \true || $tokens[$i]['code'] === \T_CLOSE_PARENTHESIS && isset($tokens[$i]['parenthesis_opener']) === \true) {
                if ($checkToken !== null) {
                ...
                if ($tokens[$parenCloser]['line'] !== $tokens[$parenOpener]['line']) {
                    $parens = 0;
                    if (isset($tokens[$parenCloser]['nested_parenthesis']) === \true && empty($tokens[$parenCloser]['nested_parenthesis']) === \false) {
                    ...
                    $condition = 0;
                    if (isset($tokens[$parenCloser]['conditions']) === \true && empty($tokens[$parenCloser]['conditions']) === \false && (isset($tokens[$parenCloser]['parenthesis_owner']) === \false || $parens > 0)) {
                    ...
                    if ($condition > 0 && $lastOpenTagCondition === $condition) {
                        ...
                        $first = $phpcsFile->findFirstOnLine([\T_WHITESPACE, \T_INLINE_HTML], $lastOpenTag, \true);
                        if ($this->debug === \true) {
                        ...
                        if (isset($adjustments[$condition]) === \true) {
                    ...
                    } elseif ($condition > 0 && isset($tokens[$condition]['scope_opener']) === \true && isset($setIndents[$tokens[$condition]['scope_opener']]) === \true) {
                        ...
                        $first = $phpcsFile->findFirstOnLine(\T_WHITESPACE, $parenOpener, \true);
                        $checkIndent = $tokens[$first]['column'] - 1;
                        if (isset($adjustments[$first]) === \true) {
                        ...
                        if ($first === $tokens[$parenCloser]['parenthesis_opener'] && $tokens[$first - 1]['line'] === $tokens[$first]['line']) {
                        ...
                        if ($prev !== $first) {
                            ...
                            $first = $phpcsFile->findFirstOnLine([\T_WHITESPACE, \T_INLINE_HTML], $prev, \true);
                            if ($first !== \false) {
                                $prev = $phpcsFile->findStartOfStatement($first, \T_COMMA);
                                $first = $phpcsFile->findFirstOnLine([\T_WHITESPACE, \T_INLINE_HTML], $prev, \true);
                            } else {
                        ...
                        if (isset($tokens[$first]['scope_closer']) === \true && $tokens[$first]['scope_closer'] === $first) {
                            if ($this->debug === \true) {
                                StatusWriter::write('* first token is a scope closer *', 1);
                            }
                            if (isset($tokens[$first]['scope_condition']) === \true) {
                                $scopeCloser = $first;
                                $first = $phpcsFile->findFirstOnLine(\T_WHITESPACE, $tokens[$scopeCloser]['scope_condition'], \true);
            ...
            if ($tokens[$i]['code'] === \T_CLOSE_SHORT_ARRAY || $checkToken !== null && $tokens[$checkToken]['code'] === \T_CLOSE_SHORT_ARRAY) {
                ...
                if ($tokens[$arrayCloser]['line'] !== $tokens[$arrayOpener]['line']) {
                    $first = $phpcsFile->findFirstOnLine(\T_WHITESPACE, $arrayOpener, \true);
                    $exact = \false;
                    if ($this->debug === \true) {
                        $line = $tokens[$first]['line'];
                        $type = $tokens[$first]['type'];
                    ...
                    if ($prev !== $first) {
                        ...
                        $first = $phpcsFile->findFirstOnLine(\T_WHITESPACE, $prev, \true);
                        $prev = $phpcsFile->findStartOfStatement($first, [\T_COMMA, \T_DOUBLE_ARROW]);
                        $first = $phpcsFile->findFirstOnLine(\T_WHITESPACE, $prev, \true);
                        if ($this->debug === \true) {
                            $line = $tokens[$first]['line'];
                        ...
                        $first = $phpcsFile->findNext(\T_WHITESPACE, $first + 1, null, \true);
                    }
                    $checkIndent = $tokens[$first]['column'] - 1;
                    if (isset($adjustments[$first]) === \true) {
                        $checkIndent += $adjustments[$first];
            ...
            if ($checkToken !== null && $exact === \false && (empty($tokens[$checkToken]['conditions']) === \false || isset($tokens[$checkToken]['scope_opener']) === \true && $tokens[$checkToken]['scope_opener'] === $checkToken)) {
                ...
                $first = $phpcsFile->findFirstOnLine(\T_WHITESPACE, $condition, \true);
                if (isset($adjustments[$first]) === \true && ($adjustments[$first] < 0 && $tokenIndent > $currentIndent || $adjustments[$first] > 0 && $tokenIndent < $currentIndent)) {
                    ...
                    if ($phpcsFile->fixer->enabled === \true) {
                        $tokenIndent = $length;
            ...
            if ($checkToken !== null && (isset($openScopes[$checkToken]) === \true || isset($tokens[$checkToken]['scope_condition']) === \true && isset($tokens[$checkToken]['scope_closer']) === \true && $tokens[$checkToken]['scope_closer'] === $checkToken && $tokens[$checkToken]['line'] !== $tokens[$tokens[$checkToken]['scope_opener']]['line']) || $checkToken === null && isset($openScopes[$i]) === \true) {
                if ($this->debug === \true) {
                    if ($checkToken === null) {
                ...
                if ($this->debug === \true && $conditionToken !== null) {
                ...
                if (isset($tokens[$scopeCloser]['scope_condition']) === \true) {
                    $first = $phpcsFile->findFirstOnLine([\T_WHITESPACE, \T_INLINE_HTML], $tokens[$scopeCloser]['scope_condition'], \true);
                    if ($this->debug === \true) {
                        $line = $tokens[$first]['line'];
                        $type = $tokens[$first]['type'];
                    ...
                    while ($tokens[$first]['code'] === \T_CONSTANT_ENCAPSED_STRING && $tokens[$first - 1]['code'] === \T_CONSTANT_ENCAPSED_STRING) {
                        $first = $phpcsFile->findFirstOnLine(\T_WHITESPACE, $first - 1, \true);
                        if ($this->debug === \true) {
                            $line = $tokens[$first]['line'];
                            $type = $tokens[$first]['type'];
            ...
            if ($checkToken !== null && isset(Tokens::SCOPE_OPENERS[$tokens[$checkToken]['code']]) === \true && in_array($tokens[$checkToken]['code'], $this->nonIndentingScopes, \true) === \false && isset($tokens[$checkToken]['scope_opener']) === \true) {
                $exact = \true;
                ...
                if ($lastOpener !== null && isset($tokens[$lastOpener]['scope_closer']) === \true && $tokens[$lastOpener]['level'] === $tokens[$checkToken]['level'] && $tokens[$lastOpener]['scope_closer'] === $tokens[$checkToken]['scope_closer']) {
            ...
            if ($checkToken !== null && isset(Tokens::METHOD_MODIFIERS[$tokens[$checkToken]['code']]) === \true && $tokens[$checkToken + 1]['code'] !== \T_DOUBLE_COLON) {
                $next = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $checkToken + 1, null, \true);
            ...
            // in most cases.
            if ($checkToken !== null && ($tokens[$checkToken]['code'] === \T_OPEN_TAG || $tokens[$checkToken]['code'] === \T_OPEN_TAG_WITH_ECHO)) {
                ...
                if (empty($tokens[$checkToken]['conditions']) === \false) {
                    ...
                    if ($close !== \false && $tokens[$checkToken]['conditions'] === $tokens[$close]['conditions']) {
                        ...
                        if ($tokens[$lastCloser]['line'] !== $tokens[$checkToken]['line'] && isset($setIndents[$lastOpener]) === \true) {
            ...
            if ($checkToken !== null && ($tokens[$checkToken]['code'] === \T_OBJECT_OPERATOR || $tokens[$checkToken]['code'] === \T_NULLSAFE_OBJECT_OPERATOR) && $exact === \true) {
                $exact = \false;
            }
            if ($checkIndent === null) {
                $checkIndent = $currentIndent;
            ...
            */
            if ($checkToken !== null && isset($this->ignoreIndentation[$tokens[$checkToken]['code']]) === \false && ($tokenIndent !== $checkIndent && $exact === \true || $tokenIndent < $checkIndent && $exact === \false)) {
                $type = 'IncorrectExact';
            ...
            if (isset($tokens[$i]['scope_condition']) === \true && isset($tokens[$i]['scope_opener']) === \true && $tokens[$i]['scope_opener'] === $i) {
                $closer = $tokens[$i]['scope_closer'];
                if ($tokens[$i]['line'] === $tokens[$closer]['line']) {
                    if ($this->debug === \true) {
                        $line = $tokens[$i]['line'];
                ...
                if (isset(Tokens::SCOPE_OPENERS[$condition]) === \true && in_array($condition, $this->nonIndentingScopes, \true) === \false) {
                    if ($this->debug === \true) {
                        $line = $tokens[$i]['line'];
                        $type = $tokens[$tokens[$i]['scope_condition']]['type'];
            ...
            if (isset($tokens[$i]['scope_condition']) === \true && $tokens[$i]['scope_closer'] === $i && ($tokens[$tokens[$i]['scope_condition']]['code'] === \T_CLOSURE || $tokens[$tokens[$i]['scope_condition']]['code'] === \T_ANON_CLASS || $tokens[$tokens[$i]['scope_condition']]['code'] === \T_MATCH || $tokens[$tokens[$i]['scope_condition']]['code'] === \T_FN)) {
                if ($this->debug === \true) {
                ...
                $prev = \false;
                $parens = 0;
                if (isset($tokens[$i]['nested_parenthesis']) === \true && empty($tokens[$i]['nested_parenthesis']) === \false) {
                ...
                $condition = 0;
                if (isset($tokens[$i]['conditions']) === \true && empty($tokens[$i]['conditions']) === \false) {
                ...
                if ($parens > $condition) {
                    ...
                    $prev = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $parens - 1, null, \true);
                    $condition = 0;
                } elseif ($condition > 0) {
                    if ($this->debug === \true) {
                        StatusWriter::write('* using condition *', 1);
                ...
                if ($prev === \false) {
                    $prev = $phpcsFile->findPrevious([\T_EQUAL, \T_RETURN], $tokens[$i]['scope_condition'] - 1, null, \false, null, \true);
                    if ($prev === \false) {
                        $prev = $i;
                        if ($this->debug === \true) {
