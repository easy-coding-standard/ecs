...
class DisallowTabIndentSniff implements Sniff
{
     ...
     */
    private const TOKENS_CONTAINING_INDENT = [\T_WHITESPACE => \true, \T_INLINE_HTML => \true, \T_DOC_COMMENT_WHITESPACE => \true, \T_DOC_COMMENT_STRING => \true, \T_COMMENT => \true, \T_END_HEREDOC => \true, \T_END_NOWDOC => \true, \T_YIELD_FROM => \true];
    /**
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        for ($i = 0; $i < $phpcsFile->numTokens; $i++) {
            ...
            $recordMetrics = \true;
            if ($content === $indentation && isset($tokens[$i + 1]) === \true && $tokens[$i]['line'] < $tokens[$i + 1]['line']) {
