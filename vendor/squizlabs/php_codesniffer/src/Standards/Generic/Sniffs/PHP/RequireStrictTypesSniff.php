...
class RequireStrictTypesSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if ($declare !== \false) {
            ...
            do {
                $next = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $next + 1, $tokens[$declare]['parenthesis_closer'], \true);
                if ($next !== \false && $tokens[$next]['code'] === \T_STRING && strtolower($tokens[$next]['content']) === 'strict_types') {
        ...
        $skip[] = \T_EQUAL;
        $valuePtr = $phpcsFile->findNext($skip, $next + 1, null, \true);
        if ($valuePtr !== \false && $tokens[$valuePtr]['code'] === \T_LNUMBER && $tokens[$valuePtr]['content'] === '0') {
