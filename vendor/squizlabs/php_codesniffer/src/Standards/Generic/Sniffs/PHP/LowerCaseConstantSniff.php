...
class LowerCaseConstantSniff implements Sniff
{
     ...
     */
    private const TARGET_TOKENS = [\T_TRUE => \T_TRUE, \T_FALSE => \T_FALSE, \T_NULL => \T_NULL];
    /**
     * Token types which can be encountered in a property type declaration.
     *
     * @var array<int|string, int|string>
     */
    private const PROPERTY_TYPE_TOKENS = Tokens::NAME_TOKENS + [\T_CALLABLE => \T_CALLABLE, \T_SELF => \T_SELF, \T_PARENT => \T_PARENT, \T_FALSE => \T_FALSE, \T_TRUE => \T_TRUE, \T_NULL => \T_NULL, \T_TYPE_UNION => \T_TYPE_UNION, \T_TYPE_INTERSECTION => \T_TYPE_INTERSECTION, \T_TYPE_OPEN_PARENTHESIS => \T_TYPE_OPEN_PARENTHESIS, \T_TYPE_CLOSE_PARENTHESIS => \T_TYPE_CLOSE_PARENTHESIS, \T_NULLABLE => \T_NULLABLE];
    /**
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
         ...
         */
        if (isset(Tokens::SCOPE_MODIFIERS[$tokens[$stackPtr]['code']]) === \true || $tokens[$stackPtr]['code'] === \T_VAR || $tokens[$stackPtr]['code'] === \T_STATIC || $tokens[$stackPtr]['code'] === \T_READONLY || $tokens[$stackPtr]['code'] === \T_FINAL || $tokens[$stackPtr]['code'] === \T_ABSTRACT) {
            $skipOver = Tokens::EMPTY_TOKENS + self::PROPERTY_TYPE_TOKENS;
            $skipTo = $phpcsFile->findNext($skipOver, $stackPtr + 1, null, \true);
            if ($skipTo !== \false) {
                return $skipTo;
            }
            // If we're at the end of the file, just return.
        ...
        if ($tokens[$stackPtr]['code'] === \T_FUNCTION || $tokens[$stackPtr]['code'] === \T_CLOSURE || $tokens[$stackPtr]['code'] === \T_FN) {
            ...
            if (isset($tokens[$stackPtr]['scope_opener']) === \true) {
                ...
                $skipTo = $phpcsFile->findNext([\T_SEMICOLON, \T_OPEN_CURLY_BRACKET], $end + 1, null, \false, null, \true);
                if ($skipTo !== \false) {
                    $end = $skipTo;
                }
            }
            ...
            foreach ($params as $param) {
                ...
                for ($i = $param['default_token']; $i < $paramEnd; $i++) {
                    if (isset(self::TARGET_TOKENS[$tokens[$i]['code']]) === \true) {
