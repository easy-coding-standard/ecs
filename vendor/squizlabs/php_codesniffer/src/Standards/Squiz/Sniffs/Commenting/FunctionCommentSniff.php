...
class FunctionCommentSniff extends PEARFunctionCommentSniff
{
    ...
    protected function processReturn(File $phpcsFile, int $stackPtr, int $commentStart)
    {
        ...
        if ($this->skipIfInheritdoc === \true) {
            if ($this->checkInheritdoc($phpcsFile, $stackPtr, $commentStart) === \true) {
                return;
            }
        }
        ...
        if ($return !== null) {
            ...
            if (empty($content) === \true || $tokens[$return + 2]['code'] !== \T_DOC_COMMENT_STRING) {
                ...
                if ($returnType === 'void') {
                    if (isset($tokens[$stackPtr]['scope_closer']) === \true) {
                        ...
                        for ($returnToken = $stackPtr; $returnToken < $endToken; $returnToken++) {
                            ...
                            if ($tokens[$returnToken]['code'] === \T_RETURN || $tokens[$returnToken]['code'] === \T_YIELD || $tokens[$returnToken]['code'] === \T_YIELD_FROM) {
                                break;
                            }
                        }
                ...
                } elseif ($returnType !== 'mixed' && $returnType !== 'never' && in_array('void', $typeNames, \true) === \false) {
    ...
    protected function processThrows(File $phpcsFile, int $stackPtr, int $commentStart)
    {
        ...
        foreach ($tokens[$commentStart]['comment_tags'] as $pos => $tag) {
            ...
            if ($tokens[$tag + 2]['code'] === \T_DOC_COMMENT_STRING) {
                ...
                $exception = $matches[1];
                if (isset($matches[2]) === \true && trim($matches[2]) !== '') {
                    $comment = $matches[2];
                }
            }
            if ($exception === null) {
    ...
    protected function processParams(File $phpcsFile, int $stackPtr, int $commentStart)
    {
        ...
        foreach ($tokens[$commentStart]['comment_tags'] as $pos => $tag) {
            ...
            if ($tokens[$tag + 2]['code'] === \T_DOC_COMMENT_STRING) {
                ...
                if ($tokens[$tag + 2]['content'][0] === '$') {
                    ...
                    if (isset($matches[4]) === \true) {
                        ...
                        $comment = $matches[4];
                        $commentLines[] = ['comment' => $comment, 'token' => $tag + 2, 'indent' => $varSpace];
                        // Any strings until the next tag belong to this comment.
                        if (isset($tokens[$commentStart]['comment_tags'][$pos + 1]) === \true) {
                            $end = $tokens[$commentStart]['comment_tags'][$pos + 1];
                        } else {
                        ...
                        for ($i = $tag + 3; $i < $end; $i++) {
                            if ($tokens[$i]['code'] === \T_DOC_COMMENT_STRING) {
                                ...
                                $comment .= ' ' . $tokens[$i]['content'];
                                $commentLines[] = ['comment' => $tokens[$i]['content'], 'token' => $i, 'indent' => $indent];
                            }
                        }
                    } else {
            ...
            $params[] = ['tag' => $tag, 'type' => $type, 'var' => $var, 'comment' => $comment, 'commentLines' => $commentLines, 'type_space' => $typeSpace, 'var_space' => $varSpace];
        }
        ...
        foreach ($params as $pos => $param) {
            ...
            foreach ($typeNames as $typeName) {
                ...
                if ($suggestedTypeHint !== '' && isset($realParams[$pos]) === \true && $param['var'] !== '') {
                    ...
                    if ($typeHint === '') {
                        ...
                        $data = [$suggestedTypeHint, $typeHint, $param['var']];
                        $phpcsFile->addError($error, $stackPtr, 'IncorrectTypeHint', $data);
                    }
                } elseif ($suggestedTypeHint === '' && isset($realParams[$pos]) === \true) {
                    $typeHint = $realParams[$pos]['type_hint'];
                    if ($typeHint !== '') {
                        $error = 'Unknown type hint "%s" found for %s';
                        $data = [$typeHint, $param['var']];
            ...
            if ($param['type'] !== $suggestedType) {
                ...
                if ($fix === \true) {
                    ...
                    if (isset($param['commentLines'][0]) === \true) {
    ...
    protected function checkInheritdoc(File $phpcsFile, int $stackPtr, int $commentStart)
    {
        ...
        for ($i = $commentStart; $i <= $tokens[$commentStart]['comment_closer']; $i++) {
            if (in_array($tokens[$i]['code'], $allowedTokens, \true) === \false) {
