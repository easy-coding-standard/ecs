...
class FunctionCommentThrowTagSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        do {
            ...
            */
            $nextToken = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $currPos + 1, null, \true);
            if ($tokens[$nextToken]['code'] === \T_NEW || isset(Tokens::NAME_TOKENS[$tokens[$nextToken]['code']]) === \true) {
                if ($tokens[$nextToken]['code'] === \T_NEW) {
                    $currException = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $nextToken + 1, $stackPtrEnd, \true);
                } else {
                    $currException = $nextToken;
                }
                if ($currException !== \false && isset(Tokens::NAME_TOKENS[$tokens[$currException]['code']]) === \true) {
                    if ($tokens[$currException]['code'] === \T_NAME_RELATIVE) {
                        ...
                        $thrownExceptions[] = (string) substr($tokens[$currException]['content'], 10);
                    } else {
                ...
                $catch = $phpcsFile->findPrevious(\T_CATCH, $currPos, $tokens[$stackPtr]['scope_opener'], \false, null, \false);
                if ($catch !== \false) {
                    $thrownVar = $phpcsFile->findPrevious(\T_VARIABLE, $tokens[$catch]['parenthesis_closer'] - 1, $tokens[$catch]['parenthesis_opener']);
