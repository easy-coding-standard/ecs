...
class PostStatementCommentSniff implements Sniff
{
     ...
     */
    private const CONTROL_STRUCTURE_EXCEPTIONS = [\T_IF => \true, \T_ELSEIF => \true, \T_SWITCH => \true, \T_WHILE => \true, \T_FOR => \true, \T_FOREACH => \true, \T_MATCH => \true];
    /**
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['nested_parenthesis']) === \true) {
            ...
            foreach ($nestedParens as $open => $close) {
                if (isset($tokens[$open]['parenthesis_owner']) === \true && isset(self::CONTROL_STRUCTURE_EXCEPTIONS[$tokens[$tokens[$open]['parenthesis_owner']]['code']]) === \true) {
                    return;
                }
            }
        }
