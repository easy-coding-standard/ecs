...
class InlineCommentSniff implements Sniff
{
     ...
     */
    private const VALID_SENTENCE_END_CHARS = ['full-stops' => '.', 'exclamation marks' => '!', 'or question marks' => '?'];
    /**
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_DOC_COMMENT_OPEN_TAG) {
            $nextToken = $stackPtr;
            do {
                $nextToken = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $nextToken + 1, null, \true);
            ...
            $ignore = [\T_CLASS, \T_INTERFACE, \T_TRAIT, \T_ENUM, \T_FUNCTION, \T_CLOSURE, \T_PUBLIC, \T_PRIVATE, \T_PROTECTED, \T_FINAL, \T_STATIC, \T_ABSTRACT, \T_READONLY, \T_CONST, \T_INCLUDE, \T_INCLUDE_ONCE, \T_REQUIRE, \T_REQUIRE_ONCE];
            if (in_array($tokens[$nextToken]['code'], $ignore, \true) === \true) {
                return;
            }
            $prevToken = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $stackPtr - 1, null, \true);
        ...
        while (($nextComment = $phpcsFile->findNext(\T_COMMENT, $nextComment + 1, null, \false)) !== \false) {
            ...
            // comment block.
            $prevNonWhitespace = $phpcsFile->findPrevious(\T_WHITESPACE, $nextComment - 1, $lastComment, \true);
        ...
        foreach ($commentTokens as $lastCommentToken) {
            ...
            if (trim((string) substr($comment, 2)) === '') {
                continue;
            }
            $spaceCount = 0;
            $tabFound = \false;
            ...
            $commentText .= trim((string) substr($tokens[$lastCommentToken]['content'], 2));
        }
        if ($commentText === '') {
        ...
        if (preg_match('/^\p{L}/u', $commentText) === 1) {
            ...
            if (in_array($commentCloser, self::VALID_SENTENCE_END_CHARS, \true) === \false) {
        ...
        if ($tokens[$previousContent]['line'] < $tokens[$stackPtr]['line']) {
            $next = $phpcsFile->findNext(\T_WHITESPACE, $lastCommentToken + 1, null, \true);
            if ($next === \false) {
