...
class OperatorSpacingSniff implements Sniff
{
     ...
     * @var boolean
     */
    public $ignoreSpacingBeforeAssignments = \true;
    /**
    ...
    public function register()
    {
        ...
        */
        ...
        $this->nonOperandTokens += [\T_RETURN => \T_RETURN, \T_ECHO => \T_ECHO, \T_EXIT => \T_EXIT, \T_PRINT => \T_PRINT, \T_YIELD => \T_YIELD, \T_FN_ARROW => \T_FN_ARROW, \T_MATCH_ARROW => \T_MATCH_ARROW];
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_BITWISE_AND) {
            ...
            $hasNext = $phpcsFile->findNext(\T_WHITESPACE, $stackPtr + 1, null, \true);
            if ($hasNext === \false) {
        ...
        if ($tokens[$stackPtr - 1]['code'] !== \T_WHITESPACE && ($tokens[$stackPtr - 1]['code'] === \T_INLINE_THEN && $tokens[$stackPtr]['code'] === \T_INLINE_ELSE) === \false) {
            ...
            $phpcsFile->recordMetric($stackPtr, 'Space before operator', 0);
        } elseif (isset(Tokens::ASSIGNMENT_TOKENS[$tokens[$stackPtr]['code']]) === \false || $this->ignoreSpacingBeforeAssignments === \false) {
            ...
            if ($found !== 1 && ($found !== 'newline' || $this->ignoreNewlines === \false)) {
                ...
                $data = [$operator, $found];
                if (isset(Tokens::COMMENT_TOKENS[$tokens[$prevNonWhitespace]['code']]) === \true) {
        ...
        if ($tokens[$stackPtr + 1]['code'] !== \T_WHITESPACE) {
            ...
            if (isset($tokens[$stackPtr + 2]) === \true && $tokens[$stackPtr + 2]['line'] !== $tokens[$stackPtr]['line']) {
                $found = 'newline';
            } else {
                $found = $tokens[$stackPtr + 1]['length'];
            }
            ...
            if ($found !== 1 && ($found !== 'newline' || $this->ignoreNewlines === \false)) {
                ...
                if ($nextNonWhitespace !== \false && isset(Tokens::COMMENT_TOKENS[$tokens[$nextNonWhitespace]['code']]) === \true && $found === 'newline') {
