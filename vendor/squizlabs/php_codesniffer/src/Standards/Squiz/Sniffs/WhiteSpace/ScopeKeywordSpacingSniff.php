...
class ScopeKeywordSpacingSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        $nextToken = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $stackPtr + 1, null, \true);
        if ($tokens[$stackPtr]['code'] === \T_STATIC) {
            if ($nextToken === \false || $tokens[$nextToken]['code'] === \T_DOUBLE_COLON || $tokens[$prevToken]['code'] === \T_NEW || $tokens[$prevToken]['code'] === \T_INSTANCEOF) {
            ...
            if ($prevToken !== \false && $tokens[$prevToken]['code'] === \T_COLON) {
                $prevPrevToken = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $prevToken - 1, null, \true);
        ...
        if (empty($tokens[$stackPtr]['nested_parenthesis']) === \false) {
            ...
            if (isset($tokens[$lastCloseParens]['parenthesis_owner']) === \true && $tokens[$tokens[$lastCloseParens]['parenthesis_owner']]['code'] === \T_FUNCTION) {
                $isInFunctionDeclaration = \true;
            }
        }
        if ($nextToken !== \false && $tokens[$nextToken]['code'] === \T_VARIABLE && $isInFunctionDeclaration === \false) {
            ...
            if ($multiProperty !== \false && $tokens[$stackPtr]['line'] !== $tokens[$nextToken]['line'] && $tokens[$nextToken]['line'] !== $tokens[$endOfStatement]['line']) {
        ...
        if ($spacing !== 1) {
            ...
            $data = [$tokens[$stackPtr]['content'], $spacing];
