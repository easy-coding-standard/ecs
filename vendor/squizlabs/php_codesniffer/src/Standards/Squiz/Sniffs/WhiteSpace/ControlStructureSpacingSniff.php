...
class ControlStructureSpacingSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        for ($firstContent = $scopeOpener + 1; $firstContent < $phpcsFile->numTokens; $firstContent++) {
            ...
            if ($code === \T_WHITESPACE || $code === \T_INLINE_HTML && trim($tokens[$firstContent]['content']) === '') {
                continue;
            }
            // Skip all empty tokens on the same line as the opener.
            if ($tokens[$firstContent]['line'] === $tokens[$scopeOpener]['line'] && (isset(Tokens::EMPTY_TOKENS[$code]) === \true || $code === \T_CLOSE_TAG)) {
                continue;
            }
            break;
        }
        ...
        if ($firstContent !== $scopeCloser) {
            ...
            $lastNonEmptyContent = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $scopeCloser - 1, null, \true);
        ...
        if ($tokens[$stackPtr]['code'] === \T_MATCH) {
            ...
            $next = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $scopeCloser + 1, null, \true);
        ...
        if ($tokens[$trailingContent]['code'] === \T_COMMENT || isset(Tokens::PHPCS_ANNOTATION_TOKENS[$tokens[$trailingContent]['code']]) === \true) {
            // Special exception for code where the comment about
            // an ELSE or ELSEIF is written between the control structures.
            $nextCode = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $scopeCloser + 1, null, \true);
        ...
        if (isset($tokens[$trailingContent]['scope_condition']) === \true && $tokens[$trailingContent]['scope_condition'] !== $trailingContent && isset($tokens[$trailingContent]['scope_opener']) === \true && $tokens[$trailingContent]['scope_opener'] !== $trailingContent) {
        ...
        } elseif ($tokens[$trailingContent]['code'] !== \T_ELSE && $tokens[$trailingContent]['code'] !== \T_ELSEIF && $tokens[$trailingContent]['code'] !== \T_CATCH && $tokens[$trailingContent]['code'] !== \T_FINALLY && $tokens[$trailingContent]['line'] === $tokens[$scopeCloser]['line'] + 1) {
            $error = 'No blank line found after control structure';
            $fix = $phpcsFile->addFixableError($error, $scopeCloser, 'NoLineAfterClose');
            if ($fix === \true) {
                $trailingContent = $phpcsFile->findNext(\T_WHITESPACE, $scopeCloser + 1, null, \true);
                if (($tokens[$trailingContent]['code'] === \T_COMMENT || isset(Tokens::PHPCS_ANNOTATION_TOKENS[$tokens[$trailingContent]['code']]) === \true) && $tokens[$trailingContent]['line'] === $tokens[$scopeCloser]['line']) {
