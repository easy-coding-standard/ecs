...
class ControlSignatureSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        $expected = 1;
        if (isset($tokens[$stackPtr]['parenthesis_closer']) === \false && $isAlternative === \true) {
            // Catching cases like:
        ...
        if (isset($tokens[$stackPtr]['scope_opener']) === \true) {
            ...
            for ($next = $opener + 1; $next < $phpcsFile->numTokens; $next++) {
                ...
                if ($tokens[$next]['line'] === $tokens[$opener]['line'] && (isset(Tokens::EMPTY_TOKENS[$code]) === \true || $code === \T_CLOSE_TAG)) {
                    continue;
                }
            ...
            $closer = $tokens[$stackPtr]['parenthesis_closer'];
            $nextNonEmpty = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $closer + 1, null, \true);
        ...
        if ($tokens[$stackPtr]['code'] === \T_WHILE) {
            ...
            $closer = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $stackPtr - 1, null, \true);
            if ($closer === \false || $tokens[$closer]['code'] !== \T_CLOSE_CURLY_BRACKET || $tokens[$tokens[$closer]['scope_condition']]['code'] !== \T_DO) {
                return;
            }
        } elseif ($tokens[$stackPtr]['code'] === \T_ELSE || $tokens[$stackPtr]['code'] === \T_ELSEIF || $tokens[$stackPtr]['code'] === \T_CATCH || $tokens[$stackPtr]['code'] === \T_FINALLY) {
            if (isset($tokens[$stackPtr]['scope_opener']) === \true && $tokens[$tokens[$stackPtr]['scope_opener']]['code'] === \T_COLON) {
