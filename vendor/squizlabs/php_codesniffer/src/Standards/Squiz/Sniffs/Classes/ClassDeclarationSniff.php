...
class ClassDeclarationSniff extends PSR2ClassDeclarationSniff
{
    ...
    public function processOpen(File $phpcsFile, int $stackPtr)
    {
        ...
        if ($tokens[$stackPtr - 1]['code'] === \T_WHITESPACE) {
            ...
            if ($prevContent !== $phpcsFile->eolChar) {
                $blankSpace = (string) substr($prevContent, strpos($prevContent, $phpcsFile->eolChar));
    ...
    public function processClose(File $phpcsFile, int $stackPtr)
    {
        ...
        for ($nextContent = $closeBrace + 1; $nextContent < $phpcsFile->numTokens; $nextContent++) {
            ...
            if ($tokens[$nextContent]['line'] === $tokens[$closeBrace]['line'] && ($tokens[$nextContent]['code'] === \T_WHITESPACE || $tokens[$nextContent]['code'] === \T_COMMENT || isset(Tokens::PHPCS_ANNOTATION_TOKENS[$tokens[$nextContent]['code']]) === \true)) {
                continue;
            }
        ...
        if ($difference !== -1 && $difference !== 1) {
            for ($nextSignificant = $nextContent; $nextSignificant < $phpcsFile->numTokens; $nextSignificant++) {
                ...
                if ($tokens[$nextSignificant]['code'] === \T_ATTRIBUTE && isset($tokens[$nextSignificant]['attribute_closer']) === \true) {
