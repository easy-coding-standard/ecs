...
class ArrayDeclarationSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_OPEN_SHORT_ARRAY && isset($tokens[$stackPtr]['nested_parenthesis']) === \true) {
            $nestedParens = $tokens[$stackPtr]['nested_parenthesis'];
            $lastParenthesisCloser = end($nestedParens);
            $lastParenthesisOpener = key($nestedParens);
            if (isset($tokens[$lastParenthesisCloser]['parenthesis_owner']) === \true && $tokens[$tokens[$lastParenthesisCloser]['parenthesis_owner']]['code'] === \T_FOREACH) {
    ...
    public function processMultiLineArray(File $phpcsFile, int $stackPtr, int $arrayStart, int $arrayEnd)
    {
        ...
        for ($nextToken = $stackPtr + 1; $nextToken < $arrayEnd; $nextToken++) {
            ...
            if ($tokens[$nextToken]['code'] === \T_ARRAY || $tokens[$nextToken]['code'] === \T_OPEN_SHORT_ARRAY || $tokens[$nextToken]['code'] === \T_CLOSURE || $tokens[$nextToken]['code'] === \T_FN || $tokens[$nextToken]['code'] === \T_MATCH) {
            ...
            if ($tokens[$nextToken]['code'] === \T_COMMA) {
                ...
                if ($keyUsed === \true && $tokens[$lastToken]['code'] === \T_COMMA) {
                    $nextToken = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $lastToken + 1, null, \true);
                ...
                if ($keyUsed === \false) {
                    if ($tokens[$nextToken - 1]['code'] === \T_WHITESPACE) {
                        $prev = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $nextToken - 1, null, \true);
                    ...
                    $valueContent = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $lastToken + 1, $nextToken, \true);
                    $indices[] = ['value' => $valueContent];
                    $usesArrayUnpacking = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $nextToken - 2, null, \true);
            ...
            if ($tokens[$nextToken]['code'] === \T_DOUBLE_ARROW) {
                ...
                $nextContent = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $nextToken + 1, $arrayEnd, \true);
        ...
        */
        if ($keyUsed === \false && empty($indices) === \false) {
            ...
            $trailingContent = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $arrayEnd - 1, $lastIndex, \true);
            ...
            foreach ($indices as $valuePosition => $value) {
                ...
                } elseif ($previousIsWhitespace === \true) {
        ...
        */
        ...
        foreach ($indices as $valuePosition => $index) {
            ...
            if ($end === \false) {
                ...
                $valueEnd = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $end - 1, $valueStart, \true);
                $nextComma = $end;
            } else {
                $valueEnd = $end;
                $next = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $end + 1, $arrayEnd, \true);
            ...
            if ($nextComma !== \false && $tokens[$nextComma - 1]['code'] === \T_WHITESPACE) {
                ...
                $prev = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $nextComma - 1, null, \true);
