...
class DisallowMultipleAssignmentsSniff implements Sniff
{
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if (empty($tokens[$stackPtr]['conditions']) === \false) {
            ...
            if (isset(Tokens::OO_SCOPE_TOKENS[$tokens[$deepestScope]['code']]) === \true) {
                return;
            }
        }
        /*
        ...
        */
        for ($varToken = $stackPtr - 1; $varToken >= 0; $varToken--) {
            if (in_array($tokens[$varToken]['code'], [\T_SEMICOLON, \T_OPEN_CURLY_BRACKET, \T_CLOSE_TAG], \true) === \true) {
            ...
            if ($tokens[$varToken]['code'] === \T_VARIABLE) {
                $prevNonEmpty = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $varToken - 1, null, \true);
        ...
        if ($tokens[$varToken]['code'] === \T_VARIABLE || $tokens[$varToken]['code'] === \T_OPEN_TAG || $tokens[$varToken]['code'] === \T_GOTO_COLON || $tokens[$varToken]['code'] === \T_INLINE_THEN || $tokens[$varToken]['code'] === \T_INLINE_ELSE || $tokens[$varToken]['code'] === \T_SEMICOLON || $tokens[$varToken]['code'] === \T_CLOSE_PARENTHESIS || isset($allowed[$tokens[$varToken]['code']]) === \true) {
            return;
        }
        $error = 'Assignments must be the first block of code on a line';
        $errorCode = 'Found';
        if (isset($nested) === \true) {
            $controlStructures = [\T_IF => \T_IF, \T_ELSEIF => \T_ELSEIF, \T_SWITCH => \T_SWITCH, \T_FOR => \T_FOR, \T_MATCH => \T_MATCH];
            foreach ($nested as $opener => $closer) {
