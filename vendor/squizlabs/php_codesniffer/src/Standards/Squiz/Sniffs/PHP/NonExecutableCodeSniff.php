...
class NonExecutableCodeSniff implements Sniff
{
     ...
     * @var array<int, int>
     */
    private const EXPRESSION_TOKENS = [\T_EXIT => \T_EXIT, \T_THROW => \T_THROW];
    /**
    ...
    public function process(File $phpcsFile, int $stackPtr)
    {
        ...
        if (isset(self::EXPRESSION_TOKENS[$tokens[$stackPtr]['code']]) === \true) {
            ...
            if (isset(Tokens::BOOLEAN_OPERATORS[$tokens[$prev]['code']]) === \true && ($tokens[$stackPtr]['code'] === \T_THROW && $tokens[$prev]['code'] === \T_LOGICAL_XOR) === \false) {
                return;
            }
        ...
        if ($tokens[$prev]['code'] === \T_ELSE || isset($tokens[$prev]['parenthesis_owner']) === \true && ($tokens[$tokens[$prev]['parenthesis_owner']]['code'] === \T_IF || $tokens[$tokens[$prev]['parenthesis_owner']]['code'] === \T_ELSEIF)) {
            return;
        }
        if ($tokens[$stackPtr]['code'] === \T_RETURN) {
            $next = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $stackPtr + 1, null, \true);
            if ($tokens[$next]['code'] === \T_SEMICOLON) {
                $next = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $next + 1, null, \true);
            ...
            if ($tokens[$owner]['code'] === \T_CASE || $tokens[$owner]['code'] === \T_DEFAULT) {
                ...
                if ($next !== \false) {
                    ...
                    for ($i = $stackPtr + 1; $i < $next; $i++) {
                        if (isset(Tokens::EMPTY_TOKENS[$tokens[$i]['code']]) === \true || $tokens[$i]['code'] === \T_OPEN_TAG || $tokens[$i]['code'] === \T_CLOSE_TAG || $tokens[$i]['code'] === \T_INLINE_HTML && trim($tokens[$i]['content']) === '') {
                            continue;
                        }
                        $line = $tokens[$i]['line'];
                        if ($line > $lastLine) {
        ...
        for ($i = $start + 1; $i < $end; $i++) {
            if (isset(Tokens::EMPTY_TOKENS[$tokens[$i]['code']]) === \true || isset(Tokens::BRACKET_TOKENS[$tokens[$i]['code']]) === \true || $tokens[$i]['code'] === \T_SEMICOLON) {
                continue;
            }
            ...
            if (isset(Tokens::OO_SCOPE_TOKENS[$tokens[$i]['code']]) === \true || $tokens[$i]['code'] === \T_FUNCTION || $tokens[$i]['code'] === \T_CLOSURE) {
