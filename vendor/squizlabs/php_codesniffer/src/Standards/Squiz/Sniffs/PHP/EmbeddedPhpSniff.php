...
class EmbeddedPhpSniff implements Sniff
{
    ...
    private function validateMultilineEmbeddedPhp(File $phpcsFile, int $stackPtr, $closingTag)
    {
        ...
        if ($closingTag !== \false) {
            $firstContentAfterBlock = $phpcsFile->findNext(\T_WHITESPACE, $closingTag + 1, $phpcsFile->numTokens, \true);
        ...
        } else if (isset($tokens[$firstContent]['scope_closer']) === \false || $tokens[$firstContent]['scope_closer'] !== $firstContent) {
        ...
        $lastContentBeforeBlock = $phpcsFile->findPrevious(\T_WHITESPACE, $stackPtr - 1, null, \true);
        if ($tokens[$lastContentBeforeBlock]['line'] === $tokens[$stackPtr]['line'] && ($tokens[$lastContentBeforeBlock]['code'] === \T_INLINE_HTML && trim($tokens[$lastContentBeforeBlock]['content']) !== '' || $tokens[$lastContentBeforeBlock - 1]['code'] !== \T_INLINE_HTML && $tokens[$lastContentBeforeBlock - 1]['line'] === $tokens[$stackPtr]['line'])) {
            $error = 'Opening PHP tag must be on a line by itself';
            ...
            if ($fix === \true) {
                ...
                if ($tokens[$stackPtr - 1]['code'] === \T_INLINE_HTML && trim($tokens[$stackPtr - 1]['content']) === '') {
                    $phpcsFile->fixer->replaceToken($stackPtr - 1, '');
                }
                $phpcsFile->fixer->endChangeset();
            }
        ...
        $firstContentAfterBlock = $phpcsFile->findNext(\T_WHITESPACE, $closingTag + 1, null, \true);
        if ($tokens[$lastContent]['line'] === $tokens[$closingTag]['line']) {
            ...
            if ($fix === \true) {
                ...
                if ($tokens[$firstContent]['line'] === $tokens[$lastContent]['line'] && isset($indent) === \true) {
                    $closerIndent = $indent;
                } else {
        ...
        } elseif ($firstContentAfterBlock !== \false && $tokens[$firstContentAfterBlock]['line'] === $tokens[$closingTag]['line']) {
            $error = 'Closing PHP tag must be on a line by itself';
        ...
        $lastNonEmpty = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $closingTag - 1, $stackPtr + 1, \true);
        if ((isset($tokens[$lastNonEmpty]['scope_closer']) === \false || $tokens[$lastNonEmpty]['scope_closer'] !== $lastNonEmpty) && $tokens[$lastContent]['line'] < $tokens[$closingTag]['line'] - 1) {
            // Find a token on the blank line to throw the error on.
            $i = $closingTag;
            do {
                $i--;
    ...
    private function validateInlineEmbeddedPhp(File $phpcsFile, int $stackPtr, int $closeTag)
    {
        ...
        $prev = $phpcsFile->findPrevious(Tokens::EMPTY_TOKENS, $closeTag - 1, $stackPtr, \true);
        if ($prev !== $stackPtr) {
        ...
        } elseif (($tokens[$closeTag - 1]['code'] === \T_COMMENT || isset(Tokens::PHPCS_ANNOTATION_TOKENS[$tokens[$closeTag - 1]['code']]) === \true) && substr($tokens[$closeTag - 1]['content'], -1) === ' ') {
        ...
        if ($trailingSpace !== 1) {
            ...
            if ($fix === \true) {
                ...
                } elseif ($tokens[$closeTag - 1]['code'] === \T_COMMENT || isset(Tokens::PHPCS_ANNOTATION_TOKENS[$tokens[$closeTag - 1]['code']]) === \true) {
    ...
    private function reportEmptyTagSet(File $phpcsFile, int $stackPtr, int $closeTag)
    {
        ...
        if ($fix === \true) {
            ...
            if (isset($tokens[$closeTag + 1]) === \true && $tokens[$closeTag + 1]['line'] !== $tokens[$closeTag]['line'] && $tokens[$stackPtr - 1]['code'] === \T_INLINE_HTML && $tokens[$stackPtr - 1]['line'] === $tokens[$stackPtr]['line'] && $tokens[$stackPtr - 1]['column'] === 1 && trim($tokens[$stackPtr - 1]['content']) === '') {
                $phpcsFile->fixer->replaceToken($stackPtr - 1, '');
            }
            $phpcsFile->fixer->endChangeset();
        }
    ...
    private function calculateLineIndent(File $phpcsFile, int $stackPtr)
    {
        ...
        while ($tokens[$firstOnLine]['code'] === \T_COMMENT && $tokens[$firstOnLine]['content'] !== ltrim($tokens[$firstOnLine]['content'])) {
        ...
        } elseif ($tokens[$firstOnLine]['code'] === \T_INLINE_HTML || $tokens[$firstOnLine]['code'] === \T_END_HEREDOC || $tokens[$firstOnLine]['code'] === \T_END_NOWDOC) {
