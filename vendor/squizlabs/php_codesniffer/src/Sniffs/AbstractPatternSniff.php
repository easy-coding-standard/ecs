...
abstract class AbstractPatternSniff implements \PHP_CodeSniffer\Sniffs\Sniff
{
        ...
        if ($patternInfo['listen_pos'] > 0) {
            ...
            for ($i = $patternInfo['listen_pos'] - 1; $i >= 0; $i--) {
                if ($pattern[$i]['type'] === 'token') {
                    ...
                    if ($this->ignoreComments === \true && isset(Tokens::COMMENT_TOKENS[$tokens[$stackPtr]['code']]) === \true) {
                        $startComment = $phpcsFile->findPrevious(Tokens::COMMENT_TOKENS, $stackPtr - 1, null, \true);
                    ...
                    if ($tokens[$stackPtr]['code'] === \T_WHITESPACE) {
                        if ($tokens[$stackPtr]['content'] !== $phpcsFile->eolChar) {
                            ...
                            if ($tokens[$stackPtr - 1]['content'] !== $phpcsFile->eolChar && ($this->ignoreComments === \true && isset(Tokens::COMMENT_TOKENS[$tokens[$stackPtr - 1]['code']]) === \false)) {
                                $hasError = \true;
                            } else {
                                $stackPtr--;
                            }
            ...
            if ($pattern[$i]['type'] === 'token') {
                if ($pattern[$i]['token'] === \T_WHITESPACE) {
                    if ($this->ignoreComments === \true) {
                        ...
                        if (isset(Tokens::COMMENT_TOKENS[$tokens[$stackPtr]['code']]) === \true) {
                            continue;
                        }
                        // If the next token is a comment, the we need to skip the
                        // current token as we should allow a space before a
                        // comment for readability.
                        if (isset($tokens[$stackPtr + 1]) === \true && isset(Tokens::COMMENT_TOKENS[$tokens[$stackPtr + 1]['code']]) === \true) {
                            continue;
                        }
                    }
                    $tokenContent = '';
                    if ($tokens[$stackPtr]['code'] === \T_WHITESPACE) {
                        if (isset($pattern[$i + 1]) === \false) {
                            ...
                            $next = $phpcsFile->findNext(Tokens::EMPTY_TOKENS, $stackPtr, null, \true);
                    ...
                    } else if ($stackPtr !== $lastAddedStackPtr) {
                    ...
                    } else if ($tokenContent !== $pattern[$i]['value']) {
                        $hasError = \true;
                    }
                } else {
                    // Check to see if this important token is the same as the
                    ...
                    if ($next - $stackPtr > 0) {
                        ...
                        for ($j = $stackPtr; $j < $next; $j++) {
                            ...
                            if (isset(Tokens::COMMENT_TOKENS[$tokens[$j]['code']]) === \true) {
                                $hasComment = \true;
                            }
                        }
                ...
                if ($pattern[$i]['to'] === 'unknown') {
                    ...
                    // Find the previous opener.
                    $next = $phpcsFile->findPrevious(Tokens::BLOCK_OPENERS, $stackPtr);
                ...
                if ($newline === 0) {
                    ...
                    if ($this->ignoreComments === \false) {
                        ...
                        if (isset(Tokens::COMMENT_TOKENS[$tokens[$newline]['code']]) === \true) {
                            $hasError = \true;
                        }
                    }
                    if ($newline === $stackPtr) {
    ...
    private function parse(string $pattern)
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            } elseif ((string) substr($pattern, $i, 3) === 'abc') {
                $specialPattern = ['type' => 'string'];
                $lastToken = $i - $firstToken;
                $firstToken = $i + 3;
                $i += 2;
            } elseif ((string) substr($pattern, $i, 3) === 'EOL') {
