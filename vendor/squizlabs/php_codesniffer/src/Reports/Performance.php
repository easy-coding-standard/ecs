...
class Performance implements \PHP_CodeSniffer\Reports\Report
{
    /**
     ...
     * @return void
     */
    public function generate(string $cachedData, int $totalFiles, int $totalErrors, int $totalWarnings, int $totalFixable, bool $showSources = \false, int $width = 80, bool $interactive = \false, bool $toScreen = \true)
    {
        ...
        arsort($totalTimes);
        echo \PHP_EOL . "\x1b[1m" . 'PHP CODE SNIFFER SNIFF PERFORMANCE REPORT' . "\x1b[0m" . \PHP_EOL;
        echo str_repeat('-', $width) . \PHP_EOL;
        echo "\x1b[1m" . 'SNIFF' . str_repeat(' ', $width - 31) . 'TIME TAKEN (SECS)     (%)' . "\x1b[0m" . \PHP_EOL;
        echo str_repeat('-', $width) . \PHP_EOL;
        ...
        $formatBold = "\x1b[1m%- {$maxNameWidth}.{$maxNameWidth}s % 12.6f (% 5.1f %%)\x1b[0m" . \PHP_EOL;
        $formatWarning = "%- {$maxNameWidth}.{$maxNameWidth}s \x1b[33m% 12.6f (% 5.1f %%)\x1b[0m" . \PHP_EOL;
        $formatError = "%- {$maxNameWidth}.{$maxNameWidth}s \x1b[31m% 12.6f (% 5.1f %%)\x1b[0m" . \PHP_EOL;
