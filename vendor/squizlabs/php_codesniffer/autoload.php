...
if (\class_exists('PHP_CodeSniffer\\Autoload', \false) === \false) {
    class Autoload
    {
        ...
        public static function load(string $className)
        {
            ...
            if ($path === \false) {
                foreach (self::$searchPaths as $searchPath => $nsPrefix) {
                    $className = $className;
                    if ($nsPrefix !== '' && (string) \substr($className, 0, \strlen($nsPrefix)) === $nsPrefix) {
                        $className = (string) \substr($className, \strlen($nsPrefix) + 1);
                    }
        ...
        public static function determineLoadedClass(array $classesBeforeLoad, array $classesAfterLoad)
        {
            ...
            $newClasses = \array_reduce($newClasses, static function ($remaining, $current) {
